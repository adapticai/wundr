...
const analysisUtils = {
  calculateComplexity: (entities: any[]) => {
    return (
      entities.reduce((sum, entity) => sum + entity.complexity, 0) /
      entities.length
    );
  },
  ...
  generateRecommendations: (entities: any[], duplicates: any[]) => {
    ...
    const entitiesWithIssues = entities.filter(
      e => e.issues && e.issues.length > 0
    );
...
describe('Analysis Utils', () => {
  ...
  describe('Complexity Analysis', () => {
    it('calculates average complexity from real data', () => {
      ...
      if (entities.length > 0) {
        ...
        const manualAvg =
          entities.reduce((sum: number, e: any) => sum + e.complexity, 0) /
          entities.length;
    ...
    it('finds high complexity entities with real threshold', () => {
      ...
      const highComplexity = analysisUtils.findHighComplexityEntities(
        entities,
        threshold
      );
      ...
      const manualHighComplexity = entities.filter(
        (e: any) => e.complexity > threshold
      );
  ...
  describe('Entity Grouping', () => {
    it('groups entities by type using real data', () => {
      ...
      // Verify counts
      const totalCounted = Object.values(grouped).reduce(
  ...
  describe('Technical Debt Calculation', () => {
    ...
    it('handles zero debt scenarios', () => {
      ...
      const debt = analysisUtils.calculateTechnicalDebt(
        lowComplexityEntities,
        []
      );
      expect(debt).toBe(0);
    });
  ...
  describe('Recommendation Generation', () => {
    ...
    it('prioritizes recommendations correctly', () => {
      ...
      const complexityRec = recommendations.find(
        (r: any) => r.type === 'complexity'
      );
      const duplicateRec = recommendations.find(
        (r: any) => r.type === 'duplication'
      );
  ...
  describe('Performance with Large Datasets', () => {
    it('handles large entity collections efficiently', () => {
      ...
      const highComplexity =
        analysisUtils.findHighComplexityEntities(largeEntitySet);
