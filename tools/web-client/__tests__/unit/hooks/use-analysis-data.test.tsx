...
describe('useAnalysis Hook', () => {
  ...
  describe('Loading Sample Data', () => {
    ...
    it('handles sample data loading errors', async () => {
      ...
      (global as any).createTestFixtures = jest
        .fn()
  ...
  describe('Loading from File', () => {
    it('loads data from valid JSON file', async () => {
      ...
      const file = new File([jsonContent], 'analysis.json', {
      ...
      expect(result.current.data.entities.length).toBe(
        testFixtures.analysisData.entities.length
      );
      expect(result.current.data.duplicates.length).toBe(
        testFixtures.analysisData.duplicates.length
      );
    });
