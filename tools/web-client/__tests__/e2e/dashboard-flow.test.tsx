        ...
        <div data-testid='analysis-content'>
          ...
          {data.metrics && (
            <div data-testid='metrics'>
              ...
              <span data-testid='maintainability'>
  ...
  return (
    <div data-testid='upload-flow'>
      ...
      {uploadStatus === 'uploading' && (
        <div data-testid='upload-loading'>Uploading...</div>
      )}
      {uploadStatus === 'success' && (
        <div data-testid='upload-success'>Upload complete</div>
      )}
      {uploadStatus === 'error' && (
        <div data-testid='upload-error'>Upload failed</div>
      )}
...
describe('Dashboard E2E Flow Tests', () => {
  ...
  describe('File Upload Flow', () => {
    it('handles file upload with real JSON data', async () => {
      ...
      const file = E2ETestUtils.createMockFile(
        'analysis.json',
        jsonContent,
        'application/json'
      );
      ...
      await waitFor(
        () => {
          expect(
            screen.queryByTestId('upload-loading')
    ...
    it('handles real data filtering and search', async () => {
      ...
      if (testFixtures.analysisData.entities.length > 0) {
        ...
        await waitFor(() => {
          ...
          const initialCountNum = parseInt(
            initialCount.textContent!.match(/\d+/)![0]
          );
          const filteredCountNum = parseInt(
            filteredCount.textContent!.match(/\d+/)![0]
          );
            ...
            setTimeout(() => {
              const processed = {
                ...
                complexitySum: testFixtures.analysisData.entities.reduce(
                  (sum: number, entity: any) => sum + entity.complexity,
                  0
                ),
