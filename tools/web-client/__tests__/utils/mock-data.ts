 ...
 */
async function analyzeRealEntities(
  projectPath: string
...
function calculateRealMetrics(
  ...
  const totalLines = entities.reduce(
    (sum, e) => sum + e.metrics.linesOfCode,
    0
  );
  ...
  return {
    ...
    quality: {
      maintainabilityIndex: Math.floor(
        entities.reduce((sum, e) => sum + e.metrics.maintainabilityIndex, 0) /
          entities.length
      ),
      technicalDebt: {
      ...
      duplicateRatio:
        duplicates.length > 0
          ? (duplicates.reduce((sum, d) => sum + d.linesCount, 0) /
              totalLines) *
            100
          : 0,
    },
...
function generateRealRecommendations(
  ...
  const highComplexityEntities = entities.filter(
    e => e.complexity.cyclomatic > 15
  );
