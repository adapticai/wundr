  ...
  describe('DashboardCharts Snapshots', () => {
    it('renders consistently with minimal data', () => {
      const { container } = render(
        <DashboardCharts data={minimalCompleteTestData} />
      );
      ...
      const complexData = createCompleteAnalysisData({
        entities: Array.from({ length: 10 }, (_, i) =>
          createSimpleEntity({
            ...
            type: (['class', 'function', 'module', 'component'] as const)[
              i % 4
            ],
    ...
    it('captures error boundary rendering', () => {
      ...
      try {
        ...
        const normalizedSnapshot =
          SnapshotTestUtils.normalizeSnapshot(container);
...
function extractDOMStructure(element: Element): any {
  return {
    ...
    attributes: Array.from(element.attributes)
      .filter(
        attr => !attr.name.startsWith('data-') || attr.name === 'data-testid'
      )
      .reduce(
        (acc, attr) => {
...
function extractThemeClasses(container: Element): string[] {
  ...
  while (node) {
    ...
    const themeSpecificClasses = classes.filter(
      cls =>
        cls.includes('dark:') ||
        cls.includes('light:') ||
        cls.includes('bg-') ||
