...
describe('API Routes Integration', () => {
  ...
  describe('/api/analysis', () => {
    ...
    it('handles real project analysis data', async () => {
      ...
      const analysisId = await testDb.insert(
        'analyses',
  ...
  describe('Database Integration', () => {
    ...
    it('maintains data integrity with real analysis data', async () => {
      ...
      expect(retrievedData.duplicates.length).toBe(
        originalData.duplicates.length
      );
      expect(retrievedData.timestamp).toBe(originalData.timestamp);

      if (originalData.entities.length > 0) {
        expect(retrievedData.entities[0].name).toBe(
          originalData.entities[0].name
        );
        expect(retrievedData.entities[0].complexity).toBe(
          originalData.entities[0].complexity
        );
      }
    });
