...
export function TemplateStats({ templates }: TemplateStatsProps) {
  const totalDownloads = templates.reduce(
    (sum, template) => sum + (template.downloads || 0),
    0
  );
  const monthlyDownloads = templates.reduce(
    (sum, template) => sum + (template.usageStats?.monthly || 0),
    0
  );
  const averageRating =
  ...
  const trendingTemplates = templates.filter(
    template => template.usageStats?.trending
  );
  ...
  // Language statistics
  const languageStats = templates.reduce(
    (stats, template) => {
  ...
  const difficultyStats = templates.reduce(
    (stats, template) => {
      ...
      <div className='grid grid-cols-1 lg:grid-cols-3 gap-6'>
            ...
            {popularTemplates.map((template, index) => (
              <div key={template.id} className='flex items-center space-x-3'>
                ...
                <div className='flex-1 min-w-0'>
                  ...
                  <p className='text-xs text-muted-foreground'>
                    {(template.downloads || 0).toLocaleString()} downloads
                  </p>
                </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Templates with best user ratings</CardDescription>
          </CardHeader>
          <CardContent className='space-y-3'>
                ...
                <div key={template.id} className='p-4 border rounded-lg'>
                  ...
                  <div className='flex items-center justify-between text-xs'>
                    ...
                    <div className='flex items-center space-x-2'>
                      ...
                      <span>{template.usageStats?.monthly || 0}</span>
                    </div>
                  </div>
                </div>
              ))}
