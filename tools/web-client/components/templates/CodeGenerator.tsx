...
export function CodeGenerator({ template, customizations, onGenerate }: CodeGeneratorProps) {
    ...
    const files: GeneratedFile[] = [
      {
        ...
        content: JSON.stringify({
          name: customizations.packageName || template.name.toLowerCase().replace(/\s+/g, '-'),
...
services:
  app:
    ...
    environment:
      ...
      - PORT=3000
    ${customizations.features?.database && customizations.database?.type === 'mongodb' ? `depends_on:
      - mongodb
...
MIT`;
  ...
  const getFileIcon = (language: string) => {
    switch (language) {
      case 'typescript': return <Code2 className="h-4 w-4 text-blue-500" />;
      case 'json': return <FileText className="h-4 w-4 text-yellow-500" />;
      case 'dockerfile': return <Settings className="h-4 w-4 text-blue-600" />;
      case 'yaml': return <Settings className="h-4 w-4 text-green-500" />;
      ...
      {!generatedFiles.length ? (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Generate production-ready code based on your template and customizations
            </CardDescription>
          </CardHeader>
