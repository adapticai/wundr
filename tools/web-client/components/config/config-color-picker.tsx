...
export function ConfigColorPicker({
  ...
  const isValidRgb = useCallback((color: string) => {
    return (
      /^rgb\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*\)$/.test(
        color
      ) ||
      /^rgba\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9\.]+)\s*\)$/.test(
        color
      )
    );
  }, []);
  ...
  const isValidHsl = useCallback((color: string) => {
    return (
      /^hsl\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})%\s*,\s*([0-9]{1,3})%\s*\)$/.test(
        color
      ) ||
      /^hsla\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})%\s*,\s*([0-9]{1,3})%\s*,\s*([0-9\.]+)\s*\)$/.test(
        color
      )
    );
  }, []);
  ...
  const handleInputChange = useCallback(
    (inputValue: string) => {
      ...
      if (
        ...
        isValidRgb(inputValue) ||
        isValidHsl(inputValue)
      ) {
        onChange(inputValue);
      }
    },
        ...
        <div className='flex gap-2'>
          <div className='relative flex-1'>
            <Input
              ...
              placeholder={
                allowTransparency ? '#000000 or rgba(0,0,0,0.5)' : '#000000'
              }
              className={cn(
                'font-mono pr-20',
            ...
            <PopoverContent className='w-80 p-0'>
              <Tabs defaultValue='presets' className='w-full'>
                <TabsList
                  ...
                  style={{
                    gridTemplateColumns: showPalettes
                      ? '1fr 1fr 1fr'
                      : '1fr 1fr',
                  }}
                >
...
export function ConfigThemeColorPicker(
  props: Omit<ConfigColorPickerProps, 'colorPalettes'>
) {
...
export function ConfigBrandColorPicker(
  props: Omit<ConfigColorPickerProps, 'colorPalettes'>
) {
