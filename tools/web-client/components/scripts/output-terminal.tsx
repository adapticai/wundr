...
`;
  ...
  return (
    <Card
      className={`${isExpanded ? 'fixed inset-4 z-50' : ''} transition-all duration-300`}
    >
      <CardHeader className='pb-3'>
        <div className='flex items-center justify-between'>
          ...
          <div className='flex items-center gap-2'>
            ...
            >
              <ScrollText
                className={`h-3 w-3 ${autoScroll ? 'text-blue-600' : ''}`}
              />
            </Button>
        ...
        <div className='flex items-center justify-between text-xs text-muted-foreground'>
          <div className='flex items-center gap-4'>
            ...
            {searchTerm && (
              <span>
                Filtered: {filteredLogs.length}/{parsedLogs.length}
              </span>
            )}
          </div>
          <div className='flex items-center gap-2'>
          ...
          {filteredLogs.length === 0 ? (
            ...
            <>
              {filteredLogs.map((log, index) => (
                <div
                  key={index}
                  className='flex items-start gap-2 mb-1 hover:bg-gray-900 px-1 rounded'
                >
                  {showTimestamps && (
