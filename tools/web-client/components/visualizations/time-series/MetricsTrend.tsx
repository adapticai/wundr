    ...
    datasets: filteredSeries.map((s, index) => {
      ...
      return {
        ...
        data: processedData.map(d => ({
          x: d.timestamp,
          y:
            comparisonMode === 'percentage' && s.data[0]
    ...
    <Card>
        ...
        <div className='flex items-center justify-between'>
          ...
          <div className='flex items-center gap-2'>
            {enableComparison && (
              <Select
                ...
                onValueChange={(v: string) =>
                  setComparisonMode(v as 'absolute' | 'percentage')
                }
              >
            ...
            <Select
              value={timeRange}
              onValueChange={(v: string) =>
                setTimeRange(v as '7d' | '30d' | '90d' | '1y')
              }
            >
      ...
      <CardContent>
        <div className='space-y-4'>
              ...
              {series.map(s => {
                ...
                return (
                  <Badge
                    ...
                    onClick={() => {
                      if (isSelected && selectedMetrics.length > 1) {
                        setSelectedMetrics(
                          selectedMetrics.filter(m => m !== s.name)
                        );
                      } else if (!isSelected) {
                  ...
                  >
                    ...
                    {isSelected && trend.trend === 'up' && (
                      <TrendingUp className='w-3 h-3 text-destructive' />
                    )}
                    {isSelected && trend.trend === 'down' && (
          ...
          {showAnomalies &&
            filteredSeries.some(s =>
              detectAnomalies(s.data).some(
                (d: ProcessedDataPoint) => d.isAnomaly
              )
            ) && (
