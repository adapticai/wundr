...
import {
  ...
  PerformanceMetrics as BasePerformanceMetric,
 ...
 */
const VirtualizedMetricsList = memo(
      ...
      <ResponsiveContainer width='100%' height='100%'>
        <AreaChart data={downsampledData}>
          ...
          <XAxis
            ...
            stroke='#666'
          />
          <YAxis tickFormatter={formatBytes} stroke='#666' />
          <Tooltip
 ...
 */
const MemoryLeakIndicator = memo(
  ({ memoryStatus }: { memoryStatus: MemoryStatus | null }) => {
    if (!memoryStatus) {
      return (
        <Alert>
 ...
 */
const MetricsSummaryCards = memo(
  ({ metrics }: { metrics: PerformanceMetric[] }) => {
...
export const PerformanceDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const updateSystemMetrics = () => {
      const now = Date.now();
      const memoryMetrics = rawMetrics
        ? Object.values(rawMetrics).filter((m: any) =>
            m.name?.includes('memory')
          )
        : [];
      const latestMemory = memoryMetrics[memoryMetrics.length - 1] as
        | PerformanceDataPoint
        | undefined;
      ...
      const metrics: SystemMetrics = {
        memory: {
          used:
            browserMemory?.usedJSHeapSize || (latestMemory?.value ?? 50000000),
          total: browserMemory?.totalJSHeapSize || 100000000,
          percentage: browserMemory
            ? (browserMemory.usedJSHeapSize / browserMemory.totalJSHeapSize) *
              100
            : Math.random() * 80 + 10,
        },
        cpu: {
                  ...
                  {memoryData.length > 0 &&
                    (() => {
                      ...
                      const heapUtilization =
                        (latest.heapUsed / latest.heapTotal) * 100;
                          ...
                          <div className='space-y-2 text-sm'>
                            <div className='flex justify-between'>
                              ...
                              <span className='font-mono'>
                                {(latest.heapUsed / 1024 / 1024).toFixed(1)} MB
                              </span>
                            </div>
                            <div className='flex justify-between'>
                              <span>Heap Total:</span>
                              <span className='font-mono'>
                                {(latest.heapTotal / 1024 / 1024).toFixed(1)} MB
                              </span>
                            </div>
                            <div className='flex justify-between'>
                              <span>External:</span>
                              <span className='font-mono'>
                                {(latest.external / 1024 / 1024).toFixed(1)} MB
                              </span>
                            </div>
                            <div className='flex justify-between'>
                              <span>RSS:</span>
                              <span className='font-mono'>
                                {(latest.rss / 1024 / 1024).toFixed(1)} MB
                              </span>
                            </div>
                          </div>
                        </>
            ...
            <Card className='p-4'>
              <div className='text-center'>
                <div className='text-2xl font-bold text-blue-600'>
                  {concurrencyData[concurrencyData.length - 1]?.activeWorkers ||
                    0}
                </div>
            ...
            <Card className='p-4'>
              <div className='text-center'>
                <div className='text-2xl font-bold text-orange-600'>
                  {concurrencyData[concurrencyData.length - 1]?.queuedTasks ||
                    0}
                </div>
