...
export const BatchProgressCard: React.FC<BatchProgressCardProps> = ({
  ...
  const getElapsedTime = () => {
    ...
    const elapsed = Math.floor(
      (now.getTime() - batch.startedAt.getTime()) / 60000
    ); // minutes
  ...
  const getETA = () => {
    ...
    const elapsedMinutes = batch.startedAt
      ? Math.floor((new Date().getTime() - batch.startedAt.getTime()) / 60000)
      : 0;
    const remainingProgress = 100 - batch.progress;
    const estimatedRemaining =
      (elapsedMinutes / batch.progress) * remainingProgress;
      ...
      <CardContent className='space-y-4'>
        ...
        <div className='grid grid-cols-2 gap-4 text-sm'>
          ...
          <div>
            ...
            <span
              className={`ml-2 font-medium ${getPriorityColor(batch.priority)}`}
            >
          ...
          <div>
            ...
            <span className='ml-2 font-medium'>
              {batch.consolidationType.charAt(0).toUpperCase() +
                batch.consolidationType.slice(1)}
            </span>
          </div>
          <div>
            <span className='text-muted-foreground'>
