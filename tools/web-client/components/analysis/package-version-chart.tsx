...
export function PackageVersionChart({
  dependencies: initialDependencies = [],
}: PackageVersionChartProps) {
  const [dependencies, setDependencies] =
    useState<DependencyData[]>(initialDependencies);
  ...
  const analyzeVersions = useCallback(() => {
    const analysis: VersionAnalysis[] = dependencies.map(dep => {
      ...
      const daysSinceUpdate = dep.lastUpdated
        ? Math.floor(
            (Date.now() - new Date(dep.lastUpdated).getTime()) /
              (1000 * 60 * 60 * 24)
          )
        : 0;
      ...
      return {
        ...
        migrationGuide:
          versionComparison.updateType === 'major'
            ? `https://github.com/search?q=${dep.name}+migration+guide&type=repositories`
            : undefined,
      };
    });
  ...
  const handleExport = () => {
    const exportData = {
      summary: {
        ...
        outdatedPackages: versionAnalysis.filter(v => v.versionsBehind > 0)
          .length,
        criticalRiskPackages: versionAnalysis.filter(
        ...
        <CardHeader>
          <div className='flex justify-between items-start'>
            <div>
              ...
              <CardDescription>
                Analyze package versions, update requirements, and identify
      ...
      <Tabs defaultValue='overview' className='space-y-4'>
        ...
        <TabsContent value='overview' className='space-y-4'>
          ...
          <div className='grid grid-cols-1 md:grid-cols-4 gap-4'>
            {(['critical', 'high', 'medium', 'low'] as const).map(risk => {
              const count = versionAnalysis.filter(
              ...
              return (
                <Card key={risk}>
                  <CardHeader className='flex flex-row items-center justify-between space-y-0 pb-2'>
                    ...
                    <div
                      className={`w-3 h-3 rounded-full ${getRiskColor(risk).split(' ')[0]}`}
                    />
                  </CardHeader>
                  <CardContent>
                    ...
                    <p className='text-xs text-muted-foreground'>
                      {versionAnalysis.length > 0
                        ? Math.round((count / versionAnalysis.length) * 100)
                        : 0}
                      % of packages
                    </p>
                  </CardContent>
        ...
        <TabsContent value='detailed' className='space-y-4'>
          ...
          <Card>
            <CardHeader>
              <CardTitle>
                Package Version Details ({sortedAnalysis.length})
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className='space-y-4'>
                    ...
                    <div className='flex items-start justify-between mb-3'>
                      <div>
                        ...
                        <div className='flex items-center gap-2 mt-1'>
                          ...
                          <Badge
                            className={getUpdateTypeColor(item.updateType)}
                          >
