...
export function DependencySizeAnalyzer({
  dependencies: initialDependencies,
}: DependencySizeAnalyzerProps) {
  ...
  const calculateImpactScore = (dep: DependencyData): number => {
    const sizeWeight = dep.size / 1000000; // Size in MB
    const popularityWeight = (dep.weeklyDownloads || 0) / 100000; // Popularity factor
    const maintenanceWeight = dep.maintenance === 'active' ? 1 : 0.5;

    return Math.min(
      10,
      sizeWeight * 0.4 + popularityWeight * 0.3 + maintenanceWeight * 0.3
    );
  };
  ...
  const getRecommendation = (
    ...
    if (sizeCategory === 'huge' && impactScore > 7) {
      return 'Consider replacing with a lighter alternative';
    }
    if (sizeCategory === 'large' && dep.type === 'dependency') {
      return 'Evaluate if all features are needed';
    }
    if (dep.weeklyDownloads && dep.weeklyDownloads < 1000) {
      return 'Low adoption - verify security and maintenance';
    }
    if (dep.maintenance === 'inactive') {
      return 'Package appears unmaintained - consider alternatives';
    }
    return 'Package size and usage appear reasonable';
  };
  ...
  const calculateSizeDistribution = useCallback((analysis: SizeAnalysis[]) => {
    ...
    const distribution = ranges.map(range => {
      const packagesInRange = analysis.filter(
        a => a.size >= range.min && a.size < range.max
      );
      const totalSize = packagesInRange.reduce((sum, p) => sum + p.size, 0);

      return {
        range: range.range,
        count: packagesInRange.length,
        totalSize,
        percentage:
          totalProjectSize > 0
            ? Math.round((totalSize / totalProjectSize) * 100)
            : 0,
        packages: packagesInRange.slice(0, 5), // Top 5 in each range
      };
    });
  ...
  const analyzeDependencySizes = useCallback(() => {
    const totalSize = dependencies.reduce(
    ...
    const depSizes = dependencies
      .filter(d => d.type === 'dependency')
      .reduce((sum, d) => sum + d.size, 0);
    const devDepSizes = dependencies
      .filter(d => d.type === 'devDependency')
      .reduce((sum, d) => sum + d.size, 0);
    const peerDepSizes = dependencies
  ...
  }, [dependencies, calculateSizeDistribution]);
  ...
  const filteredAnalysis = sizeAnalysis.filter(
        ...
        <CardHeader>
          <div className='flex justify-between items-start'>
            <div>
              ...
              <CardDescription>
                Analyze package sizes, bundle impact, and identify optimization
                opportunities
              </CardDescription>
            </div>
      ...
      {bundleImpact && (
        <div className='grid grid-cols-1 md:grid-cols-4 gap-4'>
          <Card>
            ...
            <CardContent>
              <div className='text-2xl font-bold'>
                {formatBytes(bundleImpact.totalSize)}
              </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className='text-2xl font-bold'>
                {formatBytes(bundleImpact.bundleSizeEstimate)}
              </div>
              <p className='text-xs text-muted-foreground'>
                After tree shaking
              </p>
          ...
          <Card>
            ...
            <CardContent>
              <div className='text-2xl font-bold'>
                {formatBytes(bundleImpact.gzipEstimate)}
              </div>
              <p className='text-xs text-muted-foreground'>
      ...
      <Tabs defaultValue='packages' className='space-y-4'>
        ...
        <TabsContent value='packages' className='space-y-4'>
          ...
          <Card>
            <CardHeader>
              <CardTitle>
                Package Size Analysis ({sortedAnalysis.length})
              </CardTitle>
            </CardHeader>
            <CardContent>
                    ...
                    <div className='flex items-start justify-between mb-3'>
                      <div className='flex-1'>
                        <div className='flex items-center gap-2 mb-2'>
                          ...
                          <Badge
                            className={
                              getCategoryColor(item.sizeCategory) +
                              ' text-white'
                            }
                          >
                            {item.sizeCategory}
                          ...
                          <Badge
                            variant='outline'
                            className={getImpactColor(item.impactScore)}
                          >
                    ...
                    >
                      <div className='flex-1'>
                        <div className='flex items-center gap-2'>
                          ...
                          <Badge
                            className={
                              getCategoryColor(pkg.sizeCategory) +
                              ' text-white text-xs'
                            }
                          >
                            {pkg.sizeCategory}
              ...
              <div className='space-y-6'>
                ...
                <div>
                  ...
                  <div className='space-y-3'>
                    {sortedAnalysis
                      .filter(
                        item => item.impactScore >= 40 && item.impactScore < 70
                      )
                      .slice(0, 5)
                      .map((item, index) => (
                        <div
