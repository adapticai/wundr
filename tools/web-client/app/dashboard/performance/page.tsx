...
export default function PerformancePage() {
  ...
  const performanceScore = useMemo(() => {
    ...
    return {
      overall: Math.round(
        (buildTimeScore + loadTimeScore + errorScore + cacheScore) / 4
      ),
  ...
  const handleExport = useCallback(() => {
    const exportData = {
      ...
      summary: {
        ...
        averageLoadTime:
          performanceData.reduce((sum, d) => sum + d.loadTime, 0) /
          performanceData.length,
        averageBuildTime:
          performanceData.reduce((sum, d) => sum + d.buildTime, 0) /
          performanceData.length,
        averageMemoryUsage:
          performanceData.reduce((sum, d) => sum + d.memoryUsage, 0) /
          performanceData.length,
        averageCpuUsage:
          performanceData.reduce((sum, d) => sum + d.cpuUsage, 0) /
          performanceData.length,
    ...
    <div className='space-y-6 p-6'>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Key metrics and insights from the current dataset (
