...
export default function CriticalRecommendationsPage() {
  ...
  const emergencyAlerts = criticalAlerts.filter(
    alert => alert.severity === 'emergency'
  );
  const criticalCount = criticalAlerts.filter(
    alert => alert.severity === 'critical'
  ).length;
  const activeIssues = criticalAlerts.filter(
    alert => alert.status === 'active'
  ).length;
  const autoFixAvailable = criticalAlerts.filter(
    alert => alert.autoFixAvailable
  ).length;
      ...
      <div className='flex flex-1 items-center justify-center p-4'>
        <div className='text-center space-y-4'>
          ...
          <p className='text-muted-foreground max-w-md'>
            Trigger a new analysis to identify critical issues requiring
            immediate attention
          </p>
          <div className='flex gap-2 justify-center'>
    ...
    <div className='flex flex-1 flex-col gap-6 p-4'>
      ...
      <div className='flex items-center justify-between'>
        ...
        <div className='flex gap-2'>
          ...
          >
            <RefreshCw
              className={`mr-2 h-4 w-4 ${loading.isRefreshing ? 'animate-spin' : ''}`}
            />
            Refresh
          </Button>
      ...
      {emergencyAlerts.length > 0 && (
        ...
        >
          ...
          <AlertDescription>
            {emergencyAlerts.length} emergency-level issues detected requiring
            immediate attention.
            <div className='mt-2'>
              ...
              <Card className='text-center py-12'>
                <CardContent>
                  ...
                  <p className='text-muted-foreground'>
                    Great! No critical issues detected at this time.
                  </p>
                </CardContent>
              </Card>
            ) : (
...
function CriticalAlertCard({
  ...
  return (
    <Card
      className={`relative ${alert.severity === 'emergency' ? 'border-red-500 border-2 animate-pulse' : 'border-orange-200'}`}
    >
      <CardHeader>
        <div className='flex items-start justify-between'>
          ...
          <div
            className={`px-2 py-1 rounded-md text-xs font-medium border ${getStatusColor(alert.status)}`}
          >
...
function ImmediateActionsPanel({
  ...
  const immediateActions = alerts.filter(
    alert => alert.actionRequired && alert.status === 'active'
  );
  ...
  return (
    <div className='space-y-4'>
      <Alert variant='destructive'>
        ...
        <AlertDescription>
          {immediateActions.length} critical issues require immediate attention
          to prevent system impact.
        </AlertDescription>
      </Alert>
      ...
      {immediateActions.length === 0 ? (
        <Card className='text-center py-12'>
          <CardContent>
            ...
            <p className='text-muted-foreground'>
              All critical issues are being handled or resolved.
            </p>
          </CardContent>
        </Card>
      ) : (
                ...
                <div className='flex gap-2'>
                  {alert.autoFixAvailable ? (
                    ...
                    <Button
                      ...
                      variant='outline'
                      onClick={() =>
                        updateRecommendation(alert.id, 'investigating')
                      }
                    >
  ...
  return (
    <div className='space-y-4'>
      <Alert>
        ...
        <AlertDescription>
          Real-time monitoring is active. New critical issues will appear here
          automatically.
        </AlertDescription>
      </Alert>
...
// Helper functions
function mapCategoryToAlertCategory(
  category: string
): 'security' | 'performance' | 'production' | 'data' {
...
function mapStatusToAlertStatus(
  status: string
): 'active' | 'investigating' | 'fixing' | 'resolved' {
  switch (status) {
    case 'pending':
      return 'active';
    case 'in_progress':
