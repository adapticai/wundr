...
export default function CodeScanAnalysisPage() {
  ...
  const loadScanData = useCallback(async () => {
    ...
    try {
      ...
      const runningScans = result.data.scans.filter(
        s => s.status === 'running'
      );
      ...
      const errorMessage =
        _error instanceof Error ? _error.message : 'Failed to load scan data';
  ...
  const startScan = useCallback(async () => {
    ...
    try {
      ...
      // Error starting scan
      const errorMessage =
        _error instanceof Error ? _error.message : 'Failed to start scan';
      setError(errorMessage);
    } finally {
      setLoading(false);
    }
  ...
  const filteredIssues =
    currentScan?.results?.issues?.filter(issue => {
      const matchesSearch =
      ...
      const matchesFile =
        selectedFile === 'all' || issue.file.includes(selectedFile);
  ...
  return (
    <div className='space-y-6'>
      ...
      <div className='flex justify-between items-center'>
        <div>
          ...
          <p className='text-muted-foreground'>
            Comprehensive static code analysis with issue detection and quality
            metrics
          </p>
        </div>
      ...
      <Tabs
        defaultValue={currentScan?.results ? 'results' : 'configure'}
        className='space-y-4'
      >
        <TabsList className='grid w-full grid-cols-6'>
          <TabsTrigger value='configure'>Configure</TabsTrigger>
          <TabsTrigger value='results' disabled={!currentScan?.results}>
            Results
          </TabsTrigger>
          <TabsTrigger value='issues' disabled={!currentScan?.results}>
            Issues
          </TabsTrigger>
          <TabsTrigger value='metrics' disabled={!currentScan?.results}>
            Metrics
          </TabsTrigger>
          <TabsTrigger value='files' disabled={!currentScan?.results}>
            Files
          </TabsTrigger>
          ...
          <Card>
            <CardHeader>
              ...
              <p className='text-sm text-muted-foreground'>
                Configure the paths, file types, and analysis options for your
                scan
              </p>
            </CardHeader>
            ...
            <>
              ...
              <div className='grid grid-cols-1 md:grid-cols-4 gap-4'>
                <Card>
                  ...
                  <CardContent>
                    <div className='text-2xl font-bold'>
                      {currentScan.results.summary.totalIssues}
                    </div>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className='text-2xl font-bold'>
                      {currentScan.results.summary.technicalDebt.hours}h
                    </div>
                    <p className='text-xs text-muted-foreground'>
                      $
                      {currentScan.results.summary.technicalDebt.cost.toLocaleString()}{' '}
                      estimated cost
                    </p>
                  </CardContent>
                </Card>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className='text-2xl font-bold'>
                      {(
                        currentScan.results.summary.maintainability * 100
                      ).toFixed(1)}
                      %
                    </div>
                    <p className='text-xs text-muted-foreground'>
              ...
              <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                <Card>
                  ...
                  <CardContent>
                    <div className='space-y-2'>
                      {Object.entries(
                        currentScan.results.summary.issuesByType
                      ).map(([type, count]) => (
                        <div
                          key={type}
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className='space-y-2'>
                      {Object.entries(
                        currentScan.results.summary.issuesBySeverity
                      ).map(([severity, count]) => (
                        <div
                          key={severity}
                          className='flex items-center justify-between'
                  ...
                  <div className='flex flex-wrap gap-4'>
                    ...
                    <div className='flex items-center space-x-2'>
                      <Checkbox
                        ...
                        checked={showFixable}
                        onCheckedChange={checked =>
                          setShowFixable(checked as boolean)
                        }
                      />
                    ...
                    <div>
                      <div className='flex justify-between'>
                        ...
                        <span className='font-medium'>
                          {currentScan.results.metrics.complexity.cyclomatic}
                        </span>
                      </div>
                    </div>
                    <div>
                      <div className='flex justify-between'>
                        ...
                        <span className='font-medium'>
                          {currentScan.results.metrics.complexity.cognitive}
                        </span>
                      </div>
                    </div>
                    <div>
                      <div className='flex justify-between'>
                        ...
                        <span className='font-medium'>
                          {currentScan.results.metrics.complexity.average.toFixed(
                            2
                          )}
                        </span>
                      </div>
                  ...
                  <div className='space-y-4'>
                    <div>
                      <div className='flex justify-between'>
                        ...
                        <span className='font-medium'>
                          {currentScan.results.metrics.duplication.percentage}%
                        </span>
                      </div>
                    </div>
                    <div>
                      <div className='flex justify-between'>
                        ...
                        <span className='font-medium'>
                          {currentScan.results.metrics.duplication.lines}
                        </span>
                      </div>
                    </div>
                    <div>
                      <div className='flex justify-between'>
                        ...
                        <span className='font-medium'>
                          {currentScan.results.metrics.duplication.blocks}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className='space-y-4'>
                    <div>
                      <div className='flex justify-between'>
                        ...
                        <span className='font-medium'>
                          {currentScan.results.metrics.coverage.lines}%
                        </span>
                      </div>
                    </div>
                    <div>
                      <div className='flex justify-between'>
                        ...
                        <span className='font-medium'>
                          {currentScan.results.metrics.coverage.branches}%
                        </span>
                      </div>
                    </div>
                    <div>
                      <div className='flex justify-between'>
                        ...
                        <span className='font-medium'>
                          {currentScan.results.metrics.coverage.functions}%
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className='space-y-4'>
                    <div>
                      <div className='flex justify-between'>
                        ...
                        <span className='font-medium'>
                          {currentScan.results.metrics.maintainability.index}
                        </span>
                      </div>
                    </div>
                    <div>
                      <div className='flex justify-between'>
                        ...
                        <span className='font-medium'>
                          {currentScan.results.metrics.maintainability.debt}h
                        </span>
                      </div>
                    </div>
                    <div>
                      <div className='flex justify-between'>
                        ...
                        <span className='font-medium'>
                          {(
                            currentScan.results.metrics.maintainability.ratio *
                            100
                          ).toFixed(1)}
                          %
                        </span>
        ...
        <TabsContent value='files' className='space-y-4'>
          {currentScan?.results?.files && (
            <Card>
              <CardHeader>
                <CardTitle>
                  File Analysis ({currentScan.results.files.length} files)
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className='space-y-4'>
