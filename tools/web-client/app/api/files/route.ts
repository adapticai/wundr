...
async function getFileInfo(fullPath: string): Promise<FileInfo> {
  try {
    ...
    throw new Error(
      `Failed to get file info: ${_error instanceof Error ? _error.message : 'Unknown error'}`
    );
  }
}
...
async function listFiles(
  ...
  try {
    ...
    throw new Error(
      `Failed to list files: ${_error instanceof Error ? _error.message : 'Unknown error'}`
    );
  }
}
...
async function readFileContent(
  ...
  try {
    ...
    throw new Error(
      `Failed to read file: ${_error instanceof Error ? _error.message : 'Unknown error'}`
    );
  }
}
...
async function writeFileContent(
  ...
  try {
    ...
    throw new Error(
      `Failed to write file: ${_error instanceof Error ? _error.message : 'Unknown error'}`
    );
  }
}
...
async function deleteFileOrDirectory(
  ...
  try {
    ...
    throw new Error(
      `Failed to delete: ${_error instanceof Error ? _error.message : 'Unknown error'}`
    );
  }
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const encoding =
      (searchParams.get('encoding') as 'utf8' | 'base64') || 'utf8';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    switch (action) {
      ...
      case 'delete':
        data = await deleteFileOrDirectory(
          pathValidation.resolvedPath,
          recursive
        );
        break;
      ...
      case 'copy':
        ...
        if (!newPathValidation.isValid) {
          throw new Error(
            newPathValidation.error || 'Invalid destination path'
          );
        }
        ...
        if (action === 'copy') {
          await fs.copyFile(
            pathValidation.resolvedPath,
            newPathValidation.resolvedPath
          );
        } else {
          await fs.rename(
            pathValidation.resolvedPath,
        ...
        data = {
          success: true,
          from: pathValidation.resolvedPath,
          to: newPathValidation.resolvedPath,
        };
        break;
