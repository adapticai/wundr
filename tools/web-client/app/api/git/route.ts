...
async function getGitStatus(repoPath: string): Promise<GitStatus> {
  try {
    ...
    throw new Error(
      `Failed to get git status: ${_error instanceof Error ? _error.message : 'Unknown error'}`
    );
  }
}
...
async function getGitLog(
  repoPath: string,
  options: GitOperationRequest['options'] = {}
): Promise<GitCommit[]> {
  try {
    const args = [
      'log',
    ...
    throw new Error(
      `Failed to get git log: ${_error instanceof Error ? _error.message : 'Unknown error'}`
    );
  }
}
...
async function getGitBranches(
  ...
  try {
    ...
    throw new Error(
      `Failed to get git branches: ${_error instanceof Error ? _error.message : 'Unknown error'}`
    );
  }
}
...
async function getGitRemotes(repoPath: string): Promise<GitRemote[]> {
  try {
    ...
    throw new Error(
      `Failed to get git remotes: ${_error instanceof Error ? _error.message : 'Unknown error'}`
    );
  }
}
...
async function getGitTags(
  ...
  try {
    ...
    throw new Error(
      `Failed to get git tags: ${_error instanceof Error ? _error.message : 'Unknown error'}`
    );
  }
}
