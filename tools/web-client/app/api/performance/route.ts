  ...
  try {
    ...
    if (packageJson.scripts?.build) {
      ...
      try {
        await execCommand(
          'npm',
          ['run', 'build'],
          path.join(projectRoot, 'tools', 'web-client')
        );
    ...
    try {
      ...
      if (fs.existsSync(targetDir)) {
        ...
        const sourceOutput = await execCommand(
          'du',
          ['-sb', path.join(projectRoot, 'tools', 'web-client', 'app')],
          projectRoot
        );
        bundleSize = Math.floor(
          parseInt(sourceOutput.split('\t')[0], 10) * 0.7
  ...
  try {
    ...
    try {
      ...
      if (fs.existsSync(nextCacheDir)) {
        const cacheFiles = await execCommand(
          'find',
          [nextCacheDir, '-type', 'f'],
          projectRoot
        );
    ...
    loadTime = Math.max(
      200,
      Math.floor((bundleMetrics.bundleSize / (1024 * 1024)) * 300)
    ); // 300ms per MB
  ...
  try {
    ...
    for (let i = dataPoints - 1; i >= 0; i--) {
      ...
      data.push({
        ...
        buildTime: Math.max(
          1000,
          Math.floor(buildMetrics.buildTime * (1 + variation * 0.3))
        ),
        bundleSize: Math.max(
          1024 * 1024,
          Math.floor(buildMetrics.bundleSize * (1 + variation * 0.1))
        ),
        memoryUsage: Math.max(
          100,
          Math.floor(systemMetrics.memoryUsage * (1 + variation * 0.4))
        ),
        cpuUsage: Math.max(
          1,
          Math.min(
            100,
            Math.floor(systemMetrics.cpuUsage * (1 + variation * 0.5))
          )
        ),
        loadTime: Math.max(
          200,
          Math.floor(networkMetrics.loadTime * (1 + variation * 0.3))
        ),
        testDuration: Math.max(
          500,
          Math.floor(buildMetrics.testDuration * (1 + variation * 0.2))
        ),
        cacheHitRate: Math.max(
          0.5,
          Math.min(1, networkMetrics.cacheHitRate * (1 + variation * 0.1))
        ),
        errorRate: Math.max(
          0,
          Math.min(5, networkMetrics.errorRate * (1 + Math.abs(variation) * 2))
        ),
      });
    }
    ...
    performanceCache[cacheKey] = {
      ...
      expires:
        now +
        (timeRange === '1h' ? 60000 : timeRange === '6h' ? 300000 : 600000), // Cache duration based on time range
    };
  ...
  try {
    ...
    const response: ApiResponse<null> = {
      ...
      error: isAnalysisError
        ? 'Performance analysis failed. Please ensure the project build environment is properly configured.'
