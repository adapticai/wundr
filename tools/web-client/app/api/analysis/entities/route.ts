...
async function analyzeEntities(): Promise<EntitiesAnalysisResponse> {
      ...
      issues: [
        {
          ...
          severity: 'warning',
          message:
            'Class has high cyclomatic complexity (18). Consider breaking it down.',
        ...
        },
        {
          ...
          severity: 'info',
          message:
            'Class has many dependencies (4). Consider dependency injection.',
      ...
      issues: [
        {
          ...
          severity: 'critical',
          message:
            'Component has extremely high complexity (24). Urgent refactoring needed.',
        ...
        },
        {
          ...
          severity: 'warning',
          message:
            'Component is very large (387 lines). Consider splitting into smaller components.',
          rule: 'component-size',
        },
      ],
  ...
  const relationships = [
    {
      source: 'user-service-001',
      target: 'database-service-003',
      type: 'dependency' as const,
      strength: 0.8,
    },
    {
      source: 'auth-controller-002',
      target: 'user-service-001',
      type: 'dependency' as const,
      strength: 0.9,
    },
    {
      source: 'user-service-001',
      target: 'user-interface-006',
      type: 'usage' as const,
      strength: 0.7,
    },
    {
      source: 'complex-component-007',
      target: 'user-service-001',
      type: 'dependency' as const,
      strength: 0.6,
    },
    {
      source: 'validation-utils-005',
      target: 'user-interface-006',
      type: 'usage' as const,
      strength: 0.5,
    },
  ...
  const stats: EntityStats = {
    ...
    byComplexity: {
      ...
      medium: entities.filter(e => e.complexity > 5 && e.complexity <= 15)
        .length,
      high: entities.filter(e => e.complexity > 15 && e.complexity <= 25)
        .length,
      critical: entities.filter(e => e.complexity > 25).length,
    },
    bySeverity: {
      info: entities.reduce(
        (sum, e) => sum + e.issues.filter(i => i.severity === 'info').length,
        0
      ),
      warning: entities.reduce(
        (sum, e) => sum + e.issues.filter(i => i.severity === 'warning').length,
        0
      ),
      error: entities.reduce(
        (sum, e) => sum + e.issues.filter(i => i.severity === 'error').length,
        0
      ),
      critical: entities.reduce(
        (sum, e) =>
          sum + e.issues.filter(i => i.severity === 'critical').length,
        0
      ),
    },
    averages: {
      ...
      maintainability:
        entities.reduce((sum, e) => sum + e.metrics.maintainability, 0) /
        entities.length,
      linesOfCode:
        entities.reduce((sum, e) => sum + e.size.lines, 0) / entities.length,
      dependencies:
        entities.reduce((sum, e) => sum + e.dependencies.length, 0) /
        entities.length,
    },
    distributions: {
  ...
  try {
    ...
    if (complexity && complexity !== 'all') {
      filteredEntities = filteredEntities.filter(e => {
        switch (complexity) {
          case 'low':
            return e.complexity <= 5;
          case 'medium':
            return e.complexity > 5 && e.complexity <= 15;
          case 'high':
            return e.complexity > 15 && e.complexity <= 25;
          case 'critical':
            return e.complexity > 25;
          default:
            return true;
        }
      });
    ...
    const response: ApiResponse<EntitiesAnalysisResponse> = {
      ...
      data: {
        ...
        relationships: analysisResult.relationships.filter(
          rel =>
