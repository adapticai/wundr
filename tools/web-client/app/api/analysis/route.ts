...
class AnalysisService {
  ...
  private cache: Map<string, { data: AnalysisData; timestamp: number }> =
    new Map();
  ...
  private async generateIssues(): Promise<AnalysisIssue[]> {
    ...
    for (let i = 0; i < 30; i++) {
      issues.push({
        ...
        tags: ['performance', 'security', 'maintainability'].slice(
          0,
  ...
  private async generateRecommendations(): Promise<AnalysisRecommendation[]> {
    ...
    for (let i = 0; i < 25; i++) {
      ...
      recommendations.push({
        ...
        assignedTo:
          Math.random() > 0.5 ? `Developer ${(i % 5) + 1}` : undefined,
        dueDate: new Date(
  ...
  private async generateDependencyGraph(): Promise<DependencyGraph> {
    ...
    for (let i = 0; i < 30; i++) {
      nodes.push({
        id: `node-${i}`,
        name: `Module${i}`,
        type: ['internal', 'external', 'system'][i % 3] as
          | 'internal'
          | 'external'
          | 'system',
    ...
    for (let i = 0; i < 50; i++) {
      edges.push({
        ...
        type: ['import', 'require', 'dynamic'][i % 3] as
          | 'import'
          | 'require'
          | 'dynamic',
        weight: Math.random(),
