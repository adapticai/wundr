...
async function getGitActivity(
  ...
  try {
    ...
    const contributorsOutput = await execGitCommand(
      ['log', `--since=${sinceStr}`, '--pretty=format:%an'],
      projectRoot
    );
    ...
    for (let i = dataPoints - 1; i >= 0; i--) {
      ...
      const additions = hourCommits.reduce(
        (sum, commit) => sum + commit.additions,
        0
      );
      const deletions = hourCommits.reduce(
        (sum, commit) => sum + commit.deletions,
        0
      );
      const filesSet = new Set(hourCommits.flatMap(commit => commit.files));
      const hourContributors = new Set(
        hourCommits.map(commit => commit.author)
      );
      ...
      data.push({
        ...
        files: filesSet.size,
        contributors:
          hourContributors.size ||
          (contributors.size > 0 ? contributors.size : 1),
...
async function enrichWithPullRequestData(
  data: GitActivity[],
  projectRoot: string,
  ...
  try {
    ...
    const remoteOutput = await execGitCommand(
...
async function fetchGitActivityData(
  ...
  try {
    ...
    gitCache[cacheKey] = {
      ...
      expires:
        now +
        (timeRange === '1h' ? 300000 : timeRange === '6h' ? 600000 : 1800000), // Cache duration based on time range
    };
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (
      repository &&
      (repository.includes('..') || repository.startsWith('/'))
    ) {
      const response: ApiResponse<null> = {
        success: false,
        data: null,
    ...
    const isGitError =
      errorMessage.includes('git') || errorMessage.includes('repository');

    const response: ApiResponse<null> = {
      success: false,
      data: null,
      error: isGitError
        ? 'Git repository analysis failed. Please ensure you are in a valid git repository.'
