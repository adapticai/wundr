...
async function analyzeCodeComplexity(projectRoot: string): Promise<number> {
  try {
    ...
    const tsFiles = await execCommand(
      'find',
      [
        '.',
        '-name',
    ...
    for (const file of files.slice(0, 50)) {
      ...
      try {
        ...
        const complexityMatches =
          content.match(/(if|else|while|for|switch|case|catch|&&|\|\|)/g) || [];
        const functionMatches = content.match(/(function|=>|\bclass\b)/g) || [];

        const fileComplexity =
          complexityMatches.length + functionMatches.length;
    ...
    return fileCount > 0
      ? Math.round((totalComplexity / fileCount) * 10) / 10
      : 0;
  } catch (_error) {
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        const sourceFiles = await execCommand(
          'find',
          [
            '.',
            '-name',
        ...
        const testFiles = await execCommand(
          'find',
          [
            '.',
            '-name',
        ...
        return sourceCount > 0
          ? Math.min(95, Math.round((testCount / sourceCount) * 100))
          : 0;
      }
    }
  } catch (_error) {
...
async function analyzeESLintIssues(
  projectRoot: string
): Promise<{ codeSmells: number; bugs: number }> {
  try {
    // Try to run ESLint on the project
    const eslintOutput = await execCommand(
      'npx',
    ...
    try {
      const todoOutput = await execCommand(
        'grep',
        [
          '-r',
          '--include=*.ts',
...
async function analyzeVulnerabilities(projectRoot: string): Promise<number> {
  try {
    // Try npm audit
    const auditOutput = await execCommand(
      'npm',
      ['audit', '--json'],
      projectRoot
    );
    ...
    try {
      const securityIssues = await execCommand(
        'grep',
        [
          '-r',
          '--include=*.ts',
          '--include=*.tsx',
          '-i',
          'eval\|innerhtml\|document\.write\|password.*=\|api.*key.*=',
          '.',
        ],
        projectRoot
      );
...
async function fetchQualityData(
  ...
  try {
    ...
    for (let i = dataPoints - 1; i >= 0; i--) {
      ...
      data.push({
        ...
        testCoverage: Math.max(
          0,
          Math.min(100, coverage + variation * coverage * 0.2)
        ),
        duplicateLines: Math.max(0, duplicates + variation * duplicates * 0.5),
        maintainabilityIndex: Math.max(
          0,
          Math.min(100, maintainability + variation * maintainability * 0.2)
        ),
        technicalDebt: Math.max(
          0,
          eslintIssues.codeSmells + eslintIssues.bugs + variation * 10
        ),
        codeSmells: Math.max(
          0,
          eslintIssues.codeSmells + Math.floor(variation * 5)
        ),
        bugs: Math.max(0, eslintIssues.bugs + Math.floor(variation * 3)),
        vulnerabilityCount: Math.max(
          0,
          vulnerabilities + Math.floor(variation * 2)
        ),
        linesOfCode: Math.max(
          0,
          linesOfCode + Math.floor(variation * linesOfCode * 0.1)
        ),
      });
    }
    ...
    qualityCache[cacheKey] = {
      ...
      expires:
        now +
        (timeRange === '1h' ? 300000 : timeRange === '6h' ? 900000 : 3600000), // Cache duration based on time range
    };
  ...
  try {
    ...
    const response: ApiResponse<null> = {
      ...
      error: isAnalysisError
        ? 'Project analysis failed. Please ensure the project is properly configured.'
