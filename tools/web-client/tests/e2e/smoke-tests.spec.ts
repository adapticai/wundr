...
test.describe('Smoke Tests - Web Client Dashboard', () => {
  ...
  test('dashboard loads without critical errors', async ({ page }) => {
    ...
    expect(jsErrors.filter(error => error.includes('Cannot read')).length).toBe(
      0
    );
  });
  ...
  test('navigation sidebar is functional', async ({ page }) => {
    ...
    const sidebarExists =
      (await page
        .locator('nav, [role="navigation"], [data-testid*="sidebar"]')
        .count()) > 0;
  ...
  test('analysis section loads data', async ({ page }) => {
    ...
    const hasAnalysisContent = await page
      .locator('text=Total Entities')
      .first()
      .isVisible();
    const hasLoadingState =
      (await page.locator('.loading, .spinner, .skeleton').count()) > 0;
  ...
  test('files section is accessible', async ({ page }) => {
    ...
    const hasFileContent = await Promise.race([
      page
        .locator('.file-list, .file-browser, .file-tree, input[type="file"]')
        .count()
        .then(count => count > 0),
      page
        .locator('text=/file/i')
  ...
  test('performance page displays metrics', async ({ page }) => {
    ...
    const hasPerformanceContent = await Promise.race([
      page
        .locator('.chart, .metric, .performance, canvas, svg')
        .count()
        .then(count => count > 0),
      page
        .locator('text=/performance|metric|benchmark/i')
        .count()
        .then(count => count > 0),
      new Promise(resolve => setTimeout(() => resolve(false), 2000)),
    ]);
    ...
    for (const endpoint of criticalEndpoints) {
      try {
        const response = await page.request.get(
          `${TEST_CONFIG.dashboards.webClient.baseURL}${endpoint}`
        );
        if (response.ok()) {
          workingEndpoints++;
        }
        ...
        console.log(`Endpoint ${endpoint} not available:`, _error);
      }
    }
  ...
  test('responsive layout works on mobile', async ({ page }) => {
    ...
    const hasMobileNav = await Promise.race([
      page
        ...
        .then(count => count > 0),
      page
        .locator('[class*="mobile"], [class*="collapsed"]')
        .count()
        .then(count => count > 0),
  ...
  test('error boundaries handle failures gracefully', async ({ page }) => {
    ...
    const hasUnhandledErrors = await Promise.race([
      page
        .locator('text=/error boundary|unhandled|crashed/i')
        .count()
        .then(count => count > 0),
      new Promise(resolve => setTimeout(() => resolve(false), 2000)),
    ]);
  ...
  test('theme switching works if available', async ({ page }) => {
    ...
    const themeToggle = page
      .locator(
        'button[aria-label*="theme"], button[data-testid*="theme"], .theme-toggle'
      )
      .first();
...
test.describe('Quick Health Check - Both Dashboards', () => {
  test('verify dashboard availability', async ({ page }) => {
    ...
    const webClientAvailable = await testUtils.checkUrlAccessibility(
      'http://localhost:3000'
    );

    // Check Wundr Dashboard (port 3001)
    const wundrAvailable = await testUtils.checkUrlAccessibility(
      'http://localhost:3001'
    );
