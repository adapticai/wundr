...
export class UserService implements DomainService<User, CreateUserRequest, UpdateUserRequest> {
  constructor(
  ...
  async changeUserRole(userId: string, newRole: UserRole, requesterId: string): Promise<User> {
    ...
    if (user.isAdmin && newRole !== UserRole.ADMIN) {
      const adminCount = (await this.userRepository.findUsersByRole(UserRole.ADMIN)).length;
      if (adminCount <= 1) {
  ...
  private async setupDefaultPreferences(user: User): Promise<void> {
    // Implementation would go here
    this.logger.debug('Set up default preferences for user', { userId: user.id });
  }
}
...
export class UserManagementService {
  ...
  async bulkCreateUsers(requests: CreateUserRequest[]): Promise<BulkOperationResult<User>> {
  ...
  async deactivateInactiveUsers(inactiveDays: number = 90): Promise<string[]> {
    ...
    const inactiveUsers = allUsers.filter(user => 
      user.updatedAt < cutoffDate && user.role !== UserRole.ADMIN
    );
  ...
  private async getUsersByCriteria(criteria: UserReportCriteria): Promise<User[]> {
    ...
    if (criteria.isActive !== undefined) {
      queryOptions.filters = { ...queryOptions.filters, isActive: criteria.isActive };
    }
  ...
  private groupUsersByStatus(users: User[]): { active: number; inactive: number } {
    return {
