...
export class PromiseUtils {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (executing.length >= concurrency) {
        ...
        const completedIndex = executing.findIndex(
          p => p === promise || Promise.resolve(p) === promise
        );
        if (completedIndex !== -1) {
   ...
   */
  static async allSettledSeparated<T>(promises: Promise<T>[]): Promise<{
    fulfilled: T[];
    rejected: Error[];
  }> {
...
export class RateLimiter {
  ...
  private getAvailableTokens(): number {
    ...
    const tokensToAdd = Math.floor(
      (timePassed / this.interval) * this.refillRate
    );
...
export class UserService {
  ...
  async searchUsers(
    ...
    const users = await PromiseUtils.withTimeout(
      PromiseUtils.withRetry(() => this.userRepository.search(query, options), {
        maxAttempts: 3,
...
export class OrderProcessingService {
  ...
  private async processOrder(order: Order): Promise<ProcessingResult> {
    ...
    try {
      ...
      this.sendOrderConfirmation(order, paymentResult).catch(error => {
...
interface UserRepository {
  ...
  streamInactiveUsers(
    cutoffDate: Date,
    callback: (users: User[]) => Promise<void>
  ): Promise<void>;
}
