...
export class ApiClient {
  ...
  async getUserOrders(userId: string, pagination?: PaginationRequest): Promise<PaginatedResponse<Order>> {
    const params = pagination ? `?page=${pagination.page}&limit=${pagination.limit}` : '';
    const response = await this.makeRequest<PaginatedResponse<Order>>('GET', `/users/${userId}/orders${params}`);
    return response.data;
  }
...
export class UserService {
  ...
  async createUser(userData: CreateUserRequest): Promise<User> {
    ...
    if (existingUser) {
      throw new BusinessError('User with this email already exists', 'DUPLICATE_EMAIL');
    }
  ...
  private getChangedFields(original: User, updates: UpdateUserRequest): string[] {
  ...
  async createUser(request: CreateUserRequest): Promise<ApiResponse<User>> {
    return this.logger.time('createUser', async () => {
      try {
        ...
        this.logger.error('User creation failed', error, { email: request.email });
