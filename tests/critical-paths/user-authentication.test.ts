...
describe('Critical Path: User login and session management', () => {
  let testContext: any;
  ...
  it('should handle /auth/login endpoint', async () => {
    ...
    const validRequest = getValidRequestFor('/auth/login');
    const response = await makeRequest('/auth/login', validRequest);
    
    expect(response.status).toBe(200);
    expect(response.data).toBeDefined();
    expect(response.data).toMatchObject(getExpectedResponseFor('/auth/login'));
    
    // Test authentication/authorization
    const unauthenticatedResponse = await makeRequest('/auth/login', validRequest, { skipAuth: true });
    expect(unauthenticatedResponse.status).toBe(401);
    
    const unauthorizedResponse = await makeRequest('/auth/login', validRequest, { role: 'guest' });
    expect([401, 403]).toContain(unauthorizedResponse.status);
    
    // Test input validation
    const invalidRequests = getInvalidRequestsFor('/auth/login');
    for (const invalidRequest of invalidRequests) {
      const invalidResponse = await makeRequest('/auth/login', invalidRequest);
    ...
    const errorScenarios = getErrorScenariosFor('/auth/login');
    for (const scenario of errorScenarios) {
      await simulateErrorCondition(scenario.condition);
      const errorResponse = await makeRequest('/auth/login', validRequest);
  ...
  it('should handle /auth/refresh endpoint', async () => {
    // Test valid request/response
    const validRequest = getValidRequestFor('/auth/refresh');
    const response = await makeRequest('/auth/refresh', validRequest);
    
    expect(response.status).toBe(200);
    expect(response.data).toBeDefined();
    expect(response.data).toMatchObject(getExpectedResponseFor('/auth/refresh'));
    
    // Test authentication/authorization
    const unauthenticatedResponse = await makeRequest('/auth/refresh', validRequest, { skipAuth: true });
    expect(unauthenticatedResponse.status).toBe(401);
    
    const unauthorizedResponse = await makeRequest('/auth/refresh', validRequest, { role: 'guest' });
    expect([401, 403]).toContain(unauthorizedResponse.status);
    
    // Test input validation
    const invalidRequests = getInvalidRequestsFor('/auth/refresh');
    for (const invalidRequest of invalidRequests) {
      const invalidResponse = await makeRequest('/auth/refresh', invalidRequest);
      expect(invalidResponse.status).toBeGreaterThanOrEqual(400);
      expect(invalidResponse.status).toBeLessThan(500);
      expect(invalidResponse.data.error).toBeDefined();
    }
    ...
    // Test error responses
    const errorScenarios = getErrorScenariosFor('/auth/refresh');
    for (const scenario of errorScenarios) {
      await simulateErrorCondition(scenario.condition);
      const errorResponse = await makeRequest('/auth/refresh', validRequest);
      expect(errorResponse.status).toBe(scenario.expectedStatus);
      expect(errorResponse.data.error).toContain(scenario.expectedMessage);
      await clearErrorCondition(scenario.condition);
    }
  ...
  it('should handle /auth/logout endpoint', async () => {
    // Test valid request/response
    const validRequest = getValidRequestFor('/auth/logout');
    const response = await makeRequest('/auth/logout', validRequest);
    
    expect(response.status).toBe(200);
    expect(response.data).toBeDefined();
    expect(response.data).toMatchObject(getExpectedResponseFor('/auth/logout'));
    
    // Test authentication/authorization
    const unauthenticatedResponse = await makeRequest('/auth/logout', validRequest, { skipAuth: true });
    expect(unauthenticatedResponse.status).toBe(401);
    
    const unauthorizedResponse = await makeRequest('/auth/logout', validRequest, { role: 'guest' });
    expect([401, 403]).toContain(unauthorizedResponse.status);
    
    // Test input validation
    const invalidRequests = getInvalidRequestsFor('/auth/logout');
    for (const invalidRequest of invalidRequests) {
      const invalidResponse = await makeRequest('/auth/logout', invalidRequest);
      expect(invalidResponse.status).toBeGreaterThanOrEqual(400);
      expect(invalidResponse.status).toBeLessThan(500);
      expect(invalidResponse.data.error).toBeDefined();
    }
    ...
    // Test error responses
    const errorScenarios = getErrorScenariosFor('/auth/logout');
    for (const scenario of errorScenarios) {
      await simulateErrorCondition(scenario.condition);
      const errorResponse = await makeRequest('/auth/logout', validRequest);
      expect(errorResponse.status).toBe(scenario.expectedStatus);
      expect(errorResponse.data.error).toContain(scenario.expectedMessage);
      await clearErrorCondition(scenario.condition);
    }
  ...
  it('should test AuthService functionality', async () => {
    const service = getServiceInstance('AuthService');
    
    // Test happy path scenarios
    const happyPathTests = getHappyPathTestsFor('AuthService');
    ...
    // Test edge cases
    const edgeCases = getEdgeCasesFor('AuthService');
    ...
    // Test error handling
    const errorTests = getErrorTestsFor('AuthService');
  ...
  it('should test TokenService functionality', async () => {
    const service = getServiceInstance('TokenService');
    
    // Test happy path scenarios
    const happyPathTests = getHappyPathTestsFor('TokenService');
    for (const test of happyPathTests) {
      const result = await service[test.method](...test.args);
      expect(result).toBeDefined();
      expect(result).toMatchObject(test.expectedResult);
    ...
    // Test edge cases
    const edgeCases = getEdgeCasesFor('TokenService');
    for (const edgeCase of edgeCases) {
      const result = await service[edgeCase.method](...edgeCase.args);
      expect(result).toEqual(edgeCase.expectedResult);
    }
    ...
    // Test error handling
    const errorTests = getErrorTestsFor('TokenService');
    for (const errorTest of errorTests) {
      if (errorTest.shouldThrow) {
        await expect(service[errorTest.method](...errorTest.args))
          .rejects.toThrow(errorTest.expectedError);
  ...
  it('should complete the full user-authentication workflow', async () => {
    // 1. Setup initial state
    const initialData = await setupWorkflowData('user-authentication');
    const workflowId = generateWorkflowId();
    
    // 2. Execute workflow steps sequentially
    const steps = getWorkflowSteps('user-authentication');
    const results = [];
    ...
    const sideEffects = await checkWorkflowSideEffects(workflowId, 'user-authentication');
  ...
  it('should handle errors gracefully in user-authentication', async () => {
    const workflowId = generateWorkflowId();
    
    // Test invalid inputs
    const invalidInputs = getInvalidInputsFor('user-authentication');
    ...
    const servicesToFail = ["AuthService","TokenService"];
    for (const service of servicesToFail) {
      ...
      const result = await executeWorkflowStep({
        ...
        stepName: 'test-step',
        input: getValidInputFor('user-authentication'),
        context: testContext
      });
    ...
    const slowSteps = getSlowStepsFor('user-authentication');
    ...
    const expectedServices = ["AuthService","TokenService"].map(service => expect.objectContaining({ name: service, status: 'up' }));
