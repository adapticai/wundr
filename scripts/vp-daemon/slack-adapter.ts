/**
 * SlackAdapter - Bolt.js Slack Integration for VP Daemon
 *
 * Implements Socket Mode Slack integration for the VP (Virtual Principal) daemon.
 * Handles app mentions, direct messages, and slash commands with VP persona.
 *
...
export interface SlackConfig {
  ...
  /** Optional: Default channel for VP messages */
  defaultChannel?: string;
  /** Optional: VP persona name for responses */
  vpName?: string;
...
export class SlackAdapter {
  ...
  private setupEventHandlers(): void {
    ...
    // Slash command: /vp-status
    this.app.command('/vp-status', async ({ command, ack, respond }) => {
      await ack();
      await this.handleSlashCommand(command, respond, 'status');
    });

    // Slash command: /vp-queue
    this.app.command('/vp-queue', async ({ command, ack, respond }) => {
      await ack();
  ...
  private async handleMention(
    ...
    try {
      ...
      if (!cleanText) {
        ...
        await say({
          text: VP_RESPONSES.greeting(this.vpName),
          thread_ts: threadTs,
        });
        return;
      }
  ...
  /**
   * Handle slash commands (/vp-status, /vp-queue)
   */
  ...
  /**
   * Build status blocks for /vp-status command
   */
  ...
  /**
   * Format response in VP persona/tone
   */
  private formatVPResponse(text: string): string {
    // Add VP signature style if not already present
