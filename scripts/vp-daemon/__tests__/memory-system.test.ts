...
describe('VPMemorySystem', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'vp-memory-test-'));
  ...
  describe('Context Compilation', () => {
    it('should compile context with active tasks', async () => {
      ...
      const context = await memorySystem.compileContext({
        systemPrompt: 'You are a helpful VP assistant',
        maxTokens: 8000,
        includeActiveTasks: true,
      });

      expect(context).toBeDefined();
      expect(context.systemPrompt).toBe('You are a helpful VP assistant');
