...
The VP-Daemon is a sophisticated orchestration system for managing Virtual Principals (VPs) - automated AI agents that execute tasks with defined constraints, identity, and resource limits. The daemon provides comprehensive session management, triage, resource allocation, and observability for distributed VP operations.
...
Manages VP identities and credentials.
...
Monitors VP behavior and triggers interventions when needed.
...
```bash
# Run all tests
npm test tests/vp-daemon/

# Run specific test suites
npm test tests/vp-daemon/session-spawner.test.ts
npm test tests/vp-daemon/session-pool.test.ts
npm test tests/vp-daemon/session-recovery.test.ts

# Run with coverage
npm test -- --coverage tests/vp-daemon/
```

## Examples

See `/examples/vp-daemon/session-spawner-example.ts` for comprehensive usage examples:
...
Run examples:
```bash
npx ts-node examples/vp-daemon/session-spawner-example.ts
```
...
- Documentation: `/docs/vp-daemon/`
- Issues: GitHub Issues
- Slack: #vp-daemon channel
