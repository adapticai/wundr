...
export class AutoFixPatterns {
  ...
  async previewFixes(): Promise<void> {
    ...
    if (preview.length === 0) {
      ...
      console.log(`\n=� Total: ${preview.reduce((sum, p) => sum + p.fixes, 0)} potential fixes across ${preview.length} file-rule combinations`);
    }
  }
...
`;
    ...
    console.log('\n=� Report saved to auto-fix-report.md');
  }
   ...
   */
  listRules(): void {
    console.log('=� Available Auto-Fix Rules:\n');
