...
```bash
# Run example usage
npx tsx scripts/orchestrator-daemon/examples/memory-usage.ts
...
Edit `scripts/orchestrator-daemon/index.ts`:
...
export class VPDaemon extends EventEmitter {
  ...
  async initializeSubsystems(): Promise<void> {
    ...
    this.memoryAPI = await createMemoryAPI({
      basePath: process.env['VP_MEMORY_PATH'] || './.orchestrator-daemon/memory',
      persistToDisk: true,
...
Edit `scripts/orchestrator-daemon/session-manager.ts`:
...
Edit `scripts/orchestrator-daemon/triage-engine.ts`:
...
Edit `scripts/orchestrator-daemon/telemetry-collector.ts`:
...
In `scripts/orchestrator-daemon/index.ts`, after initialization:
...
```bash
cd /Users/iroselli/wundr
npx tsx scripts/orchestrator-daemon/index.ts start --verbose
```
...
```bash
ls -la ~/.orchestrator-daemon/memory/
```
...
# Restart daemon
npx tsx scripts/orchestrator-daemon/index.ts start

# Check if task still exists
# Should load from ~/.orchestrator-daemon/memory/state.json
```
...
In `scripts/orchestrator-daemon/index.ts`:
...
```bash
mkdir -p ~/.orchestrator-daemon/memory
chmod 755 ~/.orchestrator-daemon/memory
```
