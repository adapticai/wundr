...
/**
 * Orchestrator Daemon - Main Entry Point
 *
...
/**
 * Orchestrator Daemon - Main orchestrator class
 *
...
export class VPDaemon extends EventEmitter {
  ...
  constructor(config: VPDaemonConfig = {}) {
    ...
    this.config = {
      ...
      name: config.name ?? 'orchestrator-daemon',
  ...
  /**
   * Start the Orchestrator Daemon and all subsystems
   */
  async start(): Promise<void> {
    ...
    this.log('info', 'Starting Orchestrator Daemon...');
  ...
  /**
   * Get the Orchestrator Identity
   */
  getIdentity(): VPIdentity | null {
    return this.orchestratorIdentity;
  }
  ...
  private async initializeSubsystems(): Promise<void> {
    ...
    this.orchestratorIdentity = await this.identityManager.loadIdentity();
    this.log('info', `VP Identity loaded: ${this.orchestratorIdentity.name}`);
    ...
    this.triageEngine = new TriageEngine({
      memoryBankPath:
        process.env['VP_MEMORY_BANK_PATH'] ?? './.orchestrator-daemon/memory-bank',
      enableRAG: false,
  ...
  private generateId(): string {
    return `orchestrator-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
  }
...
/**
 * Factory function to create a Orchestrator Daemon instance
 */
...
async function main(): Promise<void> {
  ...
  program
    ...
    .option('-n, --name <string>', 'Daemon instance name', 'orchestrator-daemon')
...
async function runDaemon(options: CLIOptions): Promise<void> {
  ...
  console.log(chalk.cyan('           Orchestrator Daemon - Virtual Principal'));
  console.log(chalk.cyan('================================================'));
  console.log();
...
export type { IdentityManagerConfig, VPIdentity } from './identity-manager.js';
