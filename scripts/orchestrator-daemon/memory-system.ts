...
const DEFAULT_VP_MEMORY_CONFIG: VPMemoryConfig = {
  basePath: path.join(process.env['HOME'] || '~', '.orchestrator-daemon', 'memory'),
  persistToDisk: true,
  ...
  tiers: {
    ...
    forgettingCurve: {
      initialStrength: 1.0,
      decayRate: 0.05, // Slower decay for Orchestrator memories
...
/**
 * Orchestrator Memory System - Manages tiered memory for Virtual Principal
 *
 * Provides:
...
/**
 * Factory function to create and initialize a Orchestrator Memory System
 */
