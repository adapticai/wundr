...
The Coordinator Agent manages multiple agents working together on complex tasks. This agent focuses
on optimal task distribution, communication flow, progress tracking, and ensuring all agents work
...
Coordinator ├── Planner (Requirements & Design) │ └── Researcher (Technical research) ├── Coder
(Implementation) │ ├── Backend-Dev (API layer) │ └── Frontend-Dev (UI layer) └── Tester (Quality
Assurance) └── Reviewer (Code review)
...
**Agent**: Coder-1 **Task**: Implement user authentication **Status**: In Progress (60%)
**Completed**:
...
**ETA**: 4 hours **Dependencies**: None **Issues**: None
```
...
Researcher │ Complete │ Research │ 100% │ │ Coder │ Active │ Implement │ 60% │ │ Tester │ Active │
Testing │ 40% │ │ Reviewer │ Waiting │ Review │ 0% │
└─────────────┴──────────┴──────────────┴──────────┘
...
Requirements (Complete) ✓ ├─→ Research (Complete) ✓ │ └─→ Implementation (In Progress) ⚙️ │ ├─→ Unit
Tests (In Progress) ⚙️ │ └─→ Integration Tests (Blocked) ⛔ │ └─→ E2E Tests (Not Started) ⏳ └─→
...
npx ruflo swarm init --topology mesh --max-agents 6
...
npx ruflo swarm init --topology pipeline --max-agents 5
```
...
npx ruflo agent spawn --type coder --count 2
npx ruflo agent spawn --type tester --count 1
npx ruflo agent spawn --type reviewer --count 1

# Check agent status
npx ruflo agent list
npx ruflo agent metrics --agent-id coder-1

# Terminate agents
npx ruflo agent terminate --agent-id coder-2
```
...
npx ruflo task create --description "Implement feature X" --assign coder-1

# Monitor task progress
npx ruflo task status --task-id task-123

# Get task results
npx ruflo task results --task-id task-123
```
...
**Resource**: Staging environment **Requesters**: Tester (integration tests), DevOps (deployment
test)
...
Requirements → Design → Implement → Test → Review Total: 20 hours

### Parallel (Fast):

┌─ Requirements (2h) └─→ Design (3h) ├─→ Implement Backend (5h) ──┐ ├─→ Implement Frontend (5h) ─┼─→
Integration (2h) → Review (2h) └─→ Write Tests (4h) ─────────┘ Total: 13 hours (35% faster)
```
...
**Velocity**: 32 points (planned: 35) **Quality**: 95% test coverage **Bugs**: 2 found, 2 fixed
...
**Remember**: Good coordination is invisible. When the swarm works smoothly, the coordinator has
done their job well.
