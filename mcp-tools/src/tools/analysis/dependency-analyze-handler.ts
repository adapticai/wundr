...
export class DependencyAnalyzeHandler {
  ...
  constructor() {
    this.scriptPath = path.resolve(process.cwd(), 'scripts/analysis/dependency-mapper.ts');
  }
  ...
  private analyzeAllDependencies(target?: string, outputFormat?: string): string {
  ...
  private findCircularDependencies(target?: string): string {
    const checkDepsScript = path.resolve(process.cwd(), 'scripts/monorepo/check-dependencies.ts');
  ...
  private findUnusedDependencies(target?: string): string {
    ...
    const possiblyUnused = unusedDeps.filter(dep => 
    ...
    return JSON.stringify({
      ...
      recommendations: possiblyUnused.length > 0 ? [
  ...
  private categorizeDependencies(deps: Record<string, string>): Record<string, string[]> {
...
`;
  ...
  private generateInsights(analysis: any): string[] {
    ...
    if (analysis.circularDependencies.length > 0) {
      insights.push(`⚠️ ${analysis.circularDependencies.length} circular dependencies need resolution`);
    }

    if (analysis.unusedDependencies.length > 5) {
      insights.push(`🗑️ ${analysis.unusedDependencies.length} unused dependencies can be removed`);
    }

    const depRatio = analysis.externalDependencies / analysis.totalFiles;
    if (depRatio > 2) {
      insights.push('📦 High external dependency ratio - consider consolidation');
    }
  ...
  private generateExternalDepsRecommendations(analysis: any): string[] {
    ...
    if (analysis.security.issues > 0) {
      recommendations.push(`Fix ${analysis.security.issues} security vulnerabilities immediately`);
    }

    if (analysis.outdated.major > 0) {
      recommendations.push(`Update ${analysis.outdated.major} dependencies with major version changes`);
    }
    ...
    if (analysis.production.count > 50) {
      recommendations.push('Consider consolidating dependencies to reduce bundle size');
    }
