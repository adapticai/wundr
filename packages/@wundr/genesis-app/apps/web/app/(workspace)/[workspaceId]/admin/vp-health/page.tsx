...
const filterOptions: { value: FilterStatus; label: string }[] = [
  { value: 'all', label: 'All VPs' },
  { value: 'online', label: 'Online' },
  { value: 'offline', label: 'Offline' },
  { value: 'unhealthy', label: 'Unhealthy' },
];

export default function VPHealthDashboardPage() {
  ...
  const { vpList, isLoading, refetch } = useVPHealthList(workspaceId);

  // Filter VPs based on selected filter
  const filteredVPs = useMemo(() => {
    switch (filter) {
      ...
      case 'unhealthy':
        return vpList.filter(
          (vp) => vp.daemonHealth === 'unhealthy' || vp.daemonHealth === 'degraded',
        );
      default:
        return vpList;
    }
  ...
  // Count unhealthy VPs for alert banner
  ...
  // Handle VP details view
  const handleViewDetails = useCallback(
    (vpId: string) => {
      window.location.href = `/${workspaceId}/vps/${vpId}`;
    },
    [workspaceId],
  );
  ...
  const handleRestartDaemon = useCallback(
    async (vpId: string) => {
      try {
        await fetch(`/api/vps/${vpId}/daemon/restart`, {
          method: 'POST',
        });
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-foreground">VP Health Dashboard</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Monitor the health and status of all Virtual Persons in your workspace
        </p>
      </div>

      {/* Alert Banner for Unhealthy VPs */}
      {unhealthyCount > 0 && (
        <div
          className={cn(
        ...
        >
          ...
          <div className="flex-1">
            <p className="font-medium text-red-800 dark:text-red-200">
              {unhealthyCount} VP{unhealthyCount > 1 ? 's' : ''} require
              {unhealthyCount === 1 ? 's' : ''} attention
            </p>
            <p className="text-sm text-red-700 dark:text-red-300">
              Some VPs are experiencing issues. Check their status and consider
      ...
      <div className="mb-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
        ...
        <StatCard
          label="Degraded"
          value={vpList.filter((vp) => vp.daemonHealth === 'degraded').length}
          icon={AlertTriangleIcon}
          valueColor="text-yellow-600 dark:text-yellow-400"
        />
        <StatCard
          label="Unhealthy"
          value={vpList.filter((vp) => vp.daemonHealth === 'unhealthy').length}
          icon={XCircleIcon}
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-lg border border-dashed py-12">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            {filter === 'all'
              ? 'No VPs found in this workspace'
              : `No ${filter} VPs found`}
          </p>
          {filter !== 'all' && (
            <button
