...
async function handleRoomStarted(payload: LiveKitWebhookPayload): Promise<void> {
  ...
  if (type === 'call') {
    ...
    console.log(`[LiveKit Webhook] Room started: ${roomName}`);
  }
}
...
async function handleParticipantLeft(payload: LiveKitWebhookPayload): Promise<void> {
  ...
  console.log(`[LiveKit Webhook] Participant left: ${participant.identity} from ${roomName}`);
}
...
async function handleTrackPublished(payload: LiveKitWebhookPayload): Promise<void> {
  ...
  if (!roomName || !participant?.identity || !track) return;
  ...
  console.log(`[LiveKit Webhook] Track published: ${track.type} by ${participant.identity} in ${roomName}`);
}
...
async function handleTrackUnpublished(payload: LiveKitWebhookPayload): Promise<void> {
  ...
  console.log(`[LiveKit Webhook] Track unpublished: ${track.type} by ${participant.identity} in ${roomName}`);
}
...
async function handleEgressStarted(payload: LiveKitWebhookPayload): Promise<void> {
  ...
  if (!egressInfo?.egressId || !egressInfo.roomName) return;
  ...
  console.log(`[LiveKit Webhook] Egress started: ${egressInfo.egressId} for ${egressInfo.roomName}`);
}
...
async function handleEgressEnded(payload: LiveKitWebhookPayload): Promise<void> {
  ...
  console.log(`[LiveKit Webhook] Egress ended: ${egressInfo.egressId} with status ${status}`);
}
