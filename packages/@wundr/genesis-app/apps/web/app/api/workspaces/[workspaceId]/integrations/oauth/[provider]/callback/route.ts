...
export async function GET(
  ...
  try {
    ...
    const { workspaceId, provider: providerParam } = params;
    ...
    console.error('[GET /api/workspaces/:workspaceId/integrations/oauth/:provider/callback] Error:', error);

    // Redirect with error
    const params = await context.params;
    const baseUrl = process.env.NEXTAUTH_URL ?? request.nextUrl.origin;
    return NextResponse.redirect(
      `${baseUrl}/workspace/${params.workspaceId}/settings/integrations?error=callback_failed`
    );
  }
}
