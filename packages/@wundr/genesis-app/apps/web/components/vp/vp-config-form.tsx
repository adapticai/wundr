...
export function VPConfigForm({
  ...
  const handleSave = useCallback(async () => {
    const charter: Partial<VPCharter> = {
      ...vp.charter,
      personality: {
        ...vp.charter?.personality,
  ...
  return (
    <div className={cn('space-y-6', className)}>
      {/* System Prompt */}
      <section>
        <label htmlFor="systemPrompt" className="mb-2 block text-sm font-medium text-foreground">
          System Prompt
        </label>
        <textarea
          id="systemPrompt"
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Supports markdown formatting. This prompt defines how the VP responds and behaves.
        </p>
      </section>

      {/* Personality Traits */}
      <section>
        <h3 className="mb-3 text-sm font-medium text-foreground">Personality Traits</h3>
      ...
      {/* Expertise Tags */}
      <section>
        <h3 className="mb-3 text-sm font-medium text-foreground">Expertise Areas</h3>
      ...
      <section>
        <h3 className="mb-3 text-sm font-medium text-foreground">Communication Preferences</h3>
        <div className="grid gap-4 sm:grid-cols-2">
          {/* Tone */}
          <div>
            <label htmlFor="tone" className="mb-1 block text-xs font-medium text-muted-foreground">
              Tone
            </label>
            <select
              id="tone"
              ...
              onChange={(e) => {
                setCommunicationTone(e.target.value as 'formal' | 'casual' | 'professional' | 'friendly');
                setIsDirty(true);
              }}
          ...
          {/* Response Length */}
          <div>
            <label htmlFor="responseLength" className="mb-1 block text-xs font-medium text-muted-foreground">
              Response Length
            </label>
            <select
              id="responseLength"
              ...
              onChange={(e) => {
                setResponseLength(e.target.value as 'concise' | 'detailed' | 'balanced');
                setIsDirty(true);
              }}
              className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary"
              disabled={isLoading}
        ...
        <label className="mt-4 flex cursor-pointer items-center gap-3">
          ...
          />
          <span className="text-sm text-foreground">Allow emoji in responses</span>
        </label>
      </section>
