...
generated by `@wundr/org-genesis` and the Slack workspace where VP agents and discipline channels
will be created.
...
console.log(
  `VPs created: ${migrationResult.vpMappings.successful}/${migrationResult.vpMappings.total}`
);
console.log(
...
result.vpMappings.mappings.forEach(mapping => {
...
vpMapper.addMapping('vp-engineering', 'U123ABC456');
disciplineMapper.addMapping('disc-backend', 'C789XYZ012');

// Look up IDs in either direction
const slackUserId = vpMapper.getTargetId('vp-engineering'); // 'U123ABC456'
const genesisVpId = vpMapper.getSourceId('U123ABC456'); // 'vp-engineering'

// Export for persistence
const mappingData = vpMapper.toJSON();
// { 'vp-engineering': 'U123ABC456' }
...
Create Slack users from VP definitions.

**Parameters:**

- `vps: VPDefinition[]` - Array of VP definitions
- `organizationId: string` - Organization ID
- `options: MigrationOptions` - Migration configuration

**Returns:** `Promise<VPMappingResult>`
...
4. VP agents begin operating in their assigned channels
