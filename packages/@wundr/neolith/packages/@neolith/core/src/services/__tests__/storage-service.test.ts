...
describe('StorageService', () => {
  ...
  describe('deleteFile', () => {
    ...
    it('propagates S3 errors', async () => {
      ...
      mockS3Client.deleteObject.mockRejectedValue(new Error('Access Denied'));
  ...
  describe('getMultipartPartUrl', () => {
    it('generates part upload URL', async () => {
      ...
      const url = await storageService.getMultipartPartUrl(
        key,
        uploadId,
        partNumber
      );
...
describe('StorageService Integration Scenarios', () => {
  ...
  it('complete upload flow: presigned URL -> upload -> verify', async () => {
    ...
    const uploadResult = await storageService.uploadFile(key, fileContent, {
      contentType,
    });
  ...
  it('multipart upload flow for large files', async () => {
    ...
    const initResult = await storageService.initiateMultipartUpload(
      key,
      contentType
    );
    ...
    const partUrl1 = await storageService.getMultipartPartUrl(
      key,
      initResult.uploadId,
      1
    );
    const partUrl2 = await storageService.getMultipartPartUrl(
      key,
      initResult.uploadId,
      2
    );
