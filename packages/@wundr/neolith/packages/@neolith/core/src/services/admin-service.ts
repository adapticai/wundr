...
function generateId(prefix: string): string {
  return (
    prefix + '_' + Date.now().toString(36) + crypto.randomUUID().split('-')[0]
  );
}
...
export class AdminService {
  ...
  async getSettings(workspaceId: string): Promise<WorkspaceSettings> {
    ...
    if (!s) {
      s = this.createDefaultSettings(workspaceId);
  ...
  async updateMember(
    ...
    await this.storage.saveMember(m);
    return m;
  }
  ...
  async getBillingInfo(workspaceId: string): Promise<BillingInfo> {
    ...
    if (!b) {
      b = this.createDefaultBilling(workspaceId);
      await this.storage.saveBilling(b);
    }
    return b;
  }
  ...
  async logAdminAction(
    ...
    await this.storage.saveAdminAction(a);
    return a;
  }
