...
export const VideoPreview = React.forwardRef<HTMLDivElement, VideoPreviewProps>(
  ({ filename, fileUrl, fileSize, thumbnailUrl, onDownload, className, maxHeight = 400 }, ref) => {
        ...
        <CardContent className="p-0">
          <div className="relative bg-black" style={{ maxHeight }}>
            ...
            {hasError && (
              ...
              >
                <VideoIcon className="h-16 w-16 text-muted-foreground" />
                <p className="mt-4 text-sm text-muted-foreground">Unable to load video</p>
              </div>
            )}
            ...
            <video
              ...
              src={fileUrl}
              controls
              className={cn('w-full', isPlaying && !hasError ? 'block' : 'hidden')}
              style={{ maxHeight }}
