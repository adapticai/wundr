...
export function UsageChart({ data, className, showComparison, previousPeriodData }: UsageChartProps) {
  ...
  const stats = React.useMemo(() => {
    ...
    const outputTotal = data.reduce((sum, point) => sum + point.outputTokens, 0);
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Badge variant="outline">
              Total: {formatTokens(stats.total)}
            </Badge>
            <Badge variant="outline">
              Avg: {formatTokens(stats.avg)}
            </Badge>
            <Badge variant="outline">
              Peak: {formatTokens(stats.max)}
            </Badge>
          </div>
        </div>
      </CardHeader>
            ...
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                  ...
                  content={
                    <ChartTooltipContent
                      labelFormatter={(value) => {
                        const point = chartData.find((d) => d.timestampFormatted === value);
                        return point
                ...
                <Line
                  ...
                  strokeWidth={2}
                  dot={false}
                  opacity={hoveredSeries && hoveredSeries !== 'inputTokens' ? 0.3 : 1}
                />
                <Line
                  type="monotone"
                  ...
                  strokeWidth={2}
                  dot={false}
                  opacity={hoveredSeries && hoveredSeries !== 'outputTokens' ? 0.3 : 1}
                />
                <Line
                  type="monotone"
                  dataKey="totalTokens"
                  ...
                  strokeWidth={2}
                  dot={false}
                  opacity={hoveredSeries && hoveredSeries !== 'totalTokens' ? 0.3 : 1}
                />
              </LineChart>
            </ResponsiveContainer>
          </ChartContainer>
