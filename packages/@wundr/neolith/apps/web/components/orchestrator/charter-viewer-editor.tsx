...
export function CharterViewerEditor({
  orchestratorId,
  readOnly = false,
  className,
  onSaved,
}: CharterViewerEditorProps) {
    ...
    async function load() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          const p =
            typeof data.charterData === 'string'
              ? JSON.parse(data.charterData)
              : data.charterData;
          setCharter({
            identity: { ...DEFAULT_CHARTER.identity, ...p?.identity },
            capabilities: {
              ...DEFAULT_CHARTER.capabilities,
              ...p?.capabilities,
            },
            ...
            boundaries: {
              ...
              ...p?.boundaries,
              budgetLimits: {
                ...DEFAULT_CHARTER.boundaries.budgetLimits,
                ...p?.boundaries?.budgetLimits,
              },
            },
          });
        }
      } catch {
        toast({ title: 'Failed to load charter', variant: 'destructive' });
      } finally {
        setLoading(false);
      }
    }
  ...
  const patch = <K extends keyof CharterData>(
    section: K,
    update: Partial<CharterData[K]>
  ) => setCharter(c => ({ ...c, [section]: { ...c[section], ...update } }));

  const patchBudget = (
    update: Partial<CharterData['boundaries']['budgetLimits']>
  ) =>
    setCharter(c => ({
      ...c,
      boundaries: {
        ...c.boundaries,
        budgetLimits: { ...c.boundaries.budgetLimits, ...update },
      },
    }));
  ...
  const toggleAction = (action: string) => {
    ...
    patch('boundaries', {
      allowedActions: cur.includes(action)
      ...
      <CardContent className='space-y-4'>
        <Tabs defaultValue='identity'>
          ...
          <TabsContent value='identity' className='space-y-4 pt-4'>
            {(['name', 'role', 'discipline'] as const).map(f => (
              <div key={f} className='space-y-1'>
                <Label htmlFor={`id-${f}`} className='capitalize'>
                  {f}
                </Label>
                <Input
                  id={`id-${f}`}
                  value={charter.identity[f]}
                  readOnly={readOnly}
            ...
            <div className='space-y-1'>
              ...
              <Textarea
                id='id-mission'
                rows={4}
                value={charter.identity.mission}
                readOnly={readOnly}
          ...
          <TabsContent value='capabilities' className='space-y-4 pt-4'>
            {(
              ...
              <div key={field} className='space-y-1'>
                ...
                <TagInput
                  tags={charter.capabilities[field]}
                  readOnly={readOnly}
                  placeholder={ph}
          ...
          <TabsContent value='guidelines' className='space-y-4 pt-4'>
            <div className='space-y-1'>
              ...
              {readOnly ? (
                <p className='text-sm capitalize'>
                  {charter.guidelines.communicationStyle || '—'}
                </p>
              ) : (
                <Select
                  value={charter.guidelines.communicationStyle}
                  onValueChange={v =>
                    patch('guidelines', { communicationStyle: v })
                  }
                >
            ...
            <div className='space-y-1'>
              ...
              <Textarea
                id='decision'
                rows={3}
                value={charter.guidelines.decisionMakingApproach}
                readOnly={readOnly}
                placeholder={
                  readOnly ? undefined : 'Describe how decisions are made...'
                }
                onChange={e =>
                  patch('guidelines', {
                    decisionMakingApproach: e.target.value,
                  })
                }
              />
            </div>
            <div className='space-y-1'>
              ...
              <Textarea
                id='collab'
                rows={3}
                value={charter.guidelines.collaborationPreferences}
                readOnly={readOnly}
                placeholder={
                  readOnly ? undefined : 'Describe collaboration approach...'
                }
                onChange={e =>
                  patch('guidelines', {
                    collaborationPreferences: e.target.value,
                  })
                }
              />
            </div>
          ...
          <TabsContent value='boundaries' className='space-y-4 pt-4'>
                ...
                {(
                  ...
                  <div key={k} className='space-y-1'>
                    ...
                    <Input
                      ...
                      type='number'
                      min={0}
                      value={charter.boundaries.budgetLimits[k]}
                      readOnly={readOnly}
                      onChange={e =>
                        patchBudget({ [k]: parseFloat(e.target.value) || 0 })
                      }
                    />
                  </div>
                ))}
              ...
              <div className='space-y-2'>
                {ALLOWED_ACTIONS.map(action => (
                  <div key={action} className='flex items-center gap-2'>
                    <Checkbox
                      id={`act-${action}`}
                      checked={charter.boundaries.allowedActions.includes(
                        action
                      )}
                      disabled={readOnly}
            ...
            <div className='space-y-1'>
              ...
              <TagInput
                tags={charter.boundaries.escalationTriggers}
                readOnly={readOnly}
                placeholder='e.g., budget exceeded, critical error'
                onChange={tags =>
                  patch('boundaries', { escalationTriggers: tags })
                }
              />
            </div>
            <div className='space-y-1'>
              ...
              <TagInput
                tags={charter.boundaries.restrictedOperations}
                readOnly={readOnly}
                placeholder='e.g., delete_production, external_access'
                onChange={tags =>
                  patch('boundaries', { restrictedOperations: tags })
                }
              />
            </div>
          </TabsContent>
            ...
            {versions.length === 0 ? (
              ...
              <ul className='space-y-3'>
                {versions.map(v => (
                  <li
                    key={v.id}
                    className='rounded-md border p-3 text-sm space-y-0.5'
                  >
                    ...
                    {v.changeLog && (
                      <p className='text-muted-foreground'>{v.changeLog}</p>
                    )}
                    {v.creator && (
                      <p className='text-xs text-muted-foreground'>
                        by{' '}
                        {v.creator.displayName ?? v.creator.name ?? 'Unknown'}
                      </p>
                    )}
                  </li>
                ))}
        ...
        {!readOnly && (
          <div className='flex items-center gap-3 border-t pt-4'>
            <Input
              placeholder='Change log message (optional)'
              value={changeLog}
