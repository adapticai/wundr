 ...
 */
export function CanvasTab({ channelId: _channelId, className }: CanvasTabProps) {
  ...
  const canvasTypes = [
    { type: 'note' as const, icon: FileText, label: 'Note', description: 'Free-form text and formatting' },
    { type: 'document' as const, icon: Sparkles, label: 'AI Document', description: 'AI-assisted document creation' },
    { type: 'checklist' as const, icon: List, label: 'Checklist', description: 'Track tasks and todos' },
    { type: 'table' as const, icon: TableIcon, label: 'Table', description: 'Structured data in rows and columns' },
    { type: 'timeline' as const, icon: Calendar, label: 'Timeline', description: 'Plan and visualize schedules' },
  ];
  ...
  if (canvasItems.length === 0) {
    return (
      <div className={cn('flex flex-1 flex-col items-center justify-center p-8', className)}>
        ...
        <h2 className="mb-2 text-xl font-semibold">Create collaborative content</h2>
        <p className="mb-8 max-w-md text-center text-muted-foreground">
          Canvas lets you create and collaborate on documents, notes, checklists, and more within your channel.
        </p>
  ...
  return (
    <div className={cn('flex flex-1 flex-col', className)}>
      ...
      <div className="flex items-center justify-between border-b px-4 py-3">
        ...
        <Button size="sm" onClick={() => handleCreateCanvas('note')} disabled={isCreating}>
          ...
          {canvasItems.map((item) => (
            ...
            >
              <div className="mb-2 flex w-full items-center justify-between">
                <CanvasTypeIcon type={item.type} className="h-5 w-5 text-muted-foreground" />
...
function CanvasTypeIcon({ type, className }: { type: CanvasItem['type']; className?: string }) {
  switch (type) {
    case 'document':
