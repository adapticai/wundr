...
export function NotificationsDialog({
  ...
  const options: { value: NotificationPreference; icon: typeof Bell; label: string; description: string }[] = [
    {
      value: 'all',
      icon: Bell,
      label: 'All messages',
  ...
  return (
    <ResponsiveModal open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <ResponsiveModalContent className="max-w-md">
        <ResponsiveModalHeader>
          <ResponsiveModalTitle>Notifications for #{channelName}</ResponsiveModalTitle>
        </ResponsiveModalHeader>
              ...
              {options.map(({ value, icon: Icon, label, description }) => (
                ...
                >
                  <div className={cn(
                    ...
                    preference === value ? 'bg-primary text-primary-foreground' : 'bg-muted'
                  )}>
