...
export function OrgNode({
  ...
  const getNodeIcon = () => {
    switch (node.type) {
      ...
      case 'orchestrator':
        return node.data?.avatarUrl ? null : (
  ...
  const disciplineColor = node.data?.discipline
    ? DISCIPLINE_COLORS[node.data.discipline] ||
            ...
            <div className='flex items-center justify-between'>
              <div className='flex items-center gap-4'>
                ...
                <div>
                  ...
                  {node.data?.role && (
                    <Badge
                      variant={getRoleBadgeVariant(node.data.role)}
                      className='mt-2'
                    >
                      {node.data.role}
                    </Badge>
      ...
      <div className='space-y-3'>
        ...
        {isExpanded && hasChildren && (
          <div className='ml-6 space-y-2 border-l-2 border-border pl-4'>
            ...
            {groupByDiscipline(node.children || []).map(
              ([discipline, orchestrators]) => (
            ...
            <div className='flex items-center gap-3'>
              <div className='relative'>
                <Avatar className='h-10 w-10'>
                  {node.data.avatarUrl && (
                    <AvatarImage src={node.data.avatarUrl} alt={node.name} />
                  )}
 ...
 */
export function OrgNodeSkeleton({
  type = 'orchestrator',
}: {
  type?: 'organization' | 'workspace' | 'orchestrator';
}) {
