...
export function OrgChart({
  ...
  if (orchestrators.length === 0) {
    return (
      <OrgChartEmptyState workspaceId={workspaceId} className={className} />
    );
  }
      ...
      >
        {filteredOrchestrators.length === 0 ? (
            ...
            {filteredOrchestrators.map(orch => (
              ...
              >
                <OrgChartNode
                  ...
                  isDimmed={
                    searchQuery.trim() !== '' &&
                    !highlightedOrchestrators.has(orch.id)
                  }
                />
              </div>
            ))}
          ...
          <div className='space-y-8'>
            {Array.from(orchestratorsByDiscipline.entries()).map(
              ([discipline, disciplineOrchestrators]) => (
                <div key={discipline}>
