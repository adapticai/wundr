...
import { X, Plus, Shield, FileWarning, Ban, CheckCircle2, Download, Upload } from 'lucide-react';
...
const PRESETS = {
  security: {
    forbiddenCommands: ['rm -rf /', 'sudo rm', 'format', 'fdisk', 'mkfs', 'dd if=', ':(){:|:&};:'],
    forbiddenPaths: ['/etc/passwd', '/etc/shadow', '~/.ssh', '/root', '/boot'],
    forbiddenActions: ['delete_production_data', 'expose_secrets', 'disable_encryption', 'bypass_auth'],
    requireApprovalFor: ['deploy_production', 'delete_workspace', 'modify_permissions', 'access_credentials'],
  },
  compliance: {
    forbiddenCommands: ['rm -rf /', 'sudo rm', 'format', 'chmod 777', 'chown root'],
    forbiddenPaths: ['/etc/passwd', '/etc/shadow', '/var/log/audit', '/etc/security'],
    forbiddenActions: ['delete_audit_logs', 'modify_compliance_data', 'disable_logging', 'expose_pii'],
    requireApprovalFor: ['access_pii', 'modify_audit_settings', 'export_user_data', 'change_retention_policy'],
  },
  production: {
    forbiddenCommands: ['rm -rf /', 'sudo rm', 'format', 'fdisk', 'reboot', 'shutdown'],
    forbiddenPaths: ['/etc/passwd', '/etc/shadow', '/var/lib/database', '/opt/production'],
    forbiddenActions: ['delete_production_data', 'modify_database_schema', 'disable_monitoring', 'bypass_rate_limits'],
    requireApprovalFor: ['deploy_production', 'rollback_production', 'scale_infrastructure', 'modify_billing'],
  },
} as const;
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-2 min-h-[2rem] p-3 border rounded-md bg-muted/30">
        {items.length === 0 ? (
          <span className="text-sm text-muted-foreground italic">No items added</span>
        ) : (
          items.map((item) => (
...
export function CharterConstraints({ value, onChange, className }: CharterConstraintsProps) {
  const [localConstraints, setLocalConstraints] = React.useState<CharterConstraints>(value);
  ...
  const handleImport = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        const imported = JSON.parse(e.target?.result as string) as CharterConstraints;
        // Validate structure
        if (
    ...
    <Card className={cn('w-full', className)}>
      ...
      <CardContent className="space-y-6">
        ...
        <Alert>
          ...
          <AlertDescription>
            These constraints protect your system from dangerous operations. Review carefully before
            modifying.
          </AlertDescription>
        </Alert>
        ...
        <div className="pt-4 border-t">
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <span className="ml-2 font-semibold">{localConstraints.forbiddenCommands.length}</span>
            </div>
            <div>
              <span className="text-muted-foreground">Blocked Paths:</span>
              <span className="ml-2 font-semibold">{localConstraints.forbiddenPaths.length}</span>
            </div>
            <div>
              <span className="text-muted-foreground">Blocked Actions:</span>
              <span className="ml-2 font-semibold">{localConstraints.forbiddenActions.length}</span>
            </div>
            <div>
              <span className="text-muted-foreground">Require Approval:</span>
              <span className="ml-2 font-semibold">{localConstraints.requireApprovalFor.length}</span>
            </div>
          </div>
        </div>
      </CardContent>
