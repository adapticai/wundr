    ...
    <div className={cn('rounded-lg border bg-card', className)}>
      ...
      {isExpanded && (
        <div className='border-t p-4'>
              ...
              {(Object.keys(TRIGGER_TYPE_CONFIG) as TriggerType[]).map(type => (
                ...
                >
                  ...
                  <span className='truncate'>
                    {TRIGGER_TYPE_CONFIG[type].label}
                  </span>
                </button>
              ))}
            </div>
          ...
          {(trigger.type === 'channel_join' ||
            trigger.type === 'channel_leave') && (
            <ChannelConfig
          ...
          {trigger.type === 'user_join' && (
            <div className='rounded-md bg-muted/50 p-4 text-sm text-muted-foreground'>
              This trigger fires when a new user joins the workspace. No
      ...
      {scheduleType === 'simple' ? (
        ...
        <div>
          ...
          <p className='mt-1.5 text-xs text-muted-foreground'>
            Format: minute hour day month day-of-week (e.g., &quot;0 9 * *
...
function WebhookConfig({ webhook, onChange }: WebhookConfigProps) {
  const webhookUrl = `https://api.neolith.app/webhooks/trigger/${webhook?.secret || 'YOUR_SECRET'}`;
