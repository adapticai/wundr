 ...
 */
function getFileCategory(mimeType: string): 'image' | 'video' | 'audio' | 'pdf' | 'document' | 'code' | 'archive' | 'other' {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-center gap-2 p-2 bg-black/50 backdrop-blur-sm">
        ...
        <span className="text-sm text-white min-w-[60px] text-center">{Math.round(zoom * 100)}%</span>
        ...
        <Button variant="ghost" size="sm" className="h-8 text-white hover:bg-white/20" onClick={handleReset}>
          Reset
        </Button>
      </div>
...
function GenericViewer({ file }: { file: PreviewFile }) {
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center p-8 text-white">
      ...
      <h3 className="text-xl font-medium mb-2 text-center">{file.originalName}</h3>
    ...
    <div className="fixed inset-0 z-50 bg-black/95 flex flex-col">
      ...
      <div className="flex items-center justify-between p-4 bg-black/50 backdrop-blur-sm">
        <div className="flex items-center gap-4 min-w-0">
          ...
          <div className="min-w-0">
            <h2 className="text-white font-medium truncate">{file.originalName}</h2>
            <p className="text-sm text-muted-foreground">
              {formatFileSize(file.size)}
              {file.uploadedBy && ` • Uploaded by ${file.uploadedBy.displayName || file.uploadedBy.name}`}
            </p>
          </div>
        </div>
      ...
      <div className="flex-1 flex relative overflow-hidden">
        ...
        <div className="flex-1 flex flex-col">
          ...
          {!['image', 'video', 'audio', 'pdf'].includes(category) && <GenericViewer file={file} />}
        </div>
