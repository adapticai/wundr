...
function OfflineBanner({
  ...
  return (
    <div className='bg-rose-600 text-white dark:bg-rose-700'>
      <div className='container mx-auto flex items-center justify-between gap-4 px-4 py-2'>
        <div className='flex items-center gap-3'>
          ...
          <div>
            <p className='text-sm font-medium font-sans'>You&apos;re offline</p>
            {queuedActions > 0 && (
              <p className='text-xs opacity-90 font-sans'>
                {queuedActions} action{queuedActions !== 1 ? 's' : ''} waiting
                to sync
              </p>
            )}
          </div>
...
function BackOnlineBanner() {
  return (
    <div className='bg-emerald-600 text-white animate-in slide-in-from-top duration-300 dark:bg-emerald-700'>
      <div className='container mx-auto flex items-center justify-center gap-2 px-4 py-2'>
        <OnlineIcon className='h-5 w-5' />
        <p className='text-sm font-medium font-sans'>You&apos;re back online</p>
      </div>
    </div>
  );
}
    ...
    >
      ...
      {queuedActions > 0 && (
        <>
          ...
          <span className='text-xs opacity-90'>{queuedActions} pending</span>
        </>
      )}
    </div>
  );
