...
export function ApiKeysManagement({ workspaceSlug }: ApiKeysManagementProps) {
  ...
  async function handleCreateKey(formData: FormData) {
    try {
      ...
      const requestsPerMinute =
        parseInt(formData.get('requestsPerMinute') as string) || 60;
      const requestsPerHour =
        parseInt(formData.get('requestsPerHour') as string) || 3600;
      const requestsPerDay =
        parseInt(formData.get('requestsPerDay') as string) || 86400;
  ...
  async function handleRegenerateKey(keyId: string) {
    try {
      const response = await fetch(
        `/api/workspaces/${workspaceSlug}/api-keys/${keyId}/regenerate`,
        {
          method: 'POST',
        }
      );
  ...
  const totalRequests = apiKeys.reduce(
    (sum, key) => sum + key.usageStats.totalRequests,
    0
  );
      ...
      <div className='grid gap-4 md:grid-cols-3'>
        ...
        <Card>
          ...
          <CardContent>
            <div className='text-2xl font-bold'>
              {apiKeys
                .reduce((sum, key) => sum + key.usageStats.last24Hours, 0)
                .toLocaleString()}
            </div>
          ...
          {isLoading ? (
            ...
            <div className='flex flex-col items-center justify-center py-12 text-center'>
              ...
              <p className='mb-4 text-sm text-muted-foreground'>
                Create your first API key to start accessing the API
                programmatically
              </p>
                    ...
                    <TableCell>
                      <div className='flex flex-col'>
                        ...
                        <span className='text-xs text-muted-foreground'>
                          / {apiKey.rateLimit.requestsPerDay.toLocaleString()}{' '}
                          limit
                        </span>
                      </div>
                    </TableCell>
                  ...
                  {PERMISSION_OPTIONS.map(permission => (
                    ...
                    >
                      <Switch
                        ...
                        checked={selectedKey.permissions.includes(
                          permission.value
                        )}
                        onCheckedChange={checked => {
                          const updatedPermissions = checked
                            ? [...selectedKey.permissions, permission.value]
                            : selectedKey.permissions.filter(
