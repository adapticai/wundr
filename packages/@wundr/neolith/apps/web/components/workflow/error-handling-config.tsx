 ...
 */
export type NotificationChannel = 'email' | 'slack' | 'webhook' | 'sms' | 'in-app';
...
function getErrorTypeBadgeVariant(type: ErrorType): 'default' | 'destructive' | 'outline' | 'secondary' {
  switch (type) {
    case 'network':
    case 'timeout':
      return 'secondary';
...
function getPriorityBadgeVariant(priority: ErrorPriority): 'default' | 'destructive' | 'outline' | 'secondary' {
  switch (priority) {
    case 'critical':
      return 'destructive';
    case 'high':
...
export function ErrorHandlingConfig({
  ...
  const [expandedDLQEntries, setExpandedDLQEntries] = React.useState<Set<string>>(new Set());
  ...
  const updateCircuitBreakerConfig = (updates: Partial<CircuitBreakerConfig>) => {
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              <RadioGroup
                ...
                onValueChange={(value) => updateConfig({ strategy: value as ErrorStrategy })}
                disabled={readOnly}
              >
                <div className="space-y-3">
                  <div className="flex items-start space-x-3 rounded-lg border p-4 hover:bg-accent">
                    ...
                    <div className="flex-1">
                      <Label htmlFor="strategy-stop" className="font-medium cursor-pointer">
                        Stop Workflow
                      </Label>
                      <p className="text-sm text-muted-foreground">
                  ...
                  <div className="flex items-start space-x-3 rounded-lg border p-4 hover:bg-accent">
                    ...
                    <div className="flex-1">
                      <Label htmlFor="strategy-continue" className="font-medium cursor-pointer">
                        Continue Workflow
                      </Label>
                      <p className="text-sm text-muted-foreground">
                  ...
                  <div className="flex items-start space-x-3 rounded-lg border p-4 hover:bg-accent">
                    ...
                    <div className="flex-1">
                      <Label htmlFor="strategy-retry" className="font-medium cursor-pointer">
                        Retry Step
                      </Label>
                      <p className="text-sm text-muted-foreground">
                  ...
                  <div className="flex items-start space-x-3 rounded-lg border p-4 hover:bg-accent">
                    ...
                    <div className="flex-1">
                      <Label htmlFor="strategy-fallback" className="font-medium cursor-pointer">
                        Execute Fallback
                      </Label>
                      <p className="text-sm text-muted-foreground">
                  ...
                  <div className="flex items-start space-x-3 rounded-lg border p-4 hover:bg-accent">
                    <RadioGroupItem value="circuit-breaker" id="strategy-circuit" />
                    <div className="flex-1">
                      <Label htmlFor="strategy-circuit" className="font-medium cursor-pointer">
                        Circuit Breaker
                      </Label>
                      <p className="text-sm text-muted-foreground">
              ...
              {config.retry.enabled && (
                <>
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) => updateRetryConfig({ maxAttempts: parseInt(e.target.value) })}
                        disabled={readOnly}
                      />
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(value) => updateRetryConfig({ backoffStrategy: value as BackoffStrategy })}
                        disabled={readOnly}
                      >
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) => updateRetryConfig({ initialDelay: parseInt(e.target.value) })}
                        disabled={readOnly}
                      />
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) => updateRetryConfig({ maxDelay: parseInt(e.target.value) })}
                        disabled={readOnly}
                      />
                    </div>
                  ...
                  <div className="rounded-lg border bg-muted/50 p-4">
                    <h4 className="text-sm font-medium mb-3">Retry Schedule Preview</h4>
                    <div className="space-y-2">
                      {Array.from({ length: config.retry.maxAttempts }).map((_, i) => {
                        const delay = i === 0 ? 0 : calculateBackoff(i, config.retry);
                        return (
                          <div key={i} className="flex items-center gap-3 text-sm">
                            ...
                            <span className="text-muted-foreground">
                              {i === 0 ? 'Initial' : `After ${formatDuration(delay)}`}
                            </span>
                          </div>
                        );
                      })}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {(['network', 'timeout', 'rate-limit', 'server', 'validation', 'client', 'authentication', 'unknown'] as ErrorType[]).map((type) => (
                        <div key={type} className="flex items-center space-x-2">
                          ...
                          />
                          <Label htmlFor={`retry-${type}`} className="text-sm cursor-pointer capitalize">
                            {type.replace('-', ' ')}
                          </Label>
                        </div>
                      ))}
            ...
            <CardContent className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) => updateFallbackConfig({ enabled: checked })}
                  disabled={readOnly}
                />
              </div>
                ...
                <>
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => updateFallbackConfig({ stepId: value })}
                      disabled={readOnly}
                    >
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="fallback-condition">Condition (Optional)</Label>
                    <Input
                      id="fallback-condition"
                      placeholder="error.type === 'network' && error.retries >= 3"
                      value={config.fallback.condition || ''}
                      onChange={(e) => updateFallbackConfig({ condition: e.target.value })}
                      disabled={readOnly}
                    />
                    <p className="text-xs text-muted-foreground">
                    ...
                    <div className="flex gap-2">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-xs text-blue-700 dark:text-blue-300">
                          When this step fails, the fallback step will execute instead. The workflow will then continue from the next step after the fallback completes.
                        </p>
                      </div>
                    </div>
                  </div>
        ...
        <TabsContent value="notifications" className="space-y-4">
            ...
            <CardContent className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) => updateNotificationConfig({ enabled: checked })}
                  disabled={readOnly}
                />
              </div>
              ...
              {config.notification.enabled && (
                <>
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {(['email', 'slack', 'webhook', 'sms', 'in-app'] as NotificationChannel[]).map((channel) => (
                        <div key={channel} className="flex items-center space-x-2">
                          <input
                            type="checkbox"
                            id={`channel-${channel}`}
                            checked={config.notification.channels.includes(channel)}
                            onChange={(e) => {
                              const channels = e.target.checked
                                ? [...config.notification.channels, channel]
                                : config.notification.channels.filter((c) => c !== channel);
                              updateNotificationConfig({ channels });
                            }}
                            disabled={readOnly}
                            className="rounded border-gray-300"
                          />
                          <Label htmlFor={`channel-${channel}`} className="text-sm cursor-pointer capitalize flex items-center gap-2">
                            {channel === 'email' && <Send className="h-3 w-3" />}
                            {channel === 'in-app' && <Bell className="h-3 w-3" />}
                            {channel}
                          </Label>
                        </div>
                      ))}
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(value) => updateNotificationConfig({ priority: value as ErrorPriority })}
                        disabled={readOnly}
                      >
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) => updateNotificationConfig({ threshold: parseInt(e.target.value) })}
                        disabled={readOnly}
                      />
                      <p className="text-xs text-muted-foreground">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) => updateNotificationConfig({ cooldown: parseInt(e.target.value) })}
                        disabled={readOnly}
                      />
                      <p className="text-xs text-muted-foreground">
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              <div className="flex items-center justify-between">
                <div className="space-y-0.5">
                  <Label htmlFor="circuit-enabled">Enable Circuit Breaker</Label>
                  <p className="text-sm text-muted-foreground">
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) => updateCircuitBreakerConfig({ enabled: checked })}
                  disabled={readOnly}
                />
              </div>
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => updateCircuitBreakerConfig({ failureThreshold: parseInt(e.target.value) })}
                      disabled={readOnly}
                    />
                    <p className="text-xs text-muted-foreground">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => updateCircuitBreakerConfig({ successThreshold: parseInt(e.target.value) })}
                      disabled={readOnly}
                    />
                    <p className="text-xs text-muted-foreground">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => updateCircuitBreakerConfig({ timeout: parseInt(e.target.value) })}
                      disabled={readOnly}
                    />
                    <p className="text-xs text-muted-foreground">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => updateCircuitBreakerConfig({ halfOpenRequests: parseInt(e.target.value) })}
                      disabled={readOnly}
                    />
                    <p className="text-xs text-muted-foreground">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                Dead Letter Queue
                {dlqEntries.length > 0 && (
                  <Badge variant="destructive">{dlqEntries.length} Failed</Badge>
                )}
              </CardTitle>
              <CardDescription>
            ...
            <CardContent>
              {dlqEntries.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <h4 className="text-lg font-medium mb-2">No Failed Executions</h4>
                  <p className="text-sm text-muted-foreground">
                    All workflow executions for this step have completed successfully
                  </p>
                </div>
              ) : (
                <div className="space-y-3">
...
function DLQEntryCard({ entry, expanded, onToggle, onRetry, onDelete }: DLQEntryCardProps) {
          ...
          <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  This will permanently remove this failed execution from the dead letter queue.
...
function ErrorRecoveryWizard({ config, onConfigChange, availableSteps }: ErrorRecoveryWizardProps) {
  const [isOpen, setIsOpen] = React.useState(false);
  const [step, setStep] = React.useState(0);
  const [wizardConfig, setWizardConfig] = React.useState<StepErrorConfig>(config);
      ...
      <AlertDialogContent className="max-w-2xl">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium capitalize">{wizardConfig.strategy}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Retry Enabled:</span>
                    <span className="font-medium">{wizardConfig.retry.enabled ? 'Yes' : 'No'}</span>
                  </div>
                  {wizardConfig.retry.enabled && (
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Max Attempts:</span>
                      <span className="font-medium">{wizardConfig.retry.maxAttempts}</span>
                    </div>
                  )}
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Fallback Enabled:</span>
                    <span className="font-medium">{wizardConfig.fallback.enabled ? 'Yes' : 'No'}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Notifications:</span>
                    <span className="font-medium">{wizardConfig.notification.enabled ? 'Yes' : 'No'}</span>
                  </div>
                </div>
              </div>
            ) : (
        ...
        <AlertDialogFooter>
          ...
          {step < steps.length - 1 ? (
            ...
            <AlertDialogAction onClick={handleComplete}>Apply Configuration</AlertDialogAction>
          )}
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
