# VP Daemon Integration API

Complete type-safe daemon API integration for VP session lifecycle management.
...
- Validates VP ownership and API key
- Stores daemon info in Redis
- Updates VP status to ONLINE
...
- List all sessions for authenticated VP
...
- `VP_NOT_FOUND` - VP doesn't exist (404)
