# VP Analytics Quick Start Guide

## Quick Implementation Guide

### 1. Display Analytics in VP Dashboard

```tsx
// In your VP dashboard page
...
import { getVPMetrics, calculateSuccessRate } from '@/lib/services/vp-analytics-service';
...
**Request:**
```bash
GET /api/vps/vp_123/analytics?timeRange=30d&includeDaily=true&includeWeekly=true
```
...
interface VPAnalyticsCardProps {
  vpId: string;              // Required: VP identifier
...
getVPAnalytics(
  vpId: string
): Promise<VPAnalytics>  // Complete analytics with all periods
```

## Common Use Cases

### 1. VP Performance Dashboard
```tsx
function VPPerformanceDashboard({ vpId }: { vpId: string }) {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      {/* 24 hour metrics */}
      <VPAnalyticsCard vpId={vpId} timeRange="24h" />

      {/* 7 day metrics */}
      <VPAnalyticsCard vpId={vpId} timeRange="7d" />

      {/* 30 day metrics */}
      <VPAnalyticsCard vpId={vpId} timeRange="30d" />
    </div>
  );
}
```
...
- [ ] Add to VP dashboard/detail page
- [ ] Test with existing VP data
...
<VPAnalyticsCard vpId={vpId} timeRange={timeRange} />
```
...
- Verify VP exists in database
- Check browser console for errors
- Verify API endpoint is accessible

### Issue: "Metrics showing 0"
- Ensure VP has completed tasks
