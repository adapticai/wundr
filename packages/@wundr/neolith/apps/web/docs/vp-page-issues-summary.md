# VP Page Issues - Quick Reference
...
|---------|-----------------|-------------------|--------|
| List VPs | `/api/workspaces/{id}/vps` | Same | ✅ Correct |
| Create VP | `/api/vps` | `/api/workspaces/{id}/vps` | ❌ Wrong |
| Update VP | `/api/vps/{id}` | `/api/workspaces/{id}/vps/{vpId}` | ⚠️ Works but not workspace-scoped |
| Delete VP | `/api/vps/{id}` | `/api/workspaces/{id}/vps/{vpId}` | ⚠️ Works but not workspace-scoped |
| Toggle Status | `/api/vps/{id}` (PATCH) | `/api/workspaces/{id}/vps/{vpId}/status` | ❌ Wrong |
...
2. ❌ **Toggle VP Status** - Will fail or use wrong endpoint
...
4. ✅ **Empty States** - Proper messaging when no VPs
...
- [ ] Add error boundary to VP page
- [ ] Extract VP transformation logic to utility function
- [ ] Add Playwright tests for VP creation flow
...
**User Impact:** SEVERE
- 100% of VP creation attempts will fail
- Status toggles non-functional
- Core workflow completely broken

**Business Impact:**
- Cannot onboard new Virtual Persons
- Cannot manage existing VPs effectively
