# VP Coordination API Reference

Quick reference guide for VP Cross-Coordination API endpoints.

## Base URL

All endpoints are under: `/api/vps`
...
Transfer task ownership from one VP to another.

**Endpoint**: `POST /api/vps/:id/delegate`

**Parameters**:
- `:id` - Source VP ID (path parameter)

**Request Body**:
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| toVpId | string | Yes | Target VP ID (cuid) |
...
**Example**:
```bash
curl -X POST /api/vps/vp_123/delegate \
...
**Endpoint**: `POST /api/vps/:id/collaborate`

**Parameters**:
- `:id` - Primary VP ID (path parameter)

**Request Body**:
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| taskId | string | Yes | Task ID requiring collaboration (cuid) |
| requiredVpIds | string[] | Yes | Array of VP IDs (1-10, cuid) |
| roles | object | No | Map of VP ID to role name |
| note | string | No | Collaboration note (max 1000 chars) |

**Example**:
```bash
curl -X POST /api/vps/vp_123/collaborate \
  -H "Content-Type: application/json" \
  -d '{
    "taskId": "task_789",
...
Handoff a task to another VP with context transfer.

**Endpoint**: `POST /api/vps/:id/handoff`
...
**Example**:
```bash
curl -X POST /api/vps/vp_123/handoff \
  -H "Content-Type: application/json" \
  -d '{
    "toVpId": "vp_456",
    "taskId": "task_789",
...
**Endpoint**: `POST /api/vps/conflicts`

**Request Body**:
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| vpIds | string[] | Yes | VP IDs involved (2-10, cuid) |
...
curl -X POST /api/vps/conflicts \
  ...
  -d '{
    ...
    "taskId": "task_789",
    "note": "Both VPs can work on different aspects"
  }'
```
...
| Code | Description |
|------|-------------|
| VP_COORDINATION_NOT_FOUND | VP or task not found |
| UNAUTHORIZED | Authentication required |
| FORBIDDEN | Insufficient permissions |
| VALIDATION_ERROR | Invalid input data |
| VP_NOT_FOUND | Specified VP doesn't exist |
| TASK_NOT_FOUND | Specified task doesn't exist |
| DIFFERENT_ORGANIZATION | VPs in different organizations |
| INVALID_OWNERSHIP | Task doesn't belong to VP |
