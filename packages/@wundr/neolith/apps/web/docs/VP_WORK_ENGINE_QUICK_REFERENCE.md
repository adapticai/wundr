# VP Work Engine - Quick Reference

## Service Location
```
/apps/web/lib/services/vp-work-engine-service.ts
```

## Core Functions

### 1. selectNextTask
**Purpose:** Select the highest priority task for a VP
...
**Algorithm:**
- Filters by VP availability (ONLINE/BUSY)
...
**Purpose:** Get execution statistics for a VP
...
import {
  ...
  updateTaskProgress,
  completeTask
} from '@/lib/services/vp-work-engine-service';
...
|-------|-------|----------|
| `VP not found` | Invalid VP ID | Verify VP exists |
| `Task not found` | Invalid task ID | Verify task exists |
| `not assigned to VP` | Task belongs to different VP | Check task ownership |
...
### Type Check
```bash
npx tsc --noEmit lib/services/vp-work-engine-service.ts
```
