# Virtual Persons Page - QA Test Report
...
The Virtual Persons (VPs) page has been analyzed for functionality, API integration, and user experience. Several critical issues were identified that will prevent the page from functioning correctly in production.
...
**Issue:** The `createVP` function calls `/api/vps` but should call `/api/workspaces/${workspaceId}/vps`

**Current Code:**
```typescript
const createVP = useCallback(async (input: CreateVPInput): Promise<VP | null> => {
  // ...
  const response = await fetch('/api/vps', {  // WRONG!
    method: 'POST',
...
**Expected Code:**
```typescript
const createVP = useCallback(async (input: CreateVPInput & { workspaceId: string }): Promise<VP | null> => {
  const response = await fetch(`/api/workspaces/${input.workspaceId}/vps`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(input),
  });
...
**Impact:** The "Create VP" button will fail with a 404 error. VPs cannot be created from the UI.

**Steps to Reproduce:**
1. Navigate to `/{workspaceId}/vps`
...
**Evidence:**
- API route exists at: `/app/api/workspaces/[workspaceId]/vps/route.ts` (POST handler confirmed)
- Hook incorrectly targets: `/api/vps` (no such route exists for workspace-scoped creation)

---

### 2. CRITICAL: Missing workspaceId Parameter in onCreate Handler

**Location:** `/app/(workspace)/[workspaceId]/vps/page.tsx` (Lines 46-52)
...
**Impact:** Even after fixing issue #1, VP creation will fail because the API requires the workspace context.
...
**Location:** `/components/vp/create-vp-dialog.tsx` (Lines 139-144)
...
**Location:** `/app/(workspace)/[workspaceId]/vps/page.tsx` (Lines 115-140)
...
### 5. HIGH: VP Card Toggle Status Uses Incorrect API

**Location:** `/hooks/use-vp.ts` (Lines 442-446)

**Issue:** The `toggleVPStatus` mutation calls `/api/vps/${id}` (workspace-agnostic endpoint) instead of the workspace-scoped endpoint
...
**Update VP Implementation (lines 391-416):**
```typescript
const response = await fetch(`/api/vps/${id}`, {  // Wrong endpoint
  method: 'PATCH',
  // ...
});
```

**Correct Endpoint:** `/api/workspaces/${workspaceId}/vps/${vpId}/status`

**Impact:** Status toggle button on VP cards will fail. Users cannot change VP online/offline status.
...
**Location:** `/app/(workspace)/[workspaceId]/vps/page.tsx`
...
**Issue:** VP data transformation from API response is duplicated in `useVP` (lines 130-148) and `useVPs` (lines 253-271). DRY violation.
...
**Location:** `/app/(workspace)/[workspaceId]/vps/page.tsx` (Lines 230-234)
...
2. **Clean UI Components:** Well-structured VP cards with status indicators
3. **Empty States:** Excellent empty state handling with contextual messages
4. **Stats Overview:** Real-time VP status counts (Online, Offline, Busy, Away)
5. **Multi-step Creation Dialog:** Good UX for VP creation with validation
...
|----------|--------|---------|--------|
| `/api/workspaces/${workspaceId}/vps` | GET | List VPs | ✅ Correctly implemented |
| `/api/workspaces/${workspaceId}/vps` | POST | Create VP | ❌ Hook uses wrong endpoint |
| `/api/vps/${id}` | PATCH | Update VP | ⚠️ Should be workspace-scoped |
| `/api/vps/${id}` | DELETE | Delete VP | ⚠️ Should be workspace-scoped |

### Missing Endpoints (Not Critical)

- `/api/workspaces/${workspaceId}/vps/${vpId}/status` - Dedicated status update
...
1. **404 on VP Creation:**
   ```
   POST /api/vps 404 (Not Found)
   Failed to create VP
   ```
...
1. ❌ VP list loads without errors (requires authentication)
2. ❌ Status filter buttons work (require DOM interaction)
3. ❌ Create VP modal opens (requires button click)
4. ❌ VP card interactions (require mouse events)
...
1. Refactor VP data transformation into shared utility
2. Add Playwright tests for VP page interactions
3. Implement status filter button UI for better UX
4. Add unit tests for VP hooks (`use-vp.ts`)
5. Add integration tests for API routes

### Long-term Enhancements

1. Add optimistic updates for status toggles
2. Implement VP batch operations UI
3. Add VP search with debouncing
4. Add VP sorting controls (currently API supports it, UI doesn't)
5. Implement cursor-based pagination for large VP lists
...
The Virtual Persons page has a solid foundation with good UI/UX patterns, but **cannot function in production** due to critical API endpoint mismatches. The `createVP` and `toggleVPStatus` functions will fail immediately when invoked.
...
- **Page Component:** `/Users/iroselli/wundr/packages/@wundr/neolith/apps/web/app/(workspace)/[workspaceId]/vps/page.tsx`
- **VP Hooks:** `/Users/iroselli/wundr/packages/@wundr/neolith/apps/web/hooks/use-vp.ts`
- **VP Card Component:** `/Users/iroselli/wundr/packages/@wundr/neolith/apps/web/components/vp/vp-card.tsx`
- **Create Dialog:** `/Users/iroselli/wundr/packages/@wundr/neolith/apps/web/components/vp/create-vp-dialog.tsx`
- **API Route (GET/POST):** `/Users/iroselli/wundr/packages/@wundr/neolith/apps/web/app/api/workspaces/[workspaceId]/vps/route.ts`
