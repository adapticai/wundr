...
**Date**: 2025-11-27 **Build Status**: ✅ **SUCCESS** **Build Time**: ~8.7s compilation + ~412ms
static generation **Total Routes**: 317 (77 static pages + 240 dynamic routes)
...
**Severity**: Medium **Impact**: Future compatibility **Category**: API Deprecation
...
Next.js 16.0.3 has deprecated the `viewport` property in the `metadata` export. The viewport
configuration should now be in a separate `generateViewport` export.
...
**Severity**: Low **Impact**: Build performance only **Category**: Dependency Compatibility
...
The `handlebars` package (v4.7.8) used by `@wundr/org-genesis` package uses Node.js
`require.extensions`, which is not supported by webpack's module system. This is a compatibility
warning from the handlebars library itself.
...
The same import is processed 3 times during webpack's bundling phases (likely tree-shaking,
optimization, and final bundle).
...
**Severity**: Low **Impact**: Development experience only
...
Turbopack configuration in `next.config.mjs` has a relative `turbopack.root` path. While webpack
mode is used for production builds, this warning appears during development.
...
The build is **fully functional and production-ready** with only minor warnings that can be
addressed in future sprints. All critical paths compile successfully, TypeScript checking passes,
and static generation works efficiently.
...
**Report Generated by**: Agent 19 - Build & Runtime Auditor **Timestamp**: 2025-11-27T03:07:00Z
