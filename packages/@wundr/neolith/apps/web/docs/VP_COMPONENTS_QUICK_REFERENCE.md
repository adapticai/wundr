# VP Components Quick Reference Guide

Quick reference for using VP interaction components in the Neolith web app.
...
import {
  ...
  VPMessageBadge,         // Compact VP badge
...
import {
  useVP,              // Single VP data
  useVPs,             // Multiple VPs
  useVPMutations,     // CRUD operations
  useVPPresence,      // NEW - Single VP presence
  useMultipleVPPresence, // NEW - Multiple VP presence
} from '@/hooks';
```
...
### 2. VP Message in Chat
```tsx
<VPMessageWrapper isVP={message.isFromVP}>
...
| vpName | string | No | VP name |
| taskContext | string | No | What VP is doing |
...
### Pattern: Multiple VP Monitoring
```tsx
...
- Check VP type matches database schema
...
2. **Use message wrappers** for VP messages to maintain visual consistency
3. **Show thinking indicators** whenever VP is processing
...
10. **Add analytics** to track VP interaction patterns
