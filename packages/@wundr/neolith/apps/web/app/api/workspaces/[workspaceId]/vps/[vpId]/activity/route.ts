/**
 * VP Activity Log API Routes
 *
 * Handles activity logging and retrieval for Virtual Person (VP) entities.
 * Activity is stored in the VPMemory table with specific memory types for tracking.
 *
 * Routes:
 * - GET /api/workspaces/:workspaceId/vps/:vpId/activity - List VP activity with filters
 * - POST /api/workspaces/:workspaceId/vps/:vpId/activity - Log new activity (internal use)
 *
 * @module app/api/workspaces/[workspaceId]/vps/[vpId]/activity/route
 */
 ...
 * Returns the VP if accessible, null otherwise
 */
...
/**
 * GET /api/workspaces/:workspaceId/vps/:vpId/activity
 *
 * List VP activity with filtering, pagination, and date range support.
 *
 * Query parameters:
 ...
 * @returns List of VP activities with pagination metadata
 */
export async function GET(
  ...
  try {
    ...
    console.error('[GET /api/workspaces/:workspaceId/vps/:vpId/activity] Error:', error);
    return NextResponse.json(
      createErrorResponse(
...
/**
 * POST /api/workspaces/:workspaceId/vps/:vpId/activity
 *
 * Log new VP activity (internal use - typically called by VP daemon or system).
 *
 * Request body:
 ...
 *
 * Requires authentication and admin/owner role OR VP service account.
 *
...
export async function POST(
  ...
  try {
    ...
    // Check permissions: either admin/owner OR the VP's own service account
    const isVPServiceAccount = session.user.isVP && session.user.id === result.vp.userId;
    const hasAdminAccess = result.role === 'OWNER' || result.role === 'ADMIN';

    if (!isVPServiceAccount && !hasAdminAccess) {
      return NextResponse.json(
        createErrorResponse(
          'Insufficient permissions to log activity for this VP',
    ...
    console.error('[POST /api/workspaces/:workspaceId/vps/:vpId/activity] Error:', error);
    return NextResponse.json(
      createErrorResponse(
        'An internal error occurred',
        VP_ERROR_CODES.INTERNAL_ERROR,
