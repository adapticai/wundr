 ...
 *
 * Routes:
 * - POST /api/workspaces/:workspaceId/orchestrators/:orchestratorId/status-update
 *
 * @module app/api/workspaces/[workspaceId]/orchestrators/[orchestratorId]/status-update/route
 */
...
import type { StatusUpdateInput } from '@/lib/validations/orchestrator-conversation';
 ...
 * POST /api/workspaces/:workspaceId/orchestrators/:orchestratorId/status-update
 *
 * Orchestrator posts a status update to assigned channels.
 * - Auto-targets Orchestrator's assigned channels if not specified
...
export async function POST(
  ...
  try {
    ...
    // If task ID is provided, verify it exists and belongs to this Orchestrator
    if (input.taskId) {
    ...
    if (input.channelIds && input.channelIds.length > 0) {
      // Use provided channel IDs, but verify they exist and Orchestrator has access
    ...
    const channelMembers = await prisma.channelMember.findMany({
      where: {
        ...
        userId: { not: orchestrator.user.id },
      },
      select: {
        userId: true,
        channelId: true,
