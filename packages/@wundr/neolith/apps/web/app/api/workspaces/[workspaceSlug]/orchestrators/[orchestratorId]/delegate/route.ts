...
export async function POST(
  ...
  try {
    ...
    if (input.createNotification) {
      notification = await prisma.notification.create({
        data: {
          ...
          body: input.note || `${orchestrator.user.name} has delegated a task to you`,
    ...
    console.error('[POST /api/workspaces/:workspaceId/orchestrators/:orchestratorId/delegate] Error:', error);
    return NextResponse.json(
      createErrorResponse(
