...
export async function POST(
  ...
  try {
    ...
    if (!workspaceMembership) {
      return NextResponse.json(
        createErrorResponse('You must be a workspace member to activate workflows', WORKFLOW_ERROR_CODES.FORBIDDEN),
        { status: 403 },
      );
    }
    ...
    if (workflow.status === 'ACTIVE') {
      return NextResponse.json({ workflow, message: 'Workflow is already active' });
    }

    // Check if workflow is archived
    if (workflow.status === 'ARCHIVED') {
      return NextResponse.json(
        createErrorResponse('Cannot activate an archived workflow', WORKFLOW_ERROR_CODES.WORKFLOW_INACTIVE),
        { status: 400 },
      );
    }
    ...
    return NextResponse.json({ workflow: updatedWorkflow, message: 'Workflow activated successfully' });
  } catch (error) {
    console.error('[POST /api/workspaces/:workspaceId/workflows/:workflowId/activate] Error:', error);
    return NextResponse.json(
