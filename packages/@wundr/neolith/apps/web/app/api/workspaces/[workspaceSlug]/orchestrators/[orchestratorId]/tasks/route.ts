/**
 * Orchestrator-Specific Tasks API Routes
 *
 * Handles task listing and assignment for a specific Orchestrator within a workspace.
 * All tasks are scoped to both the workspace and the Orchestrator.
 *
 * Routes:
 * - GET /api/workspaces/[workspaceId]/orchestrators/[orchestratorId]/tasks - List tasks for Orchestrator
 * - POST /api/workspaces/[workspaceId]/orchestrators/[orchestratorId]/tasks - Assign new task to Orchestrator
 *
 * @module app/api/workspaces/[workspaceId]/orchestrators/[orchestratorId]/tasks/route
 */
...
/**
 * GET /api/workspaces/[workspaceId]/orchestrators/[orchestratorId]/tasks
 *
 * List tasks assigned to a specific Orchestrator within a workspace.
 ...
 * @returns Paginated list of tasks for the Orchestrator
 *
 * @example
 * ```
 * GET /api/workspaces/ws_123/orchestrators/orch_456/tasks?status=TODO,IN_PROGRESS&limit=50
 * ```
 */
export async function GET(
  ...
  try {
    ...
    if (!workspaceId || !orchestratorId) {
      return NextResponse.json(
        createErrorResponse('Workspace ID and OrchestratorID are required', TASK_ERROR_CODES.VALIDATION_ERROR),
        { status: 400 },
      );
    }
    ...
    if (orchestrator.workspaceId && orchestrator.workspaceId !== workspaceId) {
      return NextResponse.json(
        createErrorResponse('Orchestrator does not belong to this workspace', TASK_ERROR_CODES.FORBIDDEN),
        { status: 403 },
      );
    }
    ...
    console.error('[GET /api/workspaces/[workspaceId]/orchestrators/[orchestratorId]/tasks] Error:', error);
    return NextResponse.json(
...
/**
 * POST /api/workspaces/[workspaceId]/orchestrators/[orchestratorId]/tasks
 *
 * Assign a new task to a specific Orchestrator. Requires authentication and workspace access.
 * Validates task dependencies and ensures Orchestrator belongs to the workspace.
 *
 * Request body:
 * {
 ...
 * @example
 * ```
 * POST /api/workspaces/ws_123/orchestrators/orch_456/tasks
...
export async function POST(
  ...
  try {
    ...
    console.error('[POST /api/workspaces/[workspaceId]/orchestrators/[orchestratorId]/tasks] Error:', error);
