/**
 * VP Mentions API Route
 *
 * Allows VPs to fetch and manage their @mentions in channels.
 * Supports filtering by channel, handled status, and date range.
 *
 * Routes:
 * - GET /api/workspaces/:workspaceId/vps/:vpId/mentions - Get VP mentions
 * - POST /api/workspaces/:workspaceId/vps/:vpId/mentions - Mark mentions as handled
 *
 * @module app/api/workspaces/[workspaceId]/vps/[vpId]/mentions/route
 */
...
/**
 * GET /api/workspaces/:workspaceId/vps/:vpId/mentions
 *
 * Get @mentions for this VP with filtering and pagination.
...
export async function GET(
  ...
  try {
    ...
    // Validate filters
    const parseResult = vpMentionsFiltersSchema.safeParse(queryParams);
    ...
    const filters: VPMentionsFiltersInput = parseResult.data;
    ...
    if (vp.user.id !== session.user.id) {
      return NextResponse.json(
        createErrorResponse(
          'Unauthorized: You can only access mentions for your own VP',
    ...
    const mentionPatterns = [
      `@${vp.user.name}`,
      `@${vp.user.displayName || vp.user.name}`,
      `@${vp.user.email.split('@')[0]}`, // Handle email prefix mentions
    ];
    ...
    const formattedMentions = mentions.map((mention) => {
      ...
      const vpMentionData = metadata?.vpMention as Record<string, unknown> | undefined;
    ...
    console.error('[GET /api/workspaces/:workspaceId/vps/:vpId/mentions] Error:', error);
    return NextResponse.json(
      createErrorResponse(
...
/**
 * POST /api/workspaces/:workspaceId/vps/:vpId/mentions
 *
 * Mark VP mentions as handled or unhandled.
...
export async function POST(
  ...
  try {
    ...
    // Verify workspace and VP (same as GET)
    const workspace = await prisma.workspace.findUnique({
      where: { id: workspaceId },
      select: { id: true, organizationId: true },
    });
    ...
    if (!vp || vp.user.id !== session.user.id) {
      return NextResponse.json(
        createErrorResponse(
          'VP not found or access denied',
          VP_CONVERSATION_ERROR_CODES.FORBIDDEN,
    ...
    if (mentions.length !== input.mentionIds.length) {
      return NextResponse.json(
        createErrorResponse(
          ...
          VP_CONVERSATION_ERROR_CODES.NOT_FOUND,
        ),
        { status: 404 },
      );
    }
    ...
    console.error('[POST /api/workspaces/:workspaceId/vps/:vpId/mentions] Error:', error);
    return NextResponse.json(
      createErrorResponse(
        'An internal error occurred',
        VP_CONVERSATION_ERROR_CODES.INTERNAL_ERROR,
