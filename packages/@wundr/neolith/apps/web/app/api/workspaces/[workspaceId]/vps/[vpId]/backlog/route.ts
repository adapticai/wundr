/**
 * VP Backlog API Routes
 *
 * Handles retrieving and adding tasks to a VP's backlog.
 *
 * Routes:
 * - GET /api/workspaces/[workspaceId]/vps/[vpId]/backlog - Get VP's task backlog
 * - POST /api/workspaces/[workspaceId]/vps/[vpId]/backlog - Add task to VP's backlog
 *
 * @module app/api/workspaces/[workspaceId]/vps/[vpId]/backlog/route
 */
 ...
 *
 * Returns tasks assigned to this VP in backlog order with optional filters.
 ...
 * @returns VP's task backlog with optional statistics
 *
 * @example
 * ```
 * GET /api/workspaces/ws_123/vps/vp_456/backlog?status=TODO&priority=HIGH&includeStats=true
 * ```
 */
export async function GET(
  ...
  try {
    ...
    if (filters.includeStats) {
      const stats = await getTaskMetrics(vpId, workspaceId);
      response.statistics = stats;
    }

    return NextResponse.json(response);
  } catch (error) {
    console.error('[GET /api/workspaces/[workspaceId]/vps/[vpId]/backlog] Error:', error);
    return NextResponse.json(
 ...
 *
 * Add a new task to the VP's backlog.
 *
 * Request body:
 * {
 *   "title": "Task title",
 ...
 * @example
 * ```
 * POST /api/workspaces/ws_123/vps/vp_456/backlog
...
export async function POST(
  ...
  try {
    ...
    return NextResponse.json(
      { data: task, message: 'Task added to VP backlog successfully' },
      { status: 201 },
    );
  } catch (error) {
    console.error('[POST /api/workspaces/[workspaceId]/vps/[vpId]/backlog] Error:', error);
