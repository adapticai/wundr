/**
 * VP Comparison Analytics API Route
 *
 * Compares VP performance across a workspace.
 * Ranks VPs by various metrics to identify top performers and improvement areas.
 *
 * Routes:
 * - GET /api/workspaces/:workspaceId/vps/analytics/comparison - Compare VP performance
 *
 * @module app/api/workspaces/[workspaceId]/vps/analytics/comparison/route
 */
...
import { compareVPs } from '@/lib/services/vp-analytics-service-extended';
import {
  vpComparisonQuerySchema,
...
/**
 * GET /api/workspaces/:workspaceId/vps/analytics/comparison
 *
 ...
 * @returns VP comparison data with rankings
 *
 * @example
 * ```
 * GET /api/workspaces/ws_123/vps/analytics/comparison?metric=taskCompletionRate&limit=10
 * ```
 */
export async function GET(
  ...
  try {
    ...
    const parseResult = vpComparisonQuerySchema.safeParse(searchParams);
    ...
    const query: VPComparisonQueryInput = parseResult.data;
    ...
    if (query.discipline) {
      filteredComparison = comparison.filter(
        (vp) => vp.discipline.toLowerCase() === query.discipline?.toLowerCase(),
      );
    }
    ...
    if (!query.includeInactive) {
      const activeVPIds = await prisma.vP
        ...
        .then((vps) => vps.map((vp) => vp.id));

      filteredComparison = filteredComparison.filter((vp) =>
        activeVPIds.includes(vp.vpId),
      );
    }

    // Get total VP count for context
    ...
    const metricValues = filteredComparison.map((vp) => vp.metricValue);
    ...
    const response = {
      ...
      rankings: filteredComparison.map((vp) => ({
        rank: vp.rank,
        vpId: vp.vpId,
        vpName: vp.vpName,
        discipline: vp.discipline,
        role: vp.role,
        metricValue: Math.round(vp.metricValue * 100) / 100,
        percentile: vp.percentile,
        trend: vp.trend,
      })),
      summary: {
    ...
    if (filteredComparison.length === 0) {
      response.insights.push('No VP data available for the selected criteria');
    } else {
      ...
      if (filteredComparison.length >= 3) {
        const topThreeAvg =
          filteredComparison.slice(0, 3).reduce((sum, vp) => sum + vp.metricValue, 0) / 3;
        response.insights.push(
    ...
    return NextResponse.json({
      data: response,
      message: 'VP comparison retrieved successfully',
    });
  } catch (error) {
    console.error(
      '[GET /api/workspaces/:workspaceId/vps/analytics/comparison] Error:',
      error,
    );
    return NextResponse.json(
      createAnalyticsErrorResponse(
