/**
 * VP Capacity API Routes
 *
 * Handles VP capacity and workload management including
 * max concurrent tasks, energy budget, and utilization metrics.
 *
 * Routes:
 * - GET /api/workspaces/:workspaceId/vps/:vpId/capacity - Get VP capacity and utilization
 * - PATCH /api/workspaces/:workspaceId/vps/:vpId/capacity - Update VP capacity limits
 *
 * @module app/api/workspaces/[workspaceId]/vps/[vpId]/capacity/route
 */
...
import { getCapacity, updateCapacity } from '@/lib/services/vp-scheduling-service';
...
/**
 * GET /api/workspaces/:workspaceId/vps/:vpId/capacity
 *
 * Get VP's current capacity/workload including:
 ...
 * @returns VP capacity configuration and utilization metrics
 *
 * @example
 * ```
 * GET /api/workspaces/ws_123/vps/vp_456/capacity?includeMetrics=true
 *
 * Response:
 * {
 *   "data": {
...
export async function GET(
  ...
  try {
    ...
    console.error('[GET /api/workspaces/:workspaceId/vps/:vpId/capacity] Error:', error);
    return NextResponse.json(
...
/**
 * PATCH /api/workspaces/:workspaceId/vps/:vpId/capacity
 *
 * Update VP's capacity limits.
 ...
 * @example
 * ```
 * PATCH /api/workspaces/ws_123/vps/vp_456/capacity
...
export async function PATCH(
  ...
  try {
    ...
    if (result.role !== 'OWNER' && result.role !== 'ADMIN') {
      return NextResponse.json(
        createErrorResponse(
          'Insufficient permissions to update VP capacity',
    ...
    const updatedCapacity = await updateCapacity(vpId, parseResult.data);
    ...
    console.error('[PATCH /api/workspaces/:workspaceId/vps/:vpId/capacity] Error:', error);
    return NextResponse.json(
      createErrorResponse('An internal error occurred', VP_ERROR_CODES.INTERNAL_ERROR),
      { status: 500 },
    );
