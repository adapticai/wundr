/**
 * OrchestratorChannel Activity API Routes
 *
 * Tracks and retrieves Orchestrator activity metrics per channel including
 * messages sent, tasks from channel, and last active timestamps.
 *
 * Routes:
 * - GET /api/workspaces/:workspaceId/orchestrators/:orchestratorId/channel-activity
 * - POST /api/workspaces/:workspaceId/orchestrators/:orchestratorId/channel-activity
 *
 * @module app/api/workspaces/[workspaceId]/orchestrators/[orchestratorId]/channel-activity/route
 */
 ...
 * GET /api/workspaces/:workspaceId/orchestrators/:orchestratorId/channel-activity
 *
 * Get Orchestrator's activity metrics per channel including:
 * - Messages sent
    ...
    const activityMetrics = await Promise.all(
      memberships.map(async (membership) => {
        const messageWhere = {
          ...
          authorId: result.orchestrator.user.id,
        ...
        const lastMessage = await prisma.message.findFirst({
          where: { channelId: membership.channelId, authorId: result.orchestrator.user.id },
...
return -1;
    ...
    console.error(
      '[GET /api/workspaces/:workspaceId/orchestrators/:orchestratorId/channel-activity] Error:',
      error,
    );
    return NextResponse.json(
      createChannelIntelligenceError(
 ...
 * POST /api/workspaces/:workspaceId/orchestrators/:orchestratorId/channel-activity
 *
 * Track a Orchestrator channel activity event.
 *
 * Query Parameters:
...
export async function POST(
  ...
  try {
    ...
    console.error(
      '[POST /api/workspaces/:workspaceId/orchestrators/:orchestratorId/channel-activity] Error:',
      error,
    );
    return NextResponse.json(
      createChannelIntelligenceError(
