/**
 * VP Analytics Trends API Route
 *
 * Provides VP performance trends over time with daily/weekly/monthly aggregations.
 * Includes comparison to previous periods for trend analysis.
 *
 * Routes:
 * - GET /api/workspaces/:workspaceId/vps/:vpId/analytics/trends - Get VP performance trends
 *
 * @module app/api/workspaces/[workspaceId]/vps/[vpId]/analytics/trends/route
 */
...
import { getVPTrends } from '@/lib/services/vp-analytics-service';
import {
  vpTrendsQuerySchema,
...
/**
 * GET /api/workspaces/:workspaceId/vps/:vpId/analytics/trends
 *
 * Get VP performance trends over time with aggregations.
 ...
 * @returns VP performance trends data
 *
 * @example
 * ```
 * GET /api/workspaces/ws_123/vps/vp_456/analytics/trends?period=daily&timeRange=30d
 * ```
 */
export async function GET(
  ...
  try {
    ...
    const parseResult = vpTrendsQuerySchema.safeParse(searchParams);
    ...
    const query: VPTrendsQueryInput = parseResult.data;
    ...
    const trends = await getVPTrends(vpId, 'completions', query.timeRange);
    ...
    return NextResponse.json({
      data: response,
      message: 'VP trends retrieved successfully',
    });
  } catch (error) {
    console.error(
      '[GET /api/workspaces/:workspaceId/vps/:vpId/analytics/trends] Error:',
      error,
    );
    return NextResponse.json(
      createAnalyticsErrorResponse(
