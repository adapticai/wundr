...
export async function POST(
  ...
  try {
    ...
    if (!channelMember) {
      return NextResponse.json(
        createErrorResponse('Not authorized to join this call', CALL_ERROR_CODES.FORBIDDEN),
        { status: 403 },
      );
    }
    ...
    if (!['pending', 'ringing', 'active'].includes(call.status)) {
      return NextResponse.json(
        createErrorResponse('Call is no longer available', CALL_ERROR_CODES.CALL_ALREADY_ENDED),
        { status: 409 },
      );
    }
