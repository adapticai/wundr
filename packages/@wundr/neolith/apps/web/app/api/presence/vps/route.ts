/**
 * VP Presence List API Route
 *
 * Get presence status for all VPs in an organization.
 *
 * Routes:
 * - GET /api/presence/vps?organizationId= - Get VP presence list
 *
 * @module app/api/presence/vps/route
 */
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const responses: VPPresenceResponse[] = vps.map((vp) => ({
      vpId: vp.id,
      userId: vp.userId,
    ...
    console.error('[GET /api/presence/vps] Error:', error);
    return NextResponse.json(
      createPresenceErrorResponse(
