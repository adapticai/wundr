  ...
  try {
    ...
    switch (entityType) {
      case 'orchestrator':
        ...
        if (parseResult.success) {
          ...
          if (!parseResult.data.discipline) {
            warnings.push('No discipline specified. Consider adding one for better organization.');
          }
          if (!parseResult.data.channels || parseResult.data.channels.length === 0) {
            warnings.push('No channels specified. Orchestrator will not monitor any channels initially.');
          }
      ...
      case 'channel':
        ...
        if (parseResult.success) {
          if (!parseResult.data.description) {
            warnings.push('No description provided. Consider adding one for clarity.');
          }
          if (parseResult.data.type === 'PRIVATE' && parseResult.data.initialMembers.length === 0) {
      ...
      case 'workspace':
        ...
        if (parseResult.success) {
          if (!parseResult.data.initialOrchestrators || parseResult.data.initialOrchestrators.length === 0) {
            warnings.push('No initial orchestrators specified. You can add them later.');
          }
          if (!parseResult.data.initialChannels || parseResult.data.initialChannels.length === 0) {
            warnings.push('No initial channels specified. You can add them later.');
          }
        }
        break;
      ...
      case 'session-manager':
        ...
        if (parseResult.success) {
          if (!parseResult.data.channelId) {
            warnings.push('No channel specified. Session manager will not be bound to a specific channel.');
          }
          if (!parseResult.data.escalationCriteria || parseResult.data.escalationCriteria.length === 0) {
