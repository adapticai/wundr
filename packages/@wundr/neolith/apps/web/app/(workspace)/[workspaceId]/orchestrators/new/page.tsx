...
export default function NewOrchestratorPage() {
  ...
  const handleCancel = useCallback(() => {
    router.push(`/${workspaceId}/orchestrators`);
  }, [router, workspaceId]);
  ...
  const handleCreateOrchestrator = useCallback(
    async (finalSpec: EntitySpec) => {
      ...
      try {
        ...
        const response = await fetch(`/api/workspaces/${workspaceId}/orchestrators`, {
          method: 'POST',
          headers: {
        ...
        router.push(`/${workspaceId}/orchestrators/${result.data.id}`);
      } catch (err) {
