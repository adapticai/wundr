...
export default function OrchestratorHealthDashboardPage() {
  ...
  const { orchestratorList, isLoading, refetch } = useOrchestratorHealthList(workspaceId);
  ...
  const handleHealthCheck = useCallback(async () => {
    ...
    try {
      await fetch(`/api/workspaces/${workspaceId}/orchestrators/health-check`, {
        method: 'POST',
      });
      await refetch();
    } catch {
  ...
  const handleViewDetails = useCallback(
    (orchestratorId: string) => {
      window.location.href = `/${workspaceId}/orchestrators/${orchestratorId}`;
    },
    [workspaceId],
  );
