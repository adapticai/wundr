...
export function IntegrationSettings({ config, onSave, disabled }: IntegrationSettingsProps) {
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Configure webhook endpoints to receive notifications about orchestrator activities
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div>
            ...
            <div className="flex gap-2">
              <Input
                ...
                type="url"
              />
              <Button type="button" onClick={addWebhook} disabled={disabled || !newWebhook}>
          ...
          {webhookUrls.length > 0 ? (
            <div className="space-y-2">
              {webhookUrls.map((url) => (
                <div key={url} className="flex items-center justify-between border rounded-lg p-3">
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-sm text-muted-foreground">
            Integration configurations are managed through workspace settings. Your orchestrator
            will have access to all workspace-level integrations based on your capabilities.
          </div>
        </CardContent>
      </Card>
