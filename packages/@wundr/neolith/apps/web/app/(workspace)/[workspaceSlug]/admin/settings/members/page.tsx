...
export default function MembersSettingsPage() {
  ...
  // Set page header
  useEffect(() => {
    setPageHeader('Members & Permissions', 'Configure workspace member settings and access controls');
  }, [setPageHeader]);
  ...
  const [inviteLink] = useState(`https://app.wundr.ai/invite/${workspaceSlug}/abc123`); // Mock link

  // Load settings
  useEffect(() => {
    const loadSettings = async () => {
      try {
        const response = await fetch(`/api/workspaces/${workspaceSlug}/admin/settings/members`);
  ...
  const handleToggle = useCallback(async (key: keyof MemberSettings) => {
  ...
  const handleSelectChange = useCallback(async (key: keyof MemberSettings, value: string | number) => {
    const currentValue = settings[key];
  ...
  const regenerateInviteLink = useCallback(async () => {
    setIsSaving(true);
    try {
      const response = await fetch(`/api/workspaces/${workspaceSlug}/admin/invite-link/regenerate`, {
        method: 'POST',
      });
      ...
      toast({
        title: 'Error',
        description: error instanceof Error ? error.message : 'Failed to regenerate link',
        variant: 'destructive',
      });
    } finally {
      setIsSaving(false);
  ...
  return (
    <div className="space-y-6">
            ...
            <>
              <div className="border-t pt-6">
                ...
                <select
                  ...
                  onChange={(e) => handleSelectChange('whoCanInvite', e.target.value)}
                  disabled={isSaving}
                  className={cn(
      ...
      <div className="rounded-lg border bg-card">
        <div className="border-b px-6 py-4">
          <h2 className="text-lg font-semibold text-foreground">Join Settings</h2>
          <p className="text-sm text-muted-foreground">
        ...
        <div className="space-y-6 p-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="font-medium text-foreground">Allow email domain join</p>
              <p className="text-sm text-muted-foreground">
          ...
          <div className="flex items-center justify-between border-t pt-6">
            <div>
              <p className="font-medium text-foreground">Require approval for new members</p>
              <p className="text-sm text-muted-foreground">
          ...
          <div className="border-t pt-6">
            ...
            <select
              ...
              onChange={(e) => handleSelectChange('autoAssignRoleId', e.target.value)}
              disabled={isSaving}
              className={cn(
                'block w-full rounded-md border border-input bg-background',
                'px-3 py-2 text-sm',
      ...
      <div className="rounded-lg border bg-card">
        <div className="border-b px-6 py-4">
          <h2 className="text-lg font-semibold text-foreground">Guest Access</h2>
          <p className="text-sm text-muted-foreground">
            ...
            <>
              ...
              <div className="border-t pt-6">
                ...
                <select
                  ...
                  onChange={(e) => handleSelectChange('guestChannelAccess', e.target.value)}
                  disabled={isSaving}
                  className={cn(
                    'block w-full rounded-md border border-input bg-background',
                    'px-3 py-2 text-sm',
              ...
              <div className="border-t pt-6">
                ...
                <select
                  ...
                  onChange={(e) => handleSelectChange('guestAccountExpiration', Number(e.target.value))}
                  disabled={isSaving}
                  className={cn(
                    'block w-full rounded-md border border-input bg-background',
                    'px-3 py-2 text-sm',
                ...
                <p className="mt-2 text-sm text-muted-foreground">
                  Guest accounts will be automatically deactivated after this period
                </p>
              </div>
            </>
          )}
      ...
      {settings.memberLimit && (
        <div className="rounded-lg border bg-card">
          <div className="border-b px-6 py-4">
            <h2 className="text-lg font-semibold text-foreground">Member Limits</h2>
            <p className="text-sm text-muted-foreground">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-medium text-foreground">
                    {settings.currentMemberCount} / {settings.memberLimit} members
                  </span>
                ...
                <div className="h-2 w-full overflow-hidden rounded-full bg-muted">
                  <div
                    ...
                    style={{ width: `${Math.min(memberLimitPercentage, 100)}%` }}
                  />
                </div>
              </div>
                ...
                <div className="flex items-start gap-3 rounded-lg border border-yellow-500/50 bg-yellow-50 p-4 dark:bg-yellow-900/10">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="mt-1 text-sm text-yellow-700 dark:text-yellow-300">
                      You're using {memberLimitPercentage.toFixed(0)}% of your member capacity. Consider upgrading your plan.
                    </p>
                    <Link
