...
import type { OrchestratorFilters, OrchestratorStatus } from '@/types/orchestrator';
...
export default function AdminOrchestratorsPage() {
  ...
  const { orchestrators, isLoading, error, refetch } = useOrchestrators(workspaceSlug, filters);

  useEffect(() => {
    setPageHeader('Orchestrator Management', 'Manage and monitor all workspace orchestrators');
  }, [setPageHeader]);
    ...
    <div className="space-y-6">
      ...
      {error && (
        <div className="rounded-lg border border-red-200 bg-red-50 p-4">
          <p className="text-sm text-red-800">Failed to load orchestrators: {error.message}</p>
