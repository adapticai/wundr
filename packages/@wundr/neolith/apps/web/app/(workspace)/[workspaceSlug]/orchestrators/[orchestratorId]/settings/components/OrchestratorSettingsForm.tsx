...
export function OrchestratorSettingsForm({
  ...
  const handleSave = async (section: string, data: Record<string, unknown>) => {
    ...
    try {
      const response = await fetch(`/api/orchestrators/${orchestrator.id}/config`, {
        method: 'PUT',
        headers: {
    ...
    <div className="space-y-6">
        ...
        <Card className="border-yellow-500 bg-yellow-50 dark:bg-yellow-950">
          <CardHeader>
            ...
            <CardDescription className="text-yellow-700 dark:text-yellow-300">
              These settings have been locked by an administrator. You can view but not modify
              them.
            </CardDescription>
          </CardHeader>
        </Card>
      ...
      {isAdmin && (
        <Card className="border-blue-500 bg-blue-50 dark:bg-blue-950">
          ...
          <CardContent>
            <Button
              variant={orchestrator.config?.isLocked ? 'destructive' : 'default'}
              onClick={() =>
                handleSave('Admin', {
              ...
              disabled={isPending}
            >
              {orchestrator.config?.isLocked ? 'Unlock Configuration' : 'Lock Configuration'}
            </Button>
          </CardContent>
        </Card>
      )}
