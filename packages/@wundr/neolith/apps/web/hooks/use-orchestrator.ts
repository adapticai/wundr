...
export function useOrchestrator(id: string): UseOrchestratorReturn {
  ...
  const fetchOrchestrator = useCallback(
    async (signal?: AbortSignal): Promise<void> => {
      ...
      try {
        ...
        const validStatuses: Array<Orchestrator['status']> = [
          'ONLINE',
          'OFFLINE',
          'BUSY',
          'AWAY',
        ...
        const transformedOrchestrator: Orchestrator = {
          ...
          title:
            apiOrchestrator.role ||
            apiOrchestrator.title ||
            'Untitled Orchestrator',
          description:
            apiOrchestrator.user?.bio || apiOrchestrator.description || null,
          discipline: apiOrchestrator.discipline || null,
          status: orchestratorStatus,
          charter: apiOrchestrator.charter
            ? (apiOrchestrator.charter as unknown as Orchestrator['charter'])
            : null,
          capabilities: Array.isArray(apiOrchestrator.capabilities)
            ? apiOrchestrator.capabilities
            : [],
          modelConfig: apiOrchestrator.modelConfig
            ? (apiOrchestrator.modelConfig as unknown as Orchestrator['modelConfig'])
            : null,
          systemPrompt: apiOrchestrator.systemPrompt || null,
          organizationId: apiOrchestrator.organizationId || null,
          avatarUrl:
            apiOrchestrator.user?.avatarUrl ||
            apiOrchestrator.avatarUrl ||
            null,
          lastActivityAt: apiOrchestrator.lastActivityAt
            ? new Date(apiOrchestrator.lastActivityAt)
            : apiOrchestrator.updatedAt
              ? new Date(apiOrchestrator.updatedAt)
              : null,
          messageCount: apiOrchestrator.messageCount || 0,
          agentCount: apiOrchestrator.agentCount || 0,
          createdAt: apiOrchestrator.createdAt
            ? new Date(apiOrchestrator.createdAt)
            : new Date(),
          updatedAt: apiOrchestrator.updatedAt
            ? new Date(apiOrchestrator.updatedAt)
            : new Date(),
        };
...
export function useOrchestrators(
  ...
  const fetchOrchestrators = useCallback(
    async (signal?: AbortSignal): Promise<void> => {
      if (!workspaceId) {
        return;
      }
          ...
          .map((apiOrchestrator: OrchestratorApiResponse) => {
            ...
            return {
              ...
              title:
                ...
                'Untitled Orchestrator',
              description:
                apiOrchestrator.user?.bio ||
                apiOrchestrator.description ||
                null,
              discipline: apiOrchestrator.discipline || null,
              status: orchestratorStatus,
              charter: apiOrchestrator.charter
    ...
    async (
      ...
      try {
        ...
        const error =
          err instanceof Error
            ? err
            : new Error('Unknown error occurred while creating orchestrator');
        setError(error);
        return null;
      } finally {
        setIsLoading(false);
    ...
    async (
      ...
      try {
        ...
        const error =
          err instanceof Error
            ? err
            : new Error('Unknown error occurred while updating orchestrator');
        setError(error);
        return null;
      } finally {
        setIsLoading(false);
    ...
    async (id: string, signal?: AbortSignal): Promise<boolean> => {
      ...
      try {
        ...
        const error =
          err instanceof Error
            ? err
            : new Error('Unknown error occurred while deleting orchestrator');
        setError(error);
        return false;
      } finally {
        setIsLoading(false);
    ...
    async (
      ...
      try {
        ...
        const error =
          err instanceof Error
            ? err
            : new Error('Unknown error occurred while rotating API key');
        setError(error);
        return null;
      } finally {
        setIsLoading(false);
