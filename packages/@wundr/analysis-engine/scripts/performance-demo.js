...
} = require('../dist/engines/DuplicateDetectionEngineOptimized');
...
async function runPerformanceDemo() {
  ...
  console.log(
    `   ✅ Peak memory usage: ${Math.round(memoryResult.peakMemoryMB)}MB`
  );
  ...
  console.log(
    `   Small (1K): ${smallResult.clustersFound} clusters in ${smallResult.durationMs}ms`
  );
  console.log(
    `   Medium (5K): ${mediumResult.clustersFound} clusters in ${mediumResult.durationMs}ms`
  );
  console.log(
    `   Large (10K): ${largeResult.clustersFound} clusters in ${largeResult.durationMs}ms`
  );

  console.log('\n💾 Memory Efficiency:');
  console.log(
    `   Small dataset peak: ${Math.round(smallResult.peakMemoryMB)}MB`
  );
  console.log(
    `   Medium dataset peak: ${Math.round(mediumResult.peakMemoryMB)}MB`
  );
  console.log(
    `   Large dataset peak: ${Math.round(largeResult.peakMemoryMB)}MB`
  );

  console.log('\n⚡ Throughput Performance:');
  console.log(
    `   Small: ${Math.round(smallResult.entitiesPerSecond)} entities/sec`
  );
  console.log(
    `   Medium: ${Math.round(mediumResult.entitiesPerSecond)} entities/sec`
  );
  console.log(
    `   Large: ${Math.round(largeResult.entitiesPerSecond)} entities/sec`
  );

  // Save comprehensive results
  await fs.writeJSON(
    path.join(outputDir, 'performance-demo-results.json'),
    results,
    { spaces: 2 }
  );
  ...
  await fs.writeFile(
    path.join(outputDir, 'performance-summary.md'),
    summaryReport
  );

  console.log(
    `📄 Summary report: ${path.join(outputDir, 'performance-summary.md')}`
  );
...
async function testOptimizedEngine(entities, size) {
  ...
  const startTime = Date.now();
  const results = await optimizedEngine.analyze(entities, {
    targetDir: './demo',
  });
  const duration = Date.now() - startTime;
...
function generateTestEntities(count) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    entities.push({
      id: `entity-${i}`,
      name: isDuplicate
        ? `${pattern}Data${Math.floor(baseIndex / 20)}`
        : `${pattern}Data${Math.floor(i / 20)}`,
      type: type,
      ...
      signature: isDuplicate
        ? `${pattern}Data${Math.floor(baseIndex / 20)}(): void`
        : `${pattern}Data${Math.floor(i / 20)}(): void`,
...
|--------------|----------|----------------|---------------|------------------|---------------------------|
${results.tests
  .filter(t => t.test.startsWith('optimized-engine'))
  .map(
    test =>
