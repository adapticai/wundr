...
const BaseAnalysisService_1 = require("./BaseAnalysisService");
...
class OptimizedBaseAnalysisService extends BaseAnalysisService_1.BaseAnalysisService {
    // Memory optimization components (rename to avoid conflicts)
    optimizedStreamingProcessor;
    optimizedWorkerPool;
    optimizedMemoryMonitor;
    optimizedObjectPools = {
        entities: [],
        buffers: [],
        arrays: []
    };
    constructor(name, config) {
        ...
        this.optimizedStreamingProcessor = new StreamingFileProcessor_1.StreamingFileProcessor({
        ...
        this.optimizedWorkerPool = new WorkerPoolManager_1.WorkerPoolManager({
        ...
        this.optimizedMemoryMonitor = new MemoryMonitor_1.MemoryMonitor({
     ...
     */
    setupOptimizedMemoryOptimizations() {
        // Initialize object pools
        for (let i = 0; i < 100; i++) {
            this.optimizedObjectPools.entities.push({});
     ...
     */
    async filterFilesByCriteriaOptimized(files) {
        ...
        for (let i = 0; i < files.length; i += batchSize) {
            ...
            filteredFiles.push(...batchResults.filter(Boolean));
     ...
     */
    async getLineCountEfficient(filePath) {
     ...
     */
    async extractEntitiesOptimizedConcurrent(files) {
        this.emitProgress({
            type: 'phase',
        ...
        const fileChunks = (0, utils_1.chunk)(files, batchSize);
        let processedFiles = 0;
     ...
     */
    async performAnalysisConcurrent(entities) {
        ...
        if (entitiesByType.size > 0) {
            analysisPromises.push(this.optimizedWorkerPool.submitTask({
        ...
        if (complexEntities.length > 0) {
            const complexityChunks = (0, utils_1.chunk)(complexEntities, 100);
     ...
     * Generate report with memory optimization
     */
    async generateReportOptimized(files, entities, analysisResults, startTime) {
     ...
     */
    calculateMemoryEfficiency(fileCount, peakMemory) {
    ...
    async cleanupOptimized() {
        ...
        if (this.config.verbose) {
            ...
            console.log(chalk_1.default.gray(`Peak Usage: ${(0, utils_1.formatFileSize)(finalMetrics.peak.heapUsed)}`));
            console.log(chalk_1.default.gray(`Average Usage: ${(0, utils_1.formatFileSize)(finalMetrics.average.heapUsed)}`));
        }
    }
    sleep(ms) {
