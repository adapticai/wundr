...
export abstract class BaseAnalysisService {
   ...
   */
  private async filterFilesByCriteria(files: string[]): Promise<string[]> {
  ...
  private async extractEntitiesOptimized(
    ...
    const fileChunks = chunk(files, this.config.performance.chunkSize);
...
<body>
  <div class="container">
    <div class="header">
      ...
      <p><strong>Analysis Time:</strong> ${formatDuration(report.performance.analysisTime)}</p>
    </div>
...
**Analysis Time:** ${formatDuration(report.performance.analysisTime)}
...
- **Memory Peak:** ${formatFileSize(report.performance.memoryUsage.peak)}
