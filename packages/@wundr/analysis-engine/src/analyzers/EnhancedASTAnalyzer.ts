...
export class EnhancedASTAnalyzer extends BaseAnalysisService {
  ...
  protected override extractEntityFromNode(node: ts.Node, sourceFile: ts.SourceFile): EntityInfo | null {
    ...
    switch (node.kind) {
      case ts.SyntaxKind.ClassDeclaration:
        return this.extractClassEntity(node as ts.ClassDeclaration, filePath, position);
      
      case ts.SyntaxKind.InterfaceDeclaration:
        return this.extractInterfaceEntity(node as ts.InterfaceDeclaration, filePath, position);
      
      case ts.SyntaxKind.TypeAliasDeclaration:
        return this.extractTypeAliasEntity(node as ts.TypeAliasDeclaration, filePath, position);
      
      case ts.SyntaxKind.EnumDeclaration:
        return this.extractEnumEntity(node as ts.EnumDeclaration, filePath, position);
      
      case ts.SyntaxKind.FunctionDeclaration:
        return this.extractFunctionEntity(node as ts.FunctionDeclaration, filePath, position);
      
      case ts.SyntaxKind.VariableStatement:
        return this.extractVariableEntity(node as ts.VariableStatement, filePath, position);
  ...
  private extractClassEntity(
    ...
    const entity: EntityInfo = {
      ...
      semanticHash: generateSemanticHash({ methods, properties, type: 'class' }),
  ...
  private extractInterfaceEntity(
    ...
    return {
      ...
      semanticHash: generateSemanticHash({ properties, methods, type: 'interface' }),
  ...
  private extractFunctionEntity(
    ...
    return {
      ...
      semanticHash: generateSemanticHash({ name, parameters: func.parameters.length }),
   ...
   */
  private async detectDuplicatesOptimized(entities: EntityInfo[]): Promise<any[]> {
    this.emitProgress({ type: 'phase', message: 'Detecting duplicates with clustering...' });
    ...
    for (const [hash, duplicateEntities] of hashGroups.entries()) {
      if (duplicateEntities.length > 1) {
        const cluster = {
          ...
          similarity: 1.0,
          consolidationSuggestion: this.generateConsolidationSuggestion(duplicateEntities)
        };
  ...
  private async detectCircularDependencies(): Promise<any[]> {
    try {
      this.emitProgress({ type: 'phase', message: 'Analyzing circular dependencies...' });
      ...
      if (this.config.verbose) {
        console.warn(chalk.yellow('⚠️ Could not run madge for circular dependencies'));
      }
        ...
        if (sourceFile) {
          sourceFile.forEachChild(child => {
            if (ts.isImportDeclaration(child) && child.moduleSpecifier) {
              const moduleSpecifier = child.moduleSpecifier.getText().replace(/['"]/g, '');
  ...
  private analyzeEntityForCodeSmells(entity: EntityInfo): any[] {
    ...
    if (complexity?.lines && complexity.lines > this.config.thresholds.fileSize.maxLines / 10) {
      smells.push({
        id: createId(),
        type: 'long-method',
        severity: complexity.lines > this.config.thresholds.fileSize.maxLines / 5 ? 'high' : 'medium',
        file: entity.file,
        line: entity.line,
    ...
    // High complexity
    if (complexity?.cyclomatic && complexity.cyclomatic > this.config.thresholds.complexity.cyclomatic) {
      smells.push({
        id: createId(),
        type: 'complex-conditional',
        severity: complexity.cyclomatic > this.config.thresholds.complexity.cyclomatic * 2 ? 'critical' : 'high',
        file: entity.file,
        line: entity.line,
    ...
    if (entity.type === 'class' && entity.members?.methods && entity.members.methods.length > 20) {
      smells.push({
        id: createId(),
        type: 'god-object',
        severity: 'high',
  ...
  private isServiceClass(classDecl: ts.ClassDeclaration, name: string): boolean {
    return (
  ...
  private isReactComponent(classDecl: ts.ClassDeclaration, name: string): boolean {
    return (
  ...
  private isUtilityFunction(func: ts.FunctionDeclaration, name: string): boolean {
    return (
      name.includes('util') ||
  ...
  private extendsBaseClass(classDecl: ts.ClassDeclaration, baseClassName: string): boolean {
  ...
  private extractInterfaceProperties(interfaceDecl: ts.InterfaceDeclaration): any[] {
  ...
  private extractInterfaceMethods(interfaceDecl: ts.InterfaceDeclaration): any[] {
    return interfaceDecl.members
  ...
  private calculateNodeComplexity(node: ts.Node): ComplexityMetrics {
    ...
    const visit = (child: ts.Node, currentDepth: number) => {
      ...
      const newDepth = this.isNestingStructure(child) ? currentDepth + 1 : currentDepth;
    ...
    const maintainability = Math.max(0, Math.min(100, 
      171 - 5.2 * Math.log(lines || 1) - 0.23 * cyclomatic - 16.2 * Math.log(parameters + 1)
    ));
  ...
  private extractNodeDependencies(node: ts.Node, currentFile: string): string[] {
  ...
  private extractImportedNames(importClause: ts.ImportClause, usedNames: Set<string>): void {
    // Default import
  ...
  private calculateDuplicateSeverity(entities: EntityInfo[]): 'critical' | 'high' | 'medium' | 'low' {
    const count = entities.length;
    const totalComplexity = entities.reduce((sum, e) => sum + (e.complexity?.cyclomatic || 0), 0);
    const avgDependencies = entities.reduce((sum, e) => sum + e.dependencies.length, 0) / count;
  ...
  private calculateCircularDependencySeverity(cycle: string[]): 'critical' | 'high' | 'medium' | 'low' {
  ...
  private generateConsolidationSuggestion(entities: EntityInfo[]): any {
    ...
    return {
      ...
      estimatedEffort: entities.length > 3 ? 'high' : entities.length > 2 ? 'medium' : 'low',
  ...
  private generateRecommendations(analysisResults: any): any[] {
    ...
    if (analysisResults.circularDeps?.length > 0) {
      recommendations.push({
        ...
        description: 'Circular dependencies can cause build issues and make code harder to understand',
    ...
    // Code smells
    const criticalSmells = analysisResults.codeSmells?.filter((s: any) => s.severity === 'critical') || [];
    if (criticalSmells.length > 0) {
      recommendations.push({
        ...
        description: 'High complexity code is harder to maintain and more prone to bugs',
  ...
  private generateVisualizationData(entities: EntityInfo[], analysisResults: any): any {
    ...
    return {
      ...
      duplicateNetworks: this.generateDuplicateNetworkData(analysisResults.duplicates),
