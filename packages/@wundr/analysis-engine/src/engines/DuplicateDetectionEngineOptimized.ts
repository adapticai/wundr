...
import { generateNormalizedHash, createId } from '../utils';
  ...
  implements BaseAnalyzer<DuplicateCluster[]>
{
  ...
  private async performOptimizedDuplicateAnalysis(
    ...
    if (this.config.enableStructuralAnalysis) {
      ...
      const hashClusters =
        await this.detectHashBasedDuplicatesConcurrent(entities);
      clusters.push(...hashClusters);
      this.emit('phase-completed', {
        phase: 'structural-analysis',
        clusters: hashClusters.length,
      });
    }
    ...
    if (this.config.enableSemanticAnalysis) {
      ...
      const semanticClusters =
        await this.detectSemanticDuplicatesConcurrent(entities);
    ...
    if (this.config.enableFuzzyMatching) {
      ...
      this.emit('phase-completed', {
        phase: 'fuzzy-analysis',
        clusters: fuzzyClusters.length,
      });
    }
  ...
  private async detectFuzzyDuplicatesStreaming(
    ...
    for (const [_entityType, typeEntities] of entitiesByType.entries()) {
      ...
      for (let i = 0; i < typeEntities.length; i += chunkSize) {
        const chunk = typeEntities.slice(i, i + chunkSize);
        const chunkClusters =
          await this.findFuzzyClustersForTypeOptimized(chunk);
  ...
  private async hierarchicalClusteringOptimized(
    ...
    for (const cluster of clusters) {
      ...
      for (const otherCluster of clusters) {
        ...
        if (
          this.shouldMergeClusters(cluster, otherCluster) &&
          cluster.similarity >= 0.9 &&
          otherCluster.similarity >= 0.9
        ) {
          // Check size limit before merging
          if (
            mergedCluster.entities.length + otherCluster.entities.length <=
  ...
  private optimizeClusters(clusters: DuplicateCluster[]): DuplicateCluster[] {
    ...
    for (const cluster of clusters) {
      const key = cluster.entities
        .map(e => e.id)
        .sort()
        .join('-');

      if (
        !uniqueClusters.has(key) ||
  ...
  private generateConsolidationSuggestion(
    ...
    const estimatedEffort =
      entities.length > 5 ? 'high' : entities.length > 3 ? 'medium' : 'low';
