...
import { EntityInfo } from '../../src/types';
...
describe('Memory Optimization Simple Integration Test', () => {
  ...
  const outputDir = path.join(
    __dirname,
    '../../test-output/simple-integration'
  );
  ...
  test('✅ Basic Memory Monitor Functionality', async () => {
    ...
    console.log(
      `📊 Current heap usage: ${Math.round(metrics.current.heapUsed / 1024 / 1024)}MB`
    );
  }, 10000);
  ...
  test('✅ Basic Optimized Engine Functionality', async () => {
    ...
    const results = await optimizedEngine.analyze(testEntities, {
      targetDir: './test',
    });
    ...
    console.log(
      `✅ Optimized Engine: ${results.length} clusters found in ${duration}ms`
    );
    ...
    // Save results
    await fs.writeJSON(
      path.join(outputDir, 'optimized-engine-results.json'),
      {
...
function generateSimpleEntities(count: number): EntityInfo[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const actualName = isDuplicate
      ? `TestEntity${Math.floor((i - 20) / 10)}`
      : name;
