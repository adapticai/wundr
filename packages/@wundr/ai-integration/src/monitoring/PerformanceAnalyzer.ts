...
export class PerformanceAnalyzer extends EventEmitter {
  ...
  private async collectAndAnalyzeMetrics(): Promise<void> {
    try {
      ...
      if (this.config.bottleneckDetection) {
        const bottlenecks =
          await this.bottleneckDetector.detectBottlenecks(snapshot);
  ...
  private async calculateSystemLoad(
  ...
  private async applyOptimization(bottleneck: Bottleneck): Promise<void> {
    try {
      ...
      console.error(
        `Failed to apply optimization for ${bottleneck.type}:`,
        error
      );
  ...
  private identifyRiskFactors(task: Task): RiskFactor[] {
    ...
    if (complexity > 8) {
      risks.push({
        ...
        severity: 'high',
        description:
          'Task complexity may lead to longer execution times and higher resource usage',
    ...
    if (task.requiredCapabilities.length > 5) {
      risks.push({
        ...
        severity: 'medium',
        description:
          'Multiple capability requirements may cause agent contention',
  ...
  private generateTaskRecommendations(analysis: TaskAnalysis): string[] {
    ...
    if (recentPerformance.responseTime > 3000) {
      recommendations.push(
        'System is under load - consider delaying non-critical tasks'
      );
    }
  ...
  private getRecentPerformanceAverage(): PerformanceMetrics {
    ...
    const avg = recent.reduce(
      (sum, snapshot) => ({
        ...
        activeConnections:
          sum.activeConnections + snapshot.metrics.activeConnections,
  ...
  async generatePerformanceReport(): Promise<PerformanceReport> {
    ...
    const recentTrends = await this.trendAnalyzer.analyzeTrends(
      this.performanceHistory
    );
    const activeBottlenecks =
      await this.bottleneckDetector.getCurrentBottlenecks();
  ...
  private generateSystemRecommendations(
    ...
    if (metrics.memoryUsage > 80) {
      recommendations.push(
        'High memory usage detected - consider enabling compression'
      );
    ...
    if (metrics.responseTime > this.config.alertThresholds.responseTime) {
      recommendations.push(
        'Response times are elevated - investigate bottlenecks'
      );
    ...
    if (metrics.throughput < 5) {
      recommendations.push(
        'Low throughput detected - check for blocking operations'
      );
    }
    ...
    if (trends.memoryTrend === 'increasing') {
      recommendations.push(
        'Memory usage trend is increasing - monitor for leaks'
      );
    }

    if (trends.errorTrend === 'increasing') {
      recommendations.push(
        'Error rate is trending upward - investigate root causes'
      );
    }
...
class AlertManager {
  ...
  async checkAlerts(snapshot: PerformanceSnapshot, trends: any): Promise<void> {
    ...
    if (
      snapshot.metrics.responseTime > this.config.alertThresholds.responseTime
    ) {
      alerts.push({
        type: 'threshold',
        metric: 'responseTime',
  ...
  private emitAlert(alert: any): void {
    console.warn(
      `ALERT [${alert.severity.toUpperCase()}]: ${alert.type} - ${alert.metric}`,
      alert
    );
  }
