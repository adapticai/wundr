...
export class SwarmIntelligence extends EventEmitter {
  ...
  async selectOptimalTopology(
    ...
    const bestTopology = Array.from(topologyScores.entries()).sort(
      ([, a], [, b]) => b - a
    )[0];
    ...
    const customizedTopology = await this.customizeTopology(
      selectedTemplate,
      analysis
    );
  ...
  private assessScalabilityNeed(agentTypes: string[], task: string): string {
    if (
      agentTypes.length > 10 ||
      task.toLowerCase().includes('monorepo') ||
  ...
  private assessTimeConstraints(task: string): string {
    if (
      ...
      task.toLowerCase().includes('fast')
    ) {
      return 'medium';
    } else {
      return 'relaxed';
  ...
  private assessResourceConstraints(agentTypes: string[]): string {
    const resourceHeavyAgents = [
      'ml-developer',
      'performance-benchmarker',
      'perf-analyzer',
    ];
    const hasResourceHeavyAgents = agentTypes.some(type =>
      resourceHeavyAgents.includes(type)
    );

    return hasResourceHeavyAgents ? 'high' : 'medium';
  }

  private calculateTopologyScore(
    ...
    score +=
      faultToleranceScore[analysis.faultToleranceRequirement]?.[
        topology.faultTolerance
      ] || 0;
  ...
  private async customizeTopology(
    template: SwarmTopology,
    analysis: any
  ): Promise<SwarmTopology> {
  ...
  private suggestOptimizations(
    ...
    if (
      analysis.faultToleranceRequirement === 'high' &&
      topology.faultTolerance !== 'high'
    ) {
  ...
  private async coordinateExecution(
    ...
    try {
      ...
      const taskDistribution = await this.distributeSubTasks(
        subTasks,
        swarm,
        tools
      );
  ...
  private generateSubTaskOutput(subTask: SubTask): string {
    ...
    return (
      outputs[subTask.type] || `${subTask.description} completed successfully`
    );
  }
  ...
  private generateInsights(swarm: SwarmInstance, result: any): string[] {
    ...
    const avgAgentSuccess =
      Array.from(swarm.agents.values()).reduce(
  ...
  private async processCollectiveLearning(
    ...
    if (result.success && result.data?.qualityScore > 0.8) {
      ...
      const successfulPatterns = this.collectiveMemory.get(
        'successful-patterns'
      );
  ...
  async optimizeTopology(bottleneck: Bottleneck): Promise<void> {
    const optimizationHistory = this.collectiveMemory.get(
  ...
  async getMetrics(): Promise<any> {
    return {
      ...
      patterns: {
        ...
        performance:
          this.collectiveMemory.get('performance-metrics')?.size || 0,
      },
    };
  }
...
class CommunicationMatrix {
  ...
  private buildRingConnections(): void {
    for (let i = 0; i < this.agents.length; i++) {
      ...
      const prev =
        this.agents[(i - 1 + this.agents.length) % this.agents.length];
  ...
  private findOptimalConnections(agent: Agent): string[] {
    return this.agents
      ...
      .sort(
        (a, b) =>
          this.calculateConnectionScore(agent, b) -
          this.calculateConnectionScore(agent, a)
      )
  ...
  private calculateConnectionScore(agent1: Agent, agent2: Agent): number {
    ...
    const commonCaps = agent1.capabilities.filter(cap =>
...
class ConsensusEngine {
  ...
  async reachConsensus(
    ...
    const consensus = {
      ...
      decision:
        successfulResults.length > resultsArray.length / 2
          ? 'accept'
          : 'reject',
