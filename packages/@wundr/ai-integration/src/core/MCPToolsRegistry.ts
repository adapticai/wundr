...
export class MCPToolsRegistry extends EventEmitter {
      ...
      try {
        ...
        if (await fs.pathExists(handlerPath.replace('.ts', '.js'))) {
          ...
          this.toolHandlers.set(
            toolId,
            new DefaultMCPHandler(toolId, toolConfig)
          );
        }
      } catch (error) {
        const errorMessage =
          error instanceof Error ? error.message : 'Unknown error';
        console.warn(
          `Failed to initialize handler for ${toolId}: ${errorMessage}`
        );
        this.toolHandlers.set(
          toolId,
          new DefaultMCPHandler(toolId, toolConfig)
  ...
  private async discoverServerTools(
    ...
    try {
      ...
      console.warn(
        `Failed to discover tools for server ${serverName}: ${error.message}`
      );
    }
  }
