...
class AgentLoadBalancer {
  ...
  async selectOptimalAgents(availableAgents: Agent[], task: Task): Promise<Agent[]> {
    ...
    if (isComplexTask) {
      ...
      const sortedAgents = availableAgents.sort((a, b) => {
        ...
        // Then sort by load
        return (this.agentLoads.get(a.id) || 0) - (this.agentLoads.get(b.id) || 0);
      });
...
class AgentPerformanceTracker {
  ...
  async recordTaskCompletion(agent: Agent, _task: Task, result: any): Promise<void> {
    const data = this.performanceData.get(agent.id) || { tasks: 0, successes: 0 };
    data.tasks++;
...
export class AgentCoordinator extends EventEmitter {
  ...
  private async preSpawnCoreAgents(): Promise<void> {
    ...
    for (const agentType of coreAgents) {
      try {
        ...
        console.warn(`Failed to pre-spawn ${agentType}: ${(error as Error).message}`);
      }
    }
  }
  ...
  private async createAgent(type: AgentType, sessionId: string): Promise<Agent> {
    return {
  ...
  private getCategoryForType(type: AgentType): string {
    const categoryMap: Record<string, string> = {
      'coder': 'core', 'reviewer': 'core', 'tester': 'core', 'planner': 'core', 'researcher': 'core',
      'hierarchical-coordinator': 'swarm', 'mesh-coordinator': 'swarm', 'adaptive-coordinator': 'swarm',
      'ml-developer': 'specialized', 'backend-dev': 'specialized', 'mobile-dev': 'specialized'
    };
  ...
  async selectAgentsForTask(task: Task): Promise<Agent[]> {
    ...
    const compatibleAgents = this.filterAgentsByCapabilities(availableAgents, task.requiredCapabilities);
    ...
    const idleCompatibleAgents = compatibleAgents.filter(agent => agent.status === 'idle');
  ...
  async completeTask(taskId: string, result: any): Promise<void> {
    ...
    for (const agentId of task.assignedAgents) {
      ...
      if (agent) {
        ...
        if (completedTasks > 0) {
          const successCount = result.success ? agent.metrics.tasksCompleted : agent.metrics.tasksCompleted - 1;
        ...
        if (result.executionTime) {
          ...
          agent.metrics.averageResponseTime = ((currentAvg * (completedCount - 1)) + result.executionTime) / completedCount;
        }
  ...
  private filterAgentsByCapabilities(agents: Agent[], requiredCapabilities: string[]): Agent[] {
  ...
  private async recoverAgent(agent: Agent): Promise<void> {
    try {
      ...
      console.error(`Failed to recover agent ${agent.id}: ${(error as Error).message}`);
    }
  }

  async getMetrics(): Promise<any> {
    const agents = Array.from(this.agents.values());
    const activeTasks = Array.from(this.taskQueue.values()).filter(t => t.status === 'in-progress' || t.status === 'assigned').length;
    const totalAssignments = Array.from(this.agentAssignments.values()).reduce((sum, assignments) => sum + assignments.length, 0);
