...
export const AgentDefaultsSchema = SingleAgentSchema.omit({ type: true }).extend({
...
export const AgentsSchema = z.object({
  ...
  list: z.array(SingleAgentSchema.extend({
...
export const AuditSchema = z.object({
  ...
  events: z.array(z.enum([
    ...
    'auth.login', 'auth.failed', 'config.change',
    'tool.denied', 'admin.action',
  ]),
...
const ChannelDmPolicySchema = z.enum(['open', 'pairing', 'closed']).default('pairing');

const ChannelRetrySchema = z.object({
...
}).catchall(z.record(z.unknown()).optional()).default({});
...
export const DistributedSchema = z.object({
  ...
  loadBalancingStrategy: z.enum([
    'round-robin', 'least-loaded', 'weighted', 'hash-based',
...
export const TokenBudgetSchema = z.object({
  ...
  (data) => data.weekly >= data.daily,
  { message: 'Weekly budget must be >= daily budget', path: ['weekly'] },
).refine(
  (data) => data.monthly >= data.weekly,
  { message: 'Monthly budget must be >= weekly budget', path: ['monthly'] },
);
...
export const NeolithSchema = z.object({
  /** Neolith API URL */
...
export function validateConfig(raw: unknown): ConfigValidationResult {
  ...
  try {
    ...
    partialConfig = WundrConfigSchema.parse({ openai: { apiKey: 'placeholder' } });
  }
...
function collectWarnings(config: WundrConfig): ConfigValidationIssue[] {
  ...
  if (config.security.jwt.secret === 'change-this-in-production-to-a-random-secure-string') {
    warnings.push({
  ...
  if (config.nodeEnv === 'production' && config.daemon.verbose) {
    warnings.push({
      path: 'daemon.verbose',
      message: 'Verbose mode is enabled in production. This may impact performance.',
    });
  }

  // Warn about CORS in production
  if (config.nodeEnv === 'production' && config.security.cors.enabled && config.security.cors.origins.length === 0) {
    warnings.push({
      path: 'security.cors.origins',
      message: 'CORS is enabled with no explicit origins. All origins will be reflected.',
    });
  }
