...
const DISCIPLINE_KEYWORDS: Record<string, string[]> = {
  engineering: [
    'deploy', 'code', 'bug', 'api', 'database', 'server', 'build', 'test',
    'CI', 'pipeline', 'git', 'merge', 'PR', 'pull request', 'sprint',
  ],
  design: [
  ...
  marketing: [
    'campaign', 'SEO', 'analytics', 'funnel', 'conversion', 'brand',
    'content', 'social media', 'engagement',
  ],
  finance: [
    'budget', 'invoice', 'expense', 'revenue', 'forecast', 'P&L', 'ROI',
    'cost', 'payment',
  ],
  hr: [
    'hiring', 'onboarding', 'performance review', 'PTO', 'benefits',
    'compensation', 'culture',
  ],
  legal: [
    'contract', 'compliance', 'NDA', 'terms', 'policy', 'regulation',
    'liability', 'IP',
  ],
  operations: [
    'process', 'workflow', 'SOP', 'vendor', 'logistics', 'supply chain',
    'inventory',
  ],
  product: [
    'roadmap', 'feature', 'backlog', 'user story', 'requirement', 'milestone',
    'release', 'MVP',
  ],
};
...
const POSITIVE_WORDS = new Set([
  'great', 'good', 'excellent', 'awesome', 'perfect', 'thanks', 'thank',
  'helpful', 'nice', 'fantastic', 'love', 'wonderful', 'amazing', 'success',
  'resolved', 'fixed', 'done', 'completed', 'approve', 'approved',
]);

const NEGATIVE_WORDS = new Set([
  'bad', 'broken', 'fail', 'failed', 'error', 'wrong', 'issue', 'problem',
  'bug', 'crash', 'down', 'outage', 'blocked', 'stuck', 'slow', 'terrible',
  'awful', 'hate', 'frustrated', 'urgent', 'critical', 'emergency',
]);
...
function countKeywordHits(lowered: string, keywords: string[]): number {
  return keywords.reduce((n, kw) => (lowered.includes(kw.toLowerCase()) ? n + 1 : n), 0);
}
...
export class ContentAnalyzer {
  ...
  analyze(message: NormalizedMessage): ContentAnalysis {
    ...
    for (const [discipline, keywords] of Object.entries(this.disciplineKeywords)) {
    ...
    const topics = extractTopics(lowered, this.disciplineKeywords, disciplineScores);

    // 8. Intent - derive a brief intent label from the top discipline or urgency
    const intent = deriveIntent(requiredDisciplines, urgency, isThreadContinuation);
...
export function createContentAnalyzer(
  config?: { customKeywords?: Record<string, string[]> },
): ContentAnalyzer {
