...
const createIntelligentWorkflow = async repoContext => {
  ...
  await mcp__ruflo__agent_spawn({ type: 'architect', name: 'Workflow Architect' });
  await mcp__ruflo__agent_spawn({ type: 'coder', name: 'YAML Generator' });
  await mcp__ruflo__agent_spawn({ type: 'optimizer', name: 'Performance Optimizer' });
  await mcp__ruflo__agent_spawn({ type: 'tester', name: 'Workflow Validator' });
