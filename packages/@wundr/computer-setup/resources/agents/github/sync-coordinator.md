...
const syncConflictResolver = async conflicts => {
  ...
  await mcp__ruflo__agent_spawn({ type: 'analyst', name: 'Conflict Analyzer' });
  await mcp__ruflo__agent_spawn({ type: 'coder', name: 'Resolution Developer' });
  await mcp__ruflo__agent_spawn({ type: 'reviewer', name: 'Solution Validator' });
  ...
  return await mcp__ruflo__task_orchestrate({
