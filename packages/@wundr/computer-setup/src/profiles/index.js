...
class ProfileManager {
    ...
    async listProfiles() {
        ...
        if (this.profiles.size === 0) {
            ...
            ].map(role => this.getPredefinedProfile(role)).filter(Boolean);
    ...
    getFullstackPreferences() {
        return this.getDefaultPreferences();
    }
    getFullstackTools() {
        const tools = this.getDefaultTools();
        tools.languages.node.globalPackages.push('vite', 'next', 'express', 'fastify', 'prisma', 'pm2');
        tools.databases = {
            postgresql: true,
            redis: true,
            mongodb: false,
