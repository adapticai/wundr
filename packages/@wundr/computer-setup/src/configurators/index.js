...
class ConfiguratorService {
     ...
     */
    async getGitConfigSteps(config) {
        const steps = [];
        steps.push({
            id: 'git-user-config',
     ...
     */
    async configureGPGSigning(config) {
        ...
        (0, child_process_1.execSync)(`git config --global user.signingkey ${config.gpgKey}`);
        (0, child_process_1.execSync)('git config --global commit.gpgsign true');
    ...
    async configureSSHKey(config) {
        ...
        if (!config.sshKey) {
            ...
            if (!await fs.pathExists(keyPath)) {
                (0, child_process_1.execSync)(`ssh-keygen -t ed25519 -C "${config.userEmail}" -f ${keyPath} -N ""`);
                logger.info('Generated new SSH key');
            }
        }
        // Add to SSH agent
        try {
            (0, child_process_1.execSync)('eval "$(ssh-agent -s)"');
            (0, child_process_1.execSync)(`ssh-add ${path.join(sshDir, 'id_ed25519')}`);
        }
        catch (error) {
     ...
     */
    async configureGitAliases(aliases) {
        logger.info('Configuring Git aliases');
        for (const [alias, command] of Object.entries(aliases)) {
            (0, child_process_1.execSync)(`git config --global alias.${alias} "${command}"`);
        }
...
`;
     ...
     */
    async generateShellConfig(profile) {
...
`;
     ...
     */
    async configureClaudeFlow(aiTools) {
     ...
     */
    recordConfigChange(file, changes, backup) {
