...
class MacInstaller {
    ...
    async getVersion() {
        try {
            const { stdout } = await (0, execa_1.execa)('sw_vers', ['-productVersion']);
    ...
    async validate() {
        try {
            await (0, which_1.default)('brew');
            await (0, execa_1.execa)('xcode-select', ['-p']);
            return true;
        }
        catch {
            return false;
    ...
    async installXcodeCommandLineTools() {
        try {
    ...
    async installEssentialPackages(profile) {
        ...
        for (const pkg of essentialPackages) {
            try {
                await (0, execa_1.execa)('brew', ['install', pkg]);
            }
            catch (error) {
    ...
    async installApplications(profile) {
        ...
        for (const app of applications.masApps) {
            try {
                await (0, execa_1.execa)('mas', ['install', app.id]);
            }
            catch (error) {
    ...
    getApplicationsForProfile(profile) {
        const casks = [];
        const masApps = [];
        ...
        casks.push('iterm2', 'rectangle', // Window manager
        'raycast', // Spotlight replacement
        'obsidian', // Note-taking
        'the-unarchiver');
        return { casks, masApps };
    }
    async configureMacOS(profile) {
        const commands = [
        ...
        await (0, execa_1.execa)('killall', ['Finder']);
...
`;
        ...
        await (0, execa_1.execa)('chsh', ['-s', '/bin/zsh']);
    }
...
`;
        ...
        await (0, execa_1.execa)('chsh', ['-s', '/opt/homebrew/bin/fish']);
    }
...
`;
    ...
    // Validation methods
    async validateXcodeCommandLineTools() {
        try {
            await (0, execa_1.execa)('xcode-select', ['-p']);
            return true;
        }
    ...
    async validateApplications(profile) {
        // Basic validation - check if key applications are installed
        const applications = this.getApplicationsForProfile(profile);
        // This is a simplified check
        return applications.casks.length > 0;
    }
    async validateMacOSConfig() {
        try {
            // Check if some key settings are applied
            const { stdout } = await (0, execa_1.execa)('defaults', ['read', 'com.apple.finder', 'ShowPathbar']);
