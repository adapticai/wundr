/**
 * VP Daemon Installer - Global VP Supervisor Daemon installation
 *
 * Installs the VP (Virtual Principal) Daemon at the global level:
 * - ~/vp-daemon/ - VP daemon runtime and configuration
 * - ~/.wundr/ - Global wundr configuration and resources
 *
 * The VP daemon is responsible for:
...
export interface VPDaemonConfig {
  /** Base directory for VP daemon (default: ~/vp-daemon) */
  readonly vpDaemonDir?: string;
...
/**
 * Result of VP Daemon installation
 */
export interface VPDaemonInstallResult {
  readonly success: boolean;
  readonly vpDaemonPath: string;
...
/**
 * VP Daemon Installer class
 */
export class VPDaemonInstaller extends EventEmitter {
  readonly name = 'vp-daemon';
  private readonly logger: Logger;
  private readonly homeDir: string;
  private vpDaemonDir: string;
  private wundrConfigDir: string;

  constructor(config: VPDaemonConfig = {}) {
    super();
    this.logger = new Logger({ name: 'VPDaemonInstaller' });
    this.homeDir = os.homedir();
    this.vpDaemonDir = config.vpDaemonDir || path.join(this.homeDir, 'vp-daemon');
    this.wundrConfigDir = config.wundrConfigDir || path.join(this.homeDir, '.wundr');
  }

  /**
   * Check if VP daemon installation is supported on current platform
   */
  ...
  /**
   * Check if VP daemon is already installed
   */
  async isInstalled(): Promise<boolean> {
    try {
      const vpCharterExists = await fs
        .access(path.join(this.vpDaemonDir, 'vp-charter.yaml'))
        .then(() => true)
        .catch(() => false);
      ...
      return vpCharterExists && wundrConfigExists;
    } catch {
      return false;
    }
  }
  ...
  /**
   * Get VP daemon version
   */
  async getVersion(): Promise<string | null> {
    try {
      const charterPath = path.join(this.vpDaemonDir, 'vp-charter.yaml');
  ...
  /**
   * Install VP daemon globally with detailed result
   */
  async installWithResult(): Promise<VPDaemonInstallResult> {
    const installedResources: string[] = [];
    const errors: Error[] = [];
    const warnings: string[] = [];

    try {
      this.logger.info('Installing VP Daemon globally...');
      ...
      this.emit('progress', { step: 'Copying VP daemon files', percentage: 25 });

      // 2. Copy VP daemon files
      await this.installVPDaemonFiles();
      installedResources.push('vp-daemon-files');
      ...
      this.logger.info('VP Daemon installed successfully');

      return {
        success: true,
        vpDaemonPath: this.vpDaemonDir,
      ...
      this.logger.error('VP Daemon installation failed:', error);
  ...
  /**
   * Validate VP daemon installation
   */
  async validate(): Promise<boolean> {
    try {
      // Check VP daemon directory
      const vpDaemonExists = await fs
        .access(this.vpDaemonDir)
        .then(() => true)
        .catch(() => false);

      if (!vpDaemonExists) {
        this.logger.warn('VP daemon directory not found');
        return false;
      }
      ...
      if (!charterExists) {
        this.logger.warn('VP charter not found');
        return false;
      }
  ...
  getSteps(_profile: DeveloperProfile, _platform: SetupPlatform): SetupStep[] {
    return [
      {
        id: 'install-vp-daemon',
        name: 'Install VP Daemon',
        description: 'Install VP Supervisor Daemon at global level',
        category: 'ai',
        required: true,
  ...
  private async createDirectoryStructure(): Promise<void> {
    const vpDaemonDirs = [
      this.vpDaemonDir,
      path.join(this.vpDaemonDir, 'sessions'),
      path.join(this.vpDaemonDir, 'logs'),
      path.join(this.vpDaemonDir, 'integrations'),
    ];
    ...
    for (const dir of [...vpDaemonDirs, ...wundrDirs]) {
  ...
  /**
   * Install VP daemon specific files
   */
  private async installVPDaemonFiles(): Promise<void> {
    const resourcesDir = this.getResourcesDir();
    const vpDaemonResourceDir = path.join(resourcesDir, 'vp-daemon');

    try {
      // Copy vp-charter.yaml
      const charterSrc = path.join(vpDaemonResourceDir, 'vp-charter.yaml');
      const charterDest = path.join(this.vpDaemonDir, 'vp-charter.yaml');
      ...
      if (charterExists) {
        ...
        this.logger.info('Copied VP charter');
      } else {
        // Create default VP charter
        await this.createDefaultVPCharter();
      }
  ...
  /**
   * Create default VP charter if not exists in resources
   */
  private async createDefaultVPCharter(): Promise<void> {
    const charter = `# VP Supervisor Daemon Charter
# This file defines the responsibilities and constraints of the VP daemon

version: "1.0.0"
name: "VP Supervisor Daemon"
...
`;
    ...
    await fs.writeFile(path.join(this.vpDaemonDir, 'vp-charter.yaml'), charter);
    this.logger.info('Created default VP charter');
  }
...
export const vpDaemonInstaller = new VPDaemonInstaller();
