...
export class VSCodeInstaller implements BaseInstaller {
  ...
  private async setupCodeCommand(): Promise<void> {
    try {
      ...
      // Add VS Code to PATH
      const vscodePath = '/Applications/Visual Studio Code.app/Contents/Resources/app/bin';
  ...
  private async configureVSCode(): Promise<void> {
    ...
    if (platform === 'darwin') {
      settingsPath = path.join(this.homeDir, 'Library/Application Support/Code/User/settings.json');
    } else {
  ...
  private async setupKeybindings(): Promise<void> {
    ...
    if (platform === 'darwin') {
      keybindingsPath = path.join(this.homeDir, 'Library/Application Support/Code/User/keybindings.json');
    } else {
      keybindingsPath = path.join(this.homeDir, '.config/Code/User/keybindings.json');
    }
  ...
  async uninstall(): Promise<void> {
    ...
    try {
      if (os.platform() === 'darwin') {
        // Remove VS Code app
        await fs.rm('/Applications/Visual Studio Code.app', { recursive: true });
        
        // Remove user data
        const userDataPath = path.join(this.homeDir, 'Library/Application Support/Code');
