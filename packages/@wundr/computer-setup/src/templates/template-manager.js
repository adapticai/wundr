...
class TemplateManager {
    ...
    async copyTemplates(options) {
        const spinner = (0, ora_1.default)('Copying and customizing templates').start();
        try {
            ...
            spinner.succeed(chalk_1.default.green('Templates processed successfully'));
        }
        catch (error) {
            spinner.fail(chalk_1.default.red(`Template processing failed: ${error}`));
            throw error;
        }
    }
    /**
     ...
     */
    async copyTemplate(templateName, outputPath, context, options) {
        const templatePath = this.getTemplatePath(templateName);
        if (!await fs.pathExists(templatePath)) {
            throw new Error(`Template not found: ${templateName}`);
        }
        if (!options?.overwrite && await fs.pathExists(outputPath)) {
            console.log(chalk_1.default.yellow(`Skipping existing file: ${outputPath}`));
            return;
        }
        ...
        if (options?.verbose) {
            console.log(chalk_1.default.blue(`Created: ${outputPath}`));
        }
    }
    /**
     * Generate configuration files for a project
     */
    async generateConfigs(projectPath, context, configs) {
        const spinner = (0, ora_1.default)('Generating configuration files').start();
        try {
            for (const config of configs) {
                await this.generateConfig(config, projectPath, context);
            }
            spinner.succeed(chalk_1.default.green('Configuration files generated'));
        }
        catch (error) {
            spinner.fail(chalk_1.default.red(`Configuration generation failed: ${error}`));
            throw error;
        }
    }
    /**
     ...
     */
    async generateConfig(configType, projectPath, context) {
        const configMap = {
            'eslint': {
     ...
     */
    async copyGitHubTemplates(projectPath, context) {
        ...
        for (const template of issueTemplates) {
            await this.copyTemplate(`github/ISSUE_TEMPLATE/${template}`, path.join(issueTemplatesDir, template), context, { overwrite: true });
        }
        // Copy PR template
        await this.copyTemplate('github/pull_request_template.md', path.join(githubDir, 'pull_request_template.md'), context, { overwrite: true });
    }
    /**
     * Process directory recursively
     */
    async processDirectory(templateDir, outputDir, options) {
     ...
     */
    async processFile(templatePath, outputPath, options) {
        if (!options.overwrite && await fs.pathExists(outputPath)) {
            if (options.verbose) {
                console.log(chalk_1.default.yellow(`Skipping existing file: ${outputPath}`));
            }
        ...
        if (options.dryRun) {
            console.log(chalk_1.default.blue(`Would create: ${outputPath}`));
            return;
        }
        const content = await fs.readFile(templatePath, 'utf-8');
     ...
     */
    processArrays(content, variables) {
