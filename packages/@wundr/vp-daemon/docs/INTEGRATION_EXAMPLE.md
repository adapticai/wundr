...
This document provides a complete example of integrating the VP Daemon WebSocket server with the Neolith backend.
...
app.prepare().then(() => {
  ...
  wsServer.on('connection:authenticated', (connectionId, metadata) => {
    console.log(`[WS] Connection authenticated: ${metadata.vpId} (${connectionId})`);
  });
  ...
  wsServer.on('event:queued', (eventId, vpId) => {
    console.log(`[WS] Event queued for offline VP: ${eventId} → ${vpId}`);
  });
...
export class VPDaemonClient extends EventEmitter {
...
// Usage Example
const client = new VPDaemonClient({
