/**
 * VP Daemon unit tests
 */

import { VPDaemon } from '../src/core/vp-daemon';
...
describe('VPDaemon', () => {
  ...
  beforeEach(() => {
    daemon = new VPDaemon(testConfig);
  });
  ...
  describe('constructor', () => {
    ...
    it('should throw error with invalid config', () => {
      expect(() => {
        new VPDaemon({ port: -1 } as DaemonConfig);
      }).toThrow();
    });
  });
  ...
  describe('spawnSession', () => {
    ...
    it('should track active sessions', async () => {
      ...
      await daemon.spawnSession('vp-1', task);
