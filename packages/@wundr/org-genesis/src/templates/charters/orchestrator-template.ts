/**
 * @fileoverview Orchestrator Charter Template
 *
 * This module provides default templates and factory functions for creating
 * Orchestrator charters. VPs are Tier 1 supervisory agents responsible
 * for context compilation, resource management, and coordinating Session Managers.
 *
 * The Orchestrator acts as a facilitator and orchestrator - it does NOT write code directly.
 ...
 *
 * @module @wundr/org-genesis/templates/charters/orchestrator-template
 * @version 1.0.0
 */

import { generateOrchestratorId, generateSlug } from '../../utils/index.js';
...
/**
 * Version of the Orchestrator charter template.
 * Used for migration and compatibility checks.
 */
export const ORCHESTRATOR_CHARTER_TEMPLATE_VERSION = '1.0.0';

// ============================================================================
// Default Orchestrator Capabilities
// ============================================================================

/**
 * Default capabilities granted to Orchestrator agents.
 *
 * These capabilities define the operational permissions for an Orchestrator:
 ...
 */
export const ORCHESTRATOR_TEMPLATE_CAPABILITIES: OrchestratorCapability[] = [
  'context_compilation',
  'resource_management',
  'slack_operations',
  'session_spawning',
...
/**
 * Default MCP tools available to Orchestrator agents.
 *
 * These tools enable the Orchestrator to perform its coordination duties:
 ...
 */
export const ORCHESTRATOR_TEMPLATE_MCP_TOOLS: string[] = [
  'slack',
  'filesystem',
  'shell',
  'orchestrator',
 ...
 */
export const ORCHESTRATOR_DEFAULT_RESOURCE_LIMITS: ResourceLimits = {
 ...
 * - `customMetrics`: Orchestrator-specific metrics for performance tracking
 *
 * @remarks
 * Orchestrator objectives focus on coordination efficiency and throughput
 * rather than direct implementation metrics.
 */
export const ORCHESTRATOR_DEFAULT_OBJECTIVES: MeasurableObjectives = {
 ...
 */
export const ORCHESTRATOR_DEFAULT_CONSTRAINTS: HardConstraints = {
  forbiddenCommands: [
    'rm -rf /',
    'rm -rf /*',
    'rm -rf ~',
...
/**
 * Default Orchestrator Charter template with sensible defaults.
 *
 * This template provides the foundational structure for Orchestrator agents.
 ...
 * a Orchestrator charter, as they are unique to each Orchestrator instance.
 *
 * The Orchestrator's core directive emphasizes facilitation over execution:
 * VPs compile context, manage resources, and coordinate Session
 * Managers - they do NOT write code directly.
 *
 * @example
 * ```typescript
 * const myOrchestratorCharter: OrchestratorCharter = {
 ...
 * ```
 */
export const DEFAULT_ORCHESTRATOR_CHARTER: Omit<OrchestratorCharter, 'id' | 'identity'> = {
  tier: 1,
  coreDirective:
  ...
  capabilities: ORCHESTRATOR_TEMPLATE_CAPABILITIES,
  mcpTools: ORCHESTRATOR_TEMPLATE_MCP_TOOLS,
  resourceLimits: ORCHESTRATOR_DEFAULT_RESOURCE_LIMITS,
  objectives: ORCHESTRATOR_DEFAULT_OBJECTIVES,
  constraints: ORCHESTRATOR_DEFAULT_CONSTRAINTS,
  disciplineIds: [],
  createdAt: new Date(),
  updatedAt: new Date(),
};
...
// ============================================================================
// Orchestrator Charter Creation Options
// ============================================================================

/**
 * Options for creating a Orchestrator charter from the template.
 *
 * All fields are optional except `name` - defaults from the template
 * will be used for any omitted fields.
 *
 * @property name - Required: Human-readable name for the Orchestrator
 ...
 */
export interface CreateOrchestratorCharterOptions {
  /** Human-readable name for the Orchestrator (required) */
  name: string;
  ...
  /** URL to the Orchestrator's avatar image for UI display */
  avatarUrl?: string;
...
// ============================================================================
// Orchestrator Charter Factory Function
// ============================================================================

/**
 * Create a new Orchestrator Charter with the provided options.
 *
 * This factory function creates a complete OrchestratorCharter by merging
 * the provided options with sensible defaults from the template.
 *
 * @param options - Configuration options for the Orchestrator charter
 * @returns A complete OrchestratorCharter ready for registration
 *
 * @remarks
 * - A unique ID is automatically generated using `generateOrchestratorId()`
 ...
 * const minimalOrchestrator = createOrchestratorCharter({ name: 'Engineering VP' });
 *
 * // Full customization
 * const customOrchestrator = createOrchestratorCharter({
 *   name: 'Legal VP',
 ...
 * const specializedOrchestrator = createOrchestratorCharter({
 *   name: 'Security VP',
 ...
 * ```
 */
export function createOrchestratorCharter(options: CreateOrchestratorCharterOptions): OrchestratorCharter {
  const now = new Date();
  ...
  const identity: AgentIdentity = {
    ...
    persona:
      options.persona ||
      `A capable and efficient Orchestrator responsible for coordinating ${options.name} operations.`,
  ...
  const resourceLimits: ResourceLimits = {
    ...ORCHESTRATOR_DEFAULT_RESOURCE_LIMITS,
    ...options.resourceLimits,
  };

  // Merge objectives with defaults
  const objectives: MeasurableObjectives = {
    ...ORCHESTRATOR_DEFAULT_OBJECTIVES,
    ...options.objectives,
    customMetrics: {
      ...ORCHESTRATOR_DEFAULT_OBJECTIVES.customMetrics,
  ...
  const constraints: HardConstraints = {
    forbiddenCommands: [
      ...ORCHESTRATOR_DEFAULT_CONSTRAINTS.forbiddenCommands,
      ...(options.constraints?.forbiddenCommands || []),
    ],
    forbiddenPaths: [
      ...ORCHESTRATOR_DEFAULT_CONSTRAINTS.forbiddenPaths,
      ...(options.constraints?.forbiddenPaths || []),
    ],
    forbiddenActions: [
      ...ORCHESTRATOR_DEFAULT_CONSTRAINTS.forbiddenActions,
      ...(options.constraints?.forbiddenActions || []),
    ],
    requireApprovalFor: [
      ...ORCHESTRATOR_DEFAULT_CONSTRAINTS.requireApprovalFor,
  ...
  const mcpTools = [
    ...ORCHESTRATOR_TEMPLATE_MCP_TOOLS,
    ...(options.mcpTools || []).filter((tool) => !ORCHESTRATOR_TEMPLATE_MCP_TOOLS.includes(tool)),
  ];
  ...
  return {
    ...
    tier: 1,
    identity,
    coreDirective: options.coreDirective || DEFAULT_ORCHESTRATOR_CHARTER.coreDirective,
    capabilities: options.capabilities || ORCHESTRATOR_TEMPLATE_CAPABILITIES,
    mcpTools,
    resourceLimits,
    objectives,
    constraints,
...
// ============================================================================
// Orchestrator Charter from Partial
// ============================================================================

/**
 * Create a Orchestrator Charter from partial data, filling in missing fields with defaults.
 *
 * This function is useful when loading partial charter data from storage
 * or when migrating from older charter formats.
 *
 * @param partial - Partial Orchestrator charter data
 * @returns A complete OrchestratorCharter with defaults applied to missing fields
 *
 * @example
 * ```typescript
 * // Restore from storage with missing fields
 * const storedData = JSON.parse(storedJson);
 * const charter = createOrchestratorCharterFromPartial(storedData);
 * ```
 */
export function createOrchestratorCharterFromPartial(partial: Partial<OrchestratorCharter>): OrchestratorCharter {
  const now = new Date();
  ...
  const identity: AgentIdentity = {
    ...
    persona: partial.identity?.persona || 'A Orchestrator agent.',
  ...
  return {
    id: partial.id || generateOrchestratorId(),
    tier: 1,
    identity,
    coreDirective: partial.coreDirective || DEFAULT_ORCHESTRATOR_CHARTER.coreDirective,
    capabilities: partial.capabilities || ORCHESTRATOR_TEMPLATE_CAPABILITIES,
    mcpTools: partial.mcpTools || ORCHESTRATOR_TEMPLATE_MCP_TOOLS,
    resourceLimits: {
      ...ORCHESTRATOR_DEFAULT_RESOURCE_LIMITS,
    ...
    constraints: {
      forbiddenCommands:
        partial.constraints?.forbiddenCommands || ORCHESTRATOR_DEFAULT_CONSTRAINTS.forbiddenCommands,
      forbiddenPaths: partial.constraints?.forbiddenPaths || ORCHESTRATOR_DEFAULT_CONSTRAINTS.forbiddenPaths,
      forbiddenActions:
        partial.constraints?.forbiddenActions || ORCHESTRATOR_DEFAULT_CONSTRAINTS.forbiddenActions,
      requireApprovalFor:
        partial.constraints?.requireApprovalFor || ORCHESTRATOR_DEFAULT_CONSTRAINTS.requireApprovalFor,
    },
...
// ============================================================================
// Specialized Orchestrator Templates
// ============================================================================

/**
 * Create an Engineering Orchestrator charter with engineering-focused defaults.
 *
 * @param options - Configuration options for the Orchestrator charter
 * @returns An Engineering Orchestrator charter
 *
 * @example
 * ```typescript
 * const engOrchestrator = createEngineeringOrchestratorCharter({
 *   slackHandle: 'eng-vp',
 *   disciplineIds: ['frontend', 'backend', 'devops'],
 * });
 * ```
 */
export function createEngineeringOrchestratorCharter(
  options: Partial<Omit<CreateOrchestratorCharterOptions, 'name'>> = {},
): OrchestratorCharter {
  return createOrchestratorCharter({
    name: 'Engineering VP',
    persona:
...
/**
 * Create a Product Orchestrator charter with product management focused defaults.
 *
 * @param options - Configuration options for the Orchestrator charter
 * @returns A Product Orchestrator charter
 *
 * @example
 * ```typescript
 * const productOrchestrator = createProductOrchestratorCharter({
 *   disciplineIds: ['product', 'ux', 'analytics'],
 * });
 * ```
 */
export function createProductOrchestratorCharter(
  options: Partial<Omit<CreateOrchestratorCharterOptions, 'name'>> = {},
): OrchestratorCharter {
  return createOrchestratorCharter({
    name: 'Product VP',
...
/**
 * Create an Operations Orchestrator charter with operations focused defaults.
 *
 * @param options - Configuration options for the Orchestrator charter
 * @returns An Operations Orchestrator charter
 *
 * @example
 * ```typescript
 * const opsOrchestrator = createOperationsOrchestratorCharter({
 *   disciplineIds: ['infrastructure', 'security', 'compliance'],
 * });
 * ```
 */
export function createOperationsOrchestratorCharter(
  options: Partial<Omit<CreateOrchestratorCharterOptions, 'name'>> = {},
): OrchestratorCharter {
  return createOrchestratorCharter({
    name: 'Operations VP',
