...
export const quickstartCommand = new Command('quickstart')
  ...
  .option(
    '-p, --profile <profile>',
    'Profile type (basic|developer|full)',
    'developer'
  )
  ...
  .option(
    '--preset <preset>',
    'Use predefined preset (minimal|standard|full)',
    'standard'
  )
  .action(async options => {
    ...
    try {
      ...
      // eslint-disable-next-line no-console
      console.log(
        `   🎯 Target: <300 seconds (${Number(elapsed) < 300 ? '✅ PASSED' : '❌ EXCEEDED'})`
      );
      // eslint-disable-next-line no-console
      ...
      // eslint-disable-next-line no-console
      console.log(
        '   wundr-env quickstart --preset minimal    # Minimal setup'
      );
      // eslint-disable-next-line no-console
      console.log(
        '   wundr-env quickstart --skip-ai           # Skip AI components'
      );
      // eslint-disable-next-line no-console
      console.log(
        '   wundr-env install --profile basic        # Full traditional setup'
      );
      process.exit(1);
    }
  });
...
export const initCommand = new Command('init')
  ...
  .option(
    '-p, --profile <type>',
    'Environment profile (human|ai-agent|ci-runner)',
    'human'
  )
  ...
  .action(async options => {
    ...
    try {
      ...
      // eslint-disable-next-line no-console
      console.log(
        `Development Path: ${chalk.yellow(config.paths.development)}`
      );
    } catch (error) {
...
export const installCommand = new Command('install')
  ...
  .action(async _options => {
    ...
    try {
      ...
      if (!config) {
        throw new Error(
          'No environment configuration found. Run "wundr-env init" first.'
        );
      }
  ...
  .action(async () => {
    try {
      ...
      if (!config) {
        ...
        console.log(
          chalk.blue('Run "wundr-env init" to create a new environment')
        );
        return;
      }
  ...
  .action(async options => {
    ...
    try {
      ...
      if (updatedConfig.preferences.fullName) {
        console.log(
          `Name: ${chalk.yellow(updatedConfig.preferences.fullName)}`
        );
      }
      if (updatedConfig.preferences.githubUsername) {
        console.log(
          `GitHub: ${chalk.yellow(updatedConfig.preferences.githubUsername)}`
        );
      }
    } catch (error) {
...
export const program = new Command()
  .name('wundr-env')
  .description(
    'Wundr Environment Manager - Cross-platform development environment setup'
  )
  .version('1.0.0');
