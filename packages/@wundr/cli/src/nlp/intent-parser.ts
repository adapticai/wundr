...
export interface Entity {
  text: string;
  type: 'file_path' | 'package_name' | 'command' | 'option' | 'value' | 'technology';
  confidence: number;
  startIndex: number;
  endIndex: number;
}
...
export class IntentParser extends EventEmitter {
  ...
  constructor(claudeClient: ClaudeClient, config: Partial<IntentParserConfig> = {}) {
    super();
  ...
  async parseIntent(
    ...
    try {
      ...
      const entities = this.config.enableNER ? await this.extractEntities(normalizedInput) : [];
      ...
      this.emit('intent_parsed', { input: normalizedInput, result: validatedResult });
...
Extract parameters from the user input and respond with JSON only:
{
      ...
      for (const param of commandPattern.parameters) {
        ...
        if (result.parameters[param.name] && param.validation) {
          if (!param.validation.test(result.parameters[param.name])) {
            logger.warn(`Parameter validation failed for ${param.name}: ${result.parameters[param.name]}`);
            delete result.parameters[param.name];
          }
        }

        // Apply default values
        if (!result.parameters[param.name] && param.defaultValue !== undefined) {
  ...
  getCacheStats(): {
    ...
    const now = Date.now();
    const validEntries = Array.from(this.intentCache.values())
      .filter(entry => now - entry.timestamp < this.config.cacheDuration);
    ...
    return {
      ...
      hitRate: this.intentCache.size > 0 ? validEntries.length / this.intentCache.size : 0,
      oldestEntry
    };
  }
    ...
    const defaultPatterns: CommandPattern[] = [
      {
        ...
        parameters: [
          ...
          { name: 'focus', type: 'enum', required: false, enumValues: ['dependencies', 'quality', 'security', 'performance'] },
          { name: 'format', type: 'enum', required: false, enumValues: ['json', 'table', 'csv'] }
        ],
        examples: [
          'analyze the project',
      ...
      },
      {
        ...
        parameters: [
          { name: 'type', type: 'enum', required: true, enumValues: ['component', 'service', 'test', 'config'] },
      ...
      },
      {
        ...
        parameters: [
          { name: 'view', type: 'enum', required: false, enumValues: ['overview', 'metrics', 'reports'] },
  ...
  private getCacheKey(input: string, commands: string[], context: IntentContext): string {
  ...
  private calculatePatternScore(input: string, pattern: CommandPattern): number {
    let score = 0;
    let totalWeight = 0;
...
Analyze and respond with JSON only:
{
  ...
  private getPatternSuggestions(
    partialInput: string,
    availableCommands: string[]
  ): Array<{ command: string; description: string; confidence: number; completion: string }> {
    const suggestions: Array<{ command: string; description: string; confidence: number; completion: string }> = [];
    ...
    for (const command of availableCommands) {
      ...
      const confidence = Math.max(
        ...
        ...pattern.examples.map(ex => this.calculateStringSimilarity(partialInput, ex))
      );
  ...
  private async getAISuggestions(
    ...
    context: IntentContext,
    limit: number
  ): Promise<Array<{ command: string; description: string; confidence: number; completion: string }>> {
...
Provide ${limit} suggestions in JSON format:
{
  ...
  private analyzeSentiment(input: string): 'positive' | 'neutral' | 'negative' {
    const positiveWords = ['good', 'great', 'excellent', 'awesome', 'perfect', 'love', 'like', 'thanks', 'help', 'please'];
    const negativeWords = ['bad', 'terrible', 'awful', 'hate', 'problem', 'error', 'issue', 'broken', 'fail', 'wrong'];
